...
static uint32_t dist_prefill_send_depth(uint32_t chunk_count) {
  ...
  if (chunk_count != 0 && depth > chunk_count)
    depth = chunk_count;
  return depth ? depth : 1;
}

static int dist_send_work_frame(int fd, const ds4_dist_work_fixed *work,
                                const int *tokens, const float *input_hc,
                                const void *route_blob);
static int dist_write_full(int fd, const void *buf, size_t len);
static int dist_send_snapshot_file_chunks(int fd, uint64_t request_id, FILE *fp,
                                          uint64_t bytes);

static int dist_worker_handle_work(ds4_dist_worker_state *state,
                                   ds4_dist_worker_upstream *upstream,
                                   uint32_t bytes);
static int dist_worker_handle_snapshot_save(ds4_dist_worker_state *state,
                                            ds4_dist_worker_upstream *upstream,
                                            uint32_t bytes);
static int dist_worker_handle_snapshot_load(ds4_dist_worker_state *state,
                                            ds4_dist_worker_upstream *upstream,
                                            uint32_t bytes);
static void dist_worker_upstream_init(ds4_dist_worker_upstream *upstream,
                                      ds4_dist_worker_state *state, int fd);
static void dist_worker_upstream_destroy(ds4_dist_worker_upstream *upstream);
static int
dist_worker_upstream_send_work_error(ds4_dist_worker_upstream *upstream,
                                     uint64_t request_id, const char *msg);
static int dist_coordinator_prefill_prompt(ds4_dist_coordinator_state *state,
                                           ds4_session *session,
                                           const ds4_dist_route_plan *plan,
                                           const ds4_tokens *prompt,
                                           uint64_t session_id,
                                           uint64_t *request_id, float *logits,
                                           char *err, size_t errlen);
static int dist_validate_options(const ds4_dist_options *opt, char *err,
                                 size_t errlen);

static uint32_t dist_resolved_layer_end(const ds4_dist_options *opt,
                                        uint32_t n_layers) {
...
static const char *dist_role_name(ds4_distributed_role role) {
  ...
  case DS4_DISTRIBUTED_NONE:
    return "none";
  case DS4_DISTRIBUTED_COORDINATOR:
    return "coordinator";
  case DS4_DISTRIBUTED_WORKER:
    return "worker";
  }
  return "unknown";
}

static void dist_sleep_reconnect(void) { sleep(1); }
...
static int dist_payload_write_bytes(FILE *fp, const void *ptr, uint64_t bytes,
                                    char *err, size_t errlen) {
  const uint8_t *p = ptr;
  while (bytes != 0) {
    ...
    if (fwrite(p, 1, n, fp) != n) {
      if (errlen)
        snprintf(err, errlen, "failed to write distributed payload");
      return 1;
    }
    p += n;
    bytes -= n;
...
static int dist_payload_read_bytes(FILE *fp, void *ptr, uint64_t bytes,
                                   uint64_t *remaining, char *err,
                                   size_t errlen) {
  if (remaining && *remaining < bytes) {
    if (errlen)
      snprintf(err, errlen, "truncated distributed payload");
    return 1;
  }
  uint8_t *p = ptr;
  ...
  while (bytes != 0) {
    ...
    if (fread(p, 1, n, fp) != n) {
      if (errlen)
        snprintf(err, errlen, "failed to read distributed payload");
      return 1;
    }
    p += n;
    bytes -= n;
...
static int dist_payload_write_u32(FILE *fp, uint32_t v, char *err,
                                  size_t errlen) {
  uint8_t b[4] = {
      (uint8_t)v,
      (uint8_t)(v >> 8),
...
static int dist_payload_read_u32(FILE *fp, uint32_t *v, uint64_t *remaining,
                                 char *err, size_t errlen) {
  uint8_t b[4];
  if (dist_payload_read_bytes(fp, b, sizeof(b), remaining, err, errlen) != 0)
    return 1;
...
static int dist_payload_copy_bytes(FILE *src, FILE *dst, uint64_t bytes,
                                   uint64_t *remaining, char *err,
                                   size_t errlen) {
  if (remaining && *remaining < bytes) {
    if (errlen)
  ...
  if (!buf) {
    if (errlen)
      snprintf(err, errlen, "out of memory copying distributed payload");
    return 1;
  }
  int rc = 0;
  uint64_t left = bytes;
...
static int dist_copy_file_range(FILE *src, uint64_t offset, uint64_t bytes,
                                FILE *dst, char *err, size_t errlen) {
  if (offset > (uint64_t)LLONG_MAX ||
      fseeko(src, (off_t)offset, SEEK_SET) != 0) {
    if (errlen)
      snprintf(err, errlen, "failed to seek distributed KV shard");
    return 1;
  }
  return dist_payload_copy_bytes(src, dst, bytes, NULL, err, errlen);
}

static int dist_rewind_file(FILE *fp, const char *what, char *err,
                            size_t errlen) {
  if (fflush(fp) != 0 || fseeko(fp, 0, SEEK_SET) != 0) {
    if (errlen)
      snprintf(err, errlen, "failed to rewind %s", what);
    return 1;
  }
  return 0;
}

static int dist_measure_file(FILE *fp, uint64_t *bytes, const char *what,
                             char *err, size_t errlen) {
  if (!bytes)
    return 1;
  if (fflush(fp) != 0) {
    if (errlen)
      snprintf(err, errlen, "failed to flush %s", what);
    return 1;
  }
  off_t pos = ftello(fp);
  if (pos < 0) {
    if (errlen)
      snprintf(err, errlen, "failed to measure %s", what);
    return 1;
  }
  *bytes = (uint64_t)pos;
  return 0;
...
static FILE *dist_tmpfile_or_err(const char *what, char *err, size_t errlen) {
  FILE *fp = tmpfile();
  if (!fp && errlen)
    snprintf(err, errlen, "failed to create %s temp file: %s", what,
             strerror(errno));
  return fp;
}

static bool dist_u64_add(uint64_t *acc, uint64_t add) {
  if (!acc || *acc > UINT64_MAX - add)
    return false;
  *acc += add;
  return true;
}
...
static bool dist_parse_positive_u32(const char *s, const char *name,
                                    uint32_t *out, char *err, size_t errlen) {
  if (!s || !out) {
    if (errlen)
      snprintf(err, errlen, "%s requires a positive integer", name);
    return false;
  }
  errno = 0;
  char *end = NULL;
  ...
  if (errno != 0 || s[0] == '\0' || *end != '\0' || v == 0 || v > UINT32_MAX) {
    if (errlen)
      snprintf(err, errlen, "invalid value for %s: %s", name, s);
    return false;
  }
  *out = (uint32_t)v;
  return true;
...
static bool dist_activation_wire_bytes(uint32_t bits, uint64_t values,
                                       uint32_t *out) {
  bits = dist_activation_bits_or_default(bits);
  if (!dist_activation_bits_valid(bits) || (bits % 8u) != 0)
    return false;
...
static bool dist_activation_values_from_wire_bytes(uint32_t bits,
                                                   uint32_t bytes,
                                                   uint64_t *out) {
  bits = dist_activation_bits_or_default(bits);
  if (!dist_activation_bits_valid(bits) || (bits % 8u) != 0)
  ...
  if (bytes_per_value == 0 || (bytes % bytes_per_value) != 0)
    return false;
  if (out)
    *out = bytes / bytes_per_value;
  return true;
}

static bool dist_activation_wire_bytes_from_f32_bytes(uint32_t bits,
                                                      uint32_t f32_bytes,
                                                      uint32_t *out) {
  if ((f32_bytes % (uint32_t)sizeof(float)) != 0)
    return false;
  return dist_activation_wire_bytes(bits, f32_bytes / (uint32_t)sizeof(float),
                                    out);
}
...
static uint8_t dist_f32_to_f8_e4m3(float f) {
  ...
  if (a == 0.0f)
    return sign;
  if (!isfinite(a) || a >= 240.0f)
    return (uint8_t)(sign | 0x77u);
...
static int dist_write_activation_payload(int fd, const float *src,
                                         uint64_t values, uint32_t bits) {
  bits = dist_activation_bits_or_default(bits);
  ...
  if (bits == 32u) {
    uint32_t bytes = 0;
    if (!dist_activation_wire_bytes(bits, values, &bytes))
      return -1;
  ...
  while (done < values) {
    ...
    if (bits == 16u) {
      ...
      for (uint64_t i = 0; i < n; i++)
        dst[i] = dist_f32_to_f16(src[done + i]);
    } else {
      uint8_t *dst = buf;
      for (uint64_t i = 0; i < n; i++)
        dst[i] = dist_f32_to_f8_e4m3(src[done + i]);
    }
...
static int dist_decode_activation_payload(const void *wire, uint32_t bits,
                                          uint32_t wire_bytes, float **out,
                                          uint32_t *out_f32_bytes,
                                          bool *out_uses_wire, char *err,
                                          size_t errlen) {
  if (out)
    *out = NULL;
  if (out_f32_bytes)
  ...
  if (!dist_activation_bits_valid(bits)) {
    if (errlen)
      snprintf(err, errlen, "invalid distributed activation width: %u bits",
               bits);
    return 1;
  }
  if (wire_bytes != 0 && !wire) {
    if (errlen)
      snprintf(err, errlen, "missing distributed activation payload");
    return 1;
  }
  ...
  if (!dist_activation_values_from_wire_bytes(bits, wire_bytes, &values)) {
    if (errlen)
      snprintf(err, errlen, "invalid distributed activation payload size");
    return 1;
  }
  const uint64_t f32_bytes64 = values * sizeof(float);
  if (f32_bytes64 > UINT32_MAX) {
    if (errlen)
      snprintf(err, errlen, "distributed activation payload is too large");
    return 1;
  }
  ...
  if (f32_bytes && !dst) {
    if (errlen)
      snprintf(err, errlen, "out of memory decoding distributed activations");
    return 1;
  }
  if (bits == 16u) {
    const uint16_t *src = wire;
    for (uint64_t i = 0; i < values; i++)
      dst[i] = dist_f16_to_f32(src[i]);
  } else {
    const uint8_t *src = wire;
    for (uint64_t i = 0; i < values; i++)
      dst[i] = dist_f8_e4m3_to_f32(src[i]);
  }
  if (out)
    *out = dst;
  if (out_f32_bytes)
...
static int dist_set_socket_low_latency(int fd) {
  ...
  if (recv_timeout_env && recv_timeout_env[0]) {
    ...
    if (end != recv_timeout_env && *end == '\0' && v > 0 && v <= 3600) {
      ...
      if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &recv_tv, sizeof(recv_tv)) !=
          0)
        rc = -1;
    }
  }
  if (buffer_bytes > 0 && setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &buffer_bytes,
                                     sizeof(buffer_bytes)) != 0)
    rc = -1;
  if (buffer_bytes > 0 && setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &buffer_bytes,
                                     sizeof(buffer_bytes)) != 0)
    rc = -1;
#ifdef SO_NOSIGPIPE
...
static void dist_sockaddr_string(const struct sockaddr *sa, socklen_t salen,
  ...
  if (getnameinfo(sa, salen, host, sizeof(host), service, sizeof(service),
...
static int dist_write_frame_header(int fd, uint32_t type, uint32_t bytes) {
  ds4_dist_frame_header h = {htonl(DS4_DIST_MAGIC), htonl(type), htonl(bytes)};
  return dist_write_full(fd, &h, sizeof(h));
}

static int dist_read_frame_header(int fd, uint32_t *type, uint32_t *bytes,
                                  char *err, size_t errlen) {
...
static int dist_send_error(int fd, const char *msg) {
  if (!msg)
    msg = "distributed protocol error";
  size_t len = strlen(msg);
  if (len > UINT32_MAX)
    len = UINT32_MAX;
  if (dist_write_frame_header(fd, DS4_DIST_MSG_ERROR, (uint32_t)len) != 0)
    return -1;
  return dist_write_full(fd, msg, len);
}

static void dist_peer_name(int fd, char *host, size_t hostlen, char *port,
                           size_t portlen) {
  if (hostlen)
    host[0] = '\0';
  if (portlen)
  ...
  if (getpeername(fd, (struct sockaddr *)&ss, &slen) == 0) {
    if (getnameinfo((struct sockaddr *)&ss, slen, host, (socklen_t)hostlen,
  ...
  if (portlen)
    snprintf(port, portlen, "0");
}

static int dist_open_listener(const char *host, int port, char *err,
                              size_t errlen) {
  char portbuf[16];
  ...
  if (gai != 0) {
    if (errlen)
      snprintf(err, errlen, "getaddrinfo(%s:%s): %s", host_display, portbuf,
               gai_strerror(gai));
    return -1;
  }
  ...
  if (listen_fd < 0 && errlen) {
    snprintf(err, errlen, "unable to listen on %s:%d: %s", host_display, port,
             strerror(errno));
  }
  return listen_fd;
}
...
static int dist_listener_port(int fd) {
  ...
  if (getsockname(fd, (struct sockaddr *)&ss, &slen) != 0)
    return 0;
  char service[NI_MAXSERV];
  if (getnameinfo((struct sockaddr *)&ss, slen, NULL, 0, service,
                  sizeof(service), NI_NUMERICSERV) != 0) {
    return 0;
  }
  char *end = NULL;
  unsigned long v = strtoul(service, &end, 10);
  if (end == service || *end != '\0' || v > 65535ul)
    return 0;
  return (int)v;
}

static bool dist_connect_errno_retryable(int e) {
  return e == ECONNREFUSED || e == EHOSTUNREACH || e == ENETUNREACH ||
         e == ETIMEDOUT || e == EADDRNOTAVAIL;
}

static int dist_bind_connect_source(int fd, int family, const char *host,
                                    int *bind_errno) {
  if (bind_errno)
    *bind_errno = 0;
  if (!host || !host[0])
...
static int dist_bind_connect_interface(int fd, int family, const char *ifname,
                                       int *bind_errno) {
  (void)fd;
  if (bind_errno)
    *bind_errno = 0;
  ...
  if (rc != 0) {
    if (bind_errno)
      *bind_errno = errno ? errno : EIO;
    return -1;
  }
  return 0;
}

static int dist_connect_endpoint_once(const char *host, int port,
                                      int *last_errno, char *err,
                                      size_t errlen) {
  char portbuf[16];
  snprintf(portbuf, sizeof(portbuf), "%d", port);
  if (last_errno)
  ...
  if (gai != 0) {
    if (errlen)
      snprintf(err, errlen, "getaddrinfo(%s:%s): %s", host, portbuf,
               gai_strerror(gai));
    if (last_errno)
      *last_errno = EINVAL;
    return -1;
  ...
  for (struct addrinfo *ai = res; ai; ai = ai->ai_next) {
    ...
    if (trace) {
      dist_sockaddr_string(ai->ai_addr, ai->ai_addrlen, dst_desc,
                           sizeof(dst_desc));
      fprintf(stderr,
              "ds4: distributed connect trace: candidate dst=%s family=%d "
              "bind=%s if=%s\n",
    ...
    if (bind_if) {
      int bind_errno = 0;
      if (dist_bind_connect_interface(fd, ai->ai_family, bind_if,
                                      &bind_errno) != 0) {
        saved_errno = bind_errno ? bind_errno : ENODEV;
        if (trace) {
          fprintf(
              stderr,
              "ds4: distributed connect trace: bind interface %s failed: %s\n",
    ...
    if (bind_host) {
      int bind_errno = 0;
      if (dist_bind_connect_source(fd, ai->ai_family, bind_host, &bind_errno) !=
          0) {
        saved_errno = bind_errno ? bind_errno : EADDRNOTAVAIL;
        if (trace) {
          fprintf(stderr,
                  "ds4: distributed connect trace: bind source %s failed: %s\n",
    ...
    if (connect(fd, ai->ai_addr, ai->ai_addrlen) == 0) {
      if (trace) {
        ...
        if (getsockname(fd, (struct sockaddr *)&local_ss, &local_len) == 0) {
          ...
          fprintf(
              stderr,
              "ds4: distributed connect trace: connected source=%s dst=%s\n",
              local_desc, dst_desc);
        }
      }
      break;
    ...
    if (trace) {
      fprintf(stderr,
              "ds4: distributed connect trace: connect to %s failed: %s\n",
  ...
  if (fd < 0 && errlen) {
    if (saved_errno == 0)
      saved_errno = EIO;
    snprintf(err, errlen, "unable to connect to %s:%d: %s", host, port,
             strerror(saved_errno));
  }
  if (fd < 0 && last_errno)
    *last_errno = saved_errno ? saved_errno : EIO;
  return fd;
}
...
static int dist_connect_endpoint(const char *host, int port, char *err,
  ...
  for (int attempt = 0; attempt < 200; attempt++) {
    ...
    if (fd >= 0)
      return fd;
    if (!dist_connect_errno_retryable(last_errno))
      break;
    struct timespec ts = {0, 25 * 1000 * 1000};
    nanosleep(&ts, NULL);
  }
...
static uint64_t dist_token_hash_update_span(uint64_t h, const int *tokens,
                                            uint32_t n_tokens) {
  for (uint32_t i = 0; i < n_tokens; i++)
    h = dist_token_hash_update(h, tokens[i]);
  return h;
}

static uint64_t dist_token_hash_prefix(const int *tokens, uint32_t n_tokens) {
  return dist_token_hash_update_span(DS4_DIST_TOKEN_HASH_INIT, tokens,
                                     n_tokens);
}

static int dist_session_token_hash_prefix(ds4_session *session,
                                          uint32_t n_tokens, uint64_t *hash,
                                          char *err, size_t errlen) {
  const ds4_tokens *checkpoint = ds4_session_tokens(session);
  if (!hash || !checkpoint || checkpoint->len < 0 ||
      (uint32_t)checkpoint->len < n_tokens) {
    if (errlen)
      snprintf(err, errlen, "distributed session has no %u-token prefix",
               n_tokens);
    return 1;
  }
...
static int dist_send_hello(ds4_engine *engine, const ds4_dist_options *opt,
                           int ctx_size, uint32_t listen_port, int fd) {
  ...
  if (model_name_len > DS4_DIST_MAX_MODEL_NAME)
    model_name_len = DS4_DIST_MAX_MODEL_NAME;

  ds4_dist_hello_fixed h = {(uint32_t)ds4_engine_model_id(engine),
  ...
  if (dist_write_frame_header(fd, DS4_DIST_MSG_HELLO, bytes) != 0)
    return -1;
  if (dist_write_full(fd, &wire, sizeof(wire)) != 0)
    return -1;
  if (model_name_len && dist_write_full(fd, model_name, model_name_len) != 0)
    return -1;
  return 0;
}

static int dist_recv_hello(int fd, ds4_dist_hello_fixed *hello,
                           char *model_name, size_t model_name_cap, char *err,
                           size_t errlen) {
  ...
  if (type != DS4_DIST_MSG_HELLO) {
    if (errlen)
      snprintf(err, errlen, "expected HELLO frame, got type %u", type);
    dist_discard_bytes(fd, bytes);
    return -1;
  }
  if (bytes < sizeof(*hello) ||
      bytes > sizeof(*hello) + DS4_DIST_MAX_MODEL_NAME) {
    if (errlen)
      snprintf(err, errlen, "invalid HELLO payload length %u", bytes);
    dist_discard_bytes(fd, bytes);
    return -1;
  }
  ...
  if (wire.model_name_len != remaining ||
      wire.model_name_len > DS4_DIST_MAX_MODEL_NAME) {
    if (errlen)
      snprintf(err, errlen, "invalid HELLO model name length %u",
               wire.model_name_len);
  ...
  if (wire.model_name_len) {
    ...
    if (dist_bytes_have_nul(tmp, wire.model_name_len)) {
      if (errlen)
        snprintf(err, errlen, "HELLO model family contains NUL bytes");
      return -1;
    }
...
static bool
dist_coordinator_debug_enabled(const ds4_dist_coordinator_state *state) {
  return state && state->debug;
}

#define DIST_COORD_DEBUG(state, ...)                                           \
  do {                                                                         \
...
static void dist_coordinator_add_worker(ds4_dist_coordinator_state *state,
                                        int fd, const char *peer_host,
                                        const char *peer_port,
                                        const ds4_dist_hello_fixed *hello,
                                        const char *model_name) {
  ds4_dist_worker_entry *entry = calloc(1, sizeof(*entry));
  if (!entry) {
    DIST_COORD_DEBUG(state, "ds4: distributed coordinator: out of memory while "
                            "registering worker\n");
    return;
  }
  ...
  snprintf(entry->model_name, sizeof(entry->model_name), "%s",
    ...
    if (strcmp(old->peer_host, peer_host) == 0 &&
      ...
      *link = old->next;
      DIST_COORD_DEBUG(state,
                       "ds4: distributed coordinator: dropped stale worker "
                       "%s:%s layers=%u:%u%s\n",
                       old->peer_host, old->peer_port, old->layer_start,
                       old->layer_end, old->has_output ? "+output" : "");
      free(old);
      continue;
    }
    link = &old->next;
  ...
  char layer_end[32];
  if (entry->has_output)
    snprintf(layer_end, sizeof(layer_end), "output");
  else
    snprintf(layer_end, sizeof(layer_end), "%u", entry->layer_end);
  DIST_COORD_DEBUG(
      state,
      "ds4: distributed coordinator: registered worker %s:%s data_port=%u "
      "model_id=%u quant=Q%u layers=%u:%s hidden=%u ctx=%u\n",
      entry->peer_host, entry->peer_port, entry->listen_port, entry->model_id,
      entry->quant_bits, entry->layer_start, layer_end, entry->has_hidden,
      entry->ctx_size);
  if (dist_coordinator_debug_enabled(state))
    dist_coordinator_report_plan(state);
}
...
static int dist_worker_route_cmp(const void *a, const void *b) {
  ...
  if (ea->layer_start < eb->layer_start)
    return -1;
  if (ea->layer_start > eb->layer_start)
    return 1;
  if (ea->has_output != eb->has_output)
    return ea->has_output ? -1 : 1;
  if (ea->layer_end > eb->layer_end)
    return -1;
  if (ea->layer_end < eb->layer_end)
    return 1;
  return 0;
...
static bool
dist_worker_route_candidate_ok(const ds4_dist_coordinator_state *state,
                               const ds4_dist_worker_entry *w, uint32_t last) {
  const bool needs_hidden = w->layer_end < last || !w->has_output;
  if (needs_hidden && !w->has_hidden)
    return false;
  if (w->layer_end >= last && !w->has_output && !state->local_can_output_head)
    return false;
  return true;
}

static bool dist_route_search_workers(const ds4_dist_coordinator_state *state,
                                      ds4_dist_worker_entry **workers,
                                      uint32_t n, uint32_t next, uint32_t last,
  ...
  for (uint32_t i = 0; i < n; i++) {
    ...
    if (w->layer_start > next)
      break;
    saw_start = true;
    if (!dist_worker_route_candidate_ok(state, w, last))
      continue;
    ...
    if (dist_route_search_workers(state, workers, n, child_missing, last, path,
                                  path_len, &child_missing)) {
      return true;
    }
    if (child_missing > *missing_layer)
      *missing_layer = child_missing;
    (*path_len)--;
  }
  if (!saw_start && next > *missing_layer)
    *missing_layer = next;
  return false;
}
...
static void dist_coordinator_report_plan(ds4_dist_coordinator_state *state) {
  ...
  if ((n && !workers) || (n && !path)) {
    ...
    fprintf(
        stderr,
        "ds4: distributed coordinator: out of memory building route plan\n");
    return;
  }
  uint32_t i = 0;
  for (ds4_dist_worker_entry *it = state->workers; it; it = it->next)
  ...
  if (complete && !has_output) {
    complete = dist_route_search_workers(state, workers, n, next, last, path,
                                         &path_len, &missing);
  ...
  size_t used = 0;
  char local_end[32];
  if (state->local_has_output)
    snprintf(local_end, sizeof(local_end), "output");
  else
    snprintf(local_end, sizeof(local_end), "%u", state->local_end);
  used += (size_t)snprintf(plan + used,
                           used < sizeof(plan) ? sizeof(plan) - used : 0,
                           "local %u:%s", state->local_start, local_end);
  for (i = 0; i < path_len; i++) {
    ds4_dist_worker_entry *w = path[i];
    if (used < sizeof(plan)) {
      char end[32];
      if (w->has_output)
        snprintf(end, sizeof(end), "output");
      else
        snprintf(end, sizeof(end), "%u", w->layer_end);
      used += (size_t)snprintf(
          plan + used, sizeof(plan) - used, " -> %s:%u Q%u %u:%s", w->peer_host,
          w->listen_port, w->quant_bits, w->layer_start, end);
    }
  }
  if (complete && path_len != 0 && !path[path_len - 1u]->has_output &&
      state->local_can_output_head && used < sizeof(plan)) {
    used +=
        (size_t)snprintf(plan + used, sizeof(plan) - used, " -> local output");
  }
  if (complete && path_len == 0 && state->local_end == last &&
      !state->local_has_output && state->local_can_output_head &&
      used < sizeof(plan)) {
    used +=
        (size_t)snprintf(plan + used, sizeof(plan) - used, " -> local output");
  ...
  if (complete) {
    ...
    fprintf(stderr,
            "ds4: distributed coordinator: route incomplete; next needed layer "
            "%u\n",
            missing);
  }
  free(path);
...
static void dist_route_plan_free(ds4_dist_route_plan *plan) {
  ...
  for (uint32_t i = 0; i < plan->count; i++) {
    if (plan->entry[i].fd >= 0)
      close(plan->entry[i].fd);
  }
  free(plan->entry);
  free(plan->blob);
...
static bool
dist_route_entry_matches_worker(const ds4_dist_route_entry *route,
                                const ds4_dist_worker_entry *worker) {
  const bool route_has_output =
      (route->flags & DS4_DIST_ROUTE_F_OUTPUT_LOGITS) != 0;
...
static void
dist_coordinator_forget_route_workers(ds4_dist_coordinator_state *state,
                                      const ds4_dist_route_plan *plan) {
  ...
  for (uint32_t i = 0; i < plan->count; i++) {
    ...
    while (*link) {
      ...
      *link = entry->next;
      close(entry->fd);
      DIST_COORD_DEBUG(state,
                       "ds4: distributed coordinator: forgot failed route "
                       "worker %s:%u layers=%u:%u%s\n",
                       plan->entry[i].host, plan->entry[i].port,
                       entry->layer_start, entry->layer_end,
...
static bool dist_route_plan_append_blob(ds4_dist_route_plan *plan,
  ...
  if (!new_blob) {
    if (errlen)
      snprintf(err, errlen, "out of memory building route payload");
    return false;
  }
  plan->blob = new_blob;
  uint8_t *p = (uint8_t *)plan->blob + old_bytes;
  ds4_dist_route_fixed fixed = {
      (uint32_t)host_len, entry->port,  entry->layer_start,
      entry->layer_end,   entry->flags,
  };
...
static bool dist_route_plan_append_return_upstream(ds4_dist_route_plan *plan,
                                                   char *err, size_t errlen) {
...
static bool dist_coordinator_build_route_plan(ds4_dist_coordinator_state *state,
                                              ds4_dist_route_plan *plan,
                                              uint64_t *generation, char *err,
                                              size_t errlen) {
  memset(plan, 0, sizeof(*plan));
  if (generation)
    *generation = 0;
  ...
  if ((n && !workers) || (n && !path)) {
    ...
    pthread_mutex_unlock(&state->mu);
    if (errlen)
      snprintf(err, errlen, "out of memory building route");
    return false;
  }
  uint32_t i = 0;
  for (ds4_dist_worker_entry *it = state->workers; it; it = it->next)
  ...
  if (state->local_start != 0) {
    ...
    free(path);
    if (errlen)
      snprintf(err, errlen, "coordinator route does not start at layer 0");
    return false;
  }
  if (state->local_end == last &&
      (state->local_has_output || state->local_can_output_head)) {
  ...
  uint32_t path_len = 0;
  uint32_t missing = next;
  if (!dist_route_search_workers(state, workers, n, next, last, path, &path_len,
                                 &missing)) {
    pthread_mutex_unlock(&state->mu);
    free(workers);
    free(path);
    if (errlen)
      snprintf(err, errlen, "distributed route incomplete: missing layer %u",
               missing);
    return false;
  }
  ...
  for (i = 0; i < path_len; i++) {
    ...
    if (state->use_control_for_work && plan->count == 0) {
      ...
      if (entry.fd < 0) {
        ...
        if (errlen)
          snprintf(err, errlen,
                   "failed to duplicate first-hop worker connection: %s",
                   strerror(errno));
        return false;
      }
    ...
    ds4_dist_route_entry *new_entries = realloc(
        plan->entry, (size_t)(plan->count + 1u) * sizeof(plan->entry[0]));
    if (!new_entries) {
      pthread_mutex_unlock(&state->mu);
      free(workers);
      free(path);
      ...
      dist_route_plan_free(plan);
      if (errlen)
        snprintf(err, errlen, "out of memory building route entries");
      return false;
    }
  ...
  free(path);
  if (plan->count != 0 &&
      !dist_route_plan_append_return_upstream(plan, err, errlen)) {
    dist_route_plan_free(plan);
    return false;
  }
  return true;
...
static bool dist_coordinator_ensure_route(ds4_dist_coordinator_state *state,
                                          ds4_dist_route_plan *plan,
                                          uint64_t *generation, char *err,
                                          size_t errlen) {
  return dist_coordinator_build_route_plan(state, plan, generation, err,
                                           errlen);
}
...
static int dist_recv_result_alloc(int fd,
                                  const ds4_dist_coordinator_state *state,
                                  uint64_t request_id, uint32_t *kind,
                                  uint64_t *result_hash, void **payload,
                                  uint32_t *payload_bytes, char *err,
                                  size_t errlen) {
  *payload = NULL;
  *payload_bytes = 0;
  *kind = 0;
  ...
  if (rc <= 0) {
    if (rc == 0 && errlen)
      snprintf(err, errlen, "distributed worker closed connection");
    return 1;
  }
  if (type != DS4_DIST_MSG_RESULT || bytes < sizeof(ds4_dist_result_fixed)) {
    dist_discard_bytes(fd, bytes);
    if (errlen)
      snprintf(err, errlen, "distributed worker returned invalid frame");
    return 1;
  }
  ...
  if (rc <= 0) {
    if (errlen)
      snprintf(err, errlen, "failed to read distributed result");
    return 1;
  }
  dist_result_from_wire(&result);
  const uint64_t got_request =
      dist_u64_from_halves(result.request_hi, result.request_lo);
  const uint64_t got_hash =
      dist_u64_from_halves(result.result_hash_hi, result.result_hash_lo);
  const uint32_t body_bytes = bytes - (uint32_t)sizeof(result);
  if (result.telemetry_bytes % (uint32_t)sizeof(ds4_dist_telemetry_fixed) !=
          0 ||
      result.telemetry_count !=
          result.telemetry_bytes / (uint32_t)sizeof(ds4_dist_telemetry_fixed) ||
      result.telemetry_bytes > body_bytes ||
      result.payload_bytes != body_bytes - result.telemetry_bytes) {
    dist_discard_bytes(fd, body_bytes);
    if (errlen)
      snprintf(err, errlen, "distributed result telemetry metadata mismatch");
    return 1;
  }
  if (got_request != request_id) {
    dist_discard_bytes(fd, bytes - (uint32_t)sizeof(result));
    if (errlen)
      snprintf(err, errlen, "distributed result metadata mismatch");
    return 1;
  }
  ...
  if (result.telemetry_bytes != 0) {
    if (dist_coordinator_debug_enabled(state)) {
      ...
      if (!telemetry) {
        ...
        if (errlen)
          snprintf(err, errlen, "out of memory reading distributed telemetry");
        return 1;
      }
      ...
      if (rc <= 0) {
        free(telemetry);
        if (errlen)
          snprintf(err, errlen, "failed to read distributed result telemetry");
        return 1;
      }
      for (uint32_t i = 0; i < result.telemetry_count; i++) {
        ...
        DIST_COORD_DEBUG(
            state,
            "ds4: distributed telemetry: request=%llu hop=%u layers=%u:%u "
            "route=%u pos=%u tokens=%u eval=%.3fms downstream_wait=%.3fms "
            "forward_send=%.3fms input=%.2fMiB output=%.2fMiB\n",
            (unsigned long long)got_request, i, telemetry[i].layer_start,
            telemetry[i].layer_end, telemetry[i].route_index, telemetry[i].pos0,
            telemetry[i].n_tokens, (double)telemetry[i].eval_usec / 1000.0,
  ...
  if (result.payload_bytes != 0) {
    ...
    if (!buf) {
      ...
      if (errlen)
        snprintf(err, errlen, "out of memory reading distributed result");
      return 1;
    }
    ...
    if (rc <= 0) {
      free(buf);
      if (errlen)
        snprintf(err, errlen, "failed to read distributed result payload");
      return 1;
    }
  }
  ...
  if (result.status != 0) {
    if (errlen) {
      if (buf && result.payload_bytes) {
        size_t n = result.payload_bytes < errlen - 1 ? result.payload_bytes
                                                     : errlen - 1;
        memcpy(err, buf, n);
        err[n] = '\0';
      } else {
  ...
  if (result.result_kind == DS4_DIST_RESULT_HIDDEN_STATE &&
      result.payload_bytes != 0) {
    float *decoded = NULL;
    uint32_t decoded_bytes = 0;
    bool uses_wire = false;
    if (dist_decode_activation_payload(
            buf, result.payload_bits, result.payload_bytes, &decoded,
            &decoded_bytes, &uses_wire, err, errlen) != 0) {
      free(buf);
      return 1;
    }
    if (!uses_wire) {
...
static int dist_coordinator_send_remote_work_on_fd(
    ds4_dist_coordinator_state *state, const ds4_dist_route_plan *plan, int fd,
    const int *tokens, uint32_t n_tokens, uint32_t pos0, uint64_t session_id,
    uint64_t request_id, uint64_t prefix_hash, uint64_t result_hash,
    bool reset_session, bool ack_only, const float *hidden_hc,
    uint32_t hidden_hc_bytes, char *err, size_t errlen) {
  if (plan->count == 0) {
    if (errlen)
      snprintf(err, errlen, "distributed route has no remote worker");
    return 1;
  }
  ...
  if (reset_session)
    work.flags |= DS4_DIST_WORK_F_RESET_SESSION;
  if (ack_only)
    work.flags |= DS4_DIST_WORK_F_ACK_ONLY;
  ...
  if (!dist_activation_wire_bytes_from_f32_bytes(
    ...
    if (errlen)
      snprintf(err, errlen, "invalid distributed hidden-state size");
    return 1;
  }
  ...
  if (dist_send_work_frame(fd, &work, tokens, hidden_hc, plan->blob) != 0) {
    if (errlen)
      snprintf(err, errlen, "failed to send distributed work");
    return 1;
  }
  return 0;
}
...
static int dist_coordinator_eval_remote_on_fd(
    ...
    const ds4_dist_route_plan *plan, int fd, const int *tokens,
    uint32_t n_tokens, uint32_t pos0, uint64_t session_id, uint64_t request_id,
    uint64_t prefix_hash, uint64_t expected_result_hash, bool reset_session,
    const float *hidden_hc, uint32_t hidden_hc_bytes, float *logits, char *err,
    size_t errlen) {
  ...
  int rc = dist_coordinator_send_remote_work_on_fd(
      state, plan, fd, tokens, n_tokens, pos0, session_id, request_id,
      prefix_hash, expected_result_hash, reset_session, false, hidden_hc,
      hidden_hc_bytes, err, errlen);
  ...
  if (rc == 0) {
    ...
    rc = dist_recv_result_alloc(fd, state, request_id, &kind, &result_hash,
                                &payload, &payload_bytes, err, errlen);
    const double recv_t1 = profile ? dist_now_sec() : 0.0;
    if (profile) {
      fprintf(stderr,
              "ds4: dist decode profile: remote request=%llu pos=%u "
              "send=%.3fms wait_result=%.3fms kind=%u payload=%.2fMiB\n",
              (unsigned long long)request_id, pos0,
              (send_t1 - send_t0) * 1000.0, (recv_t1 - recv_t0) * 1000.0, kind,
  ...
  if (result_hash != expected_result_hash) {
    free(payload);
    if (errlen)
      snprintf(err, errlen, "distributed result prefix hash mismatch");
    return 1;
  }

  const uint32_t logits_bytes =
      (uint32_t)((uint64_t)ds4_engine_vocab_size(state->engine) *
                 sizeof(float));
  if (kind == DS4_DIST_RESULT_LOGITS && payload_bytes == logits_bytes) {
    ...
    if (profile) {
      ...
      fprintf(stderr,
              "ds4: dist decode profile: remote request=%llu "
              "copy_logits=%.3fms total=%.3fms\n",
              (unsigned long long)request_id, (copy_t1 - copy_t0) * 1000.0,
              (copy_t1 - total_t0) * 1000.0);
    }
    return 0;
  }
  if (kind == DS4_DIST_RESULT_HIDDEN_STATE &&
      payload_bytes == hidden_hc_bytes) {
    const double head_t0 = profile ? dist_now_sec() : 0.0;
    int head_rc = ds4_session_eval_output_head_from_hc(
        session, payload, n_tokens, logits, err, errlen);
    free(payload);
    if (profile) {
      const double head_t1 = dist_now_sec();
      fprintf(stderr,
              "ds4: dist decode profile: remote request=%llu "
              "output_head=%.3fms total=%.3fms rc=%d\n",
              (unsigned long long)request_id, (head_t1 - head_t0) * 1000.0,
              (head_t1 - total_t0) * 1000.0, head_rc);
    }
    return head_rc;
  }
  if (kind == DS4_DIST_RESULT_HIDDEN_STATE) {
    ...
    if (errlen)
      snprintf(err, errlen,
               "distributed route returned invalid hidden-state size");
    return 1;
  }
  free(payload);
  if (errlen)
    snprintf(err, errlen,
             "distributed route did not return logits or hidden-state");
  return 1;
}

static int dist_coordinator_eval_span(ds4_dist_coordinator_state *state,
                                      ds4_session *session,
                                      const ds4_dist_route_plan *plan,
                                      const int *tokens, uint32_t n_tokens,
                                      uint32_t pos0, uint64_t session_id,
                                      uint64_t request_id, bool reset_session,
  ...
  if (hidden_bytes64 > UINT32_MAX) {
    if (errlen)
      snprintf(err, errlen,
               "distributed coordinator hidden-state chunk is too large");
    return 1;
  }
  ...
  if (reset_session) {
    if (pos0 != 0) {
      if (errlen)
        snprintf(err, errlen,
                 "distributed reset span must start at position 0");
      return 1;
    }
  } else if (dist_session_token_hash_prefix(session, pos0, &prefix_hash, err,
                                            errlen) != 0) {
    return 1;
  }
  ...
  if (plan->count != 0) {
    ...
    if (!hidden) {
      if (errlen)
        snprintf(err, errlen,
                 "out of memory allocating coordinator hidden-state");
      return 1;
    }
  }
  if (reset_session &&
  ...
  if (plan->count != 0) {
    ...
    if (remote_fd < 0) {
      if (errlen)
        snprintf(err, errlen,
                 "distributed route has no live first-hop connection");
      free(hidden);
      return 1;
    }
  }
  ...
  int rc = ds4_session_eval_layer_slice(
      session, tokens, n_tokens, pos0, state->local_start, state->local_end,
      NULL, local_logits ? NULL : hidden, local_logits,
      local_logits ? logits : NULL, err, errlen);
  ...
  if (rc == 0 && plan->count != 0) {
    ...
    rc = dist_coordinator_eval_remote_on_fd(
        state, session, plan, remote_fd, tokens, n_tokens, pos0, session_id,
        request_id, prefix_hash, result_hash, reset_session, hidden,
        hidden_bytes, logits, err, errlen);
    remote_t1 = profile ? dist_now_sec() : 0.0;
  }
  if (profile) {
    const double span_t1 = dist_now_sec();
    fprintf(stderr,
            "ds4: dist decode profile: span request=%llu pos=%u layers=%u:%u "
            "local=%.3fms remote=%.3fms total=%.3fms hidden=%.2fMiB rc=%d\n",
            (unsigned long long)request_id, pos0, state->local_start,
            state->local_end, (local_t1 - local_t0) * 1000.0,
            (remote_t1 - remote_t0) * 1000.0, (span_t1 - span_t0) * 1000.0,
            (double)hidden_bytes / (1024.0 * 1024.0), rc);
  }
  free(hidden);
  return rc;
}
...
static bool dist_json_utf8_valid(const char *s, size_t n) {
  for (size_t i = 0; i < n;) {
    ...
    if (need == 2 && c < 0xc2)
      return false;
    if (need == 3 && c == 0xe0 && c1 < 0xa0)
      return false;
    if (need == 3 && c == 0xed && c1 >= 0xa0)
      return false;
    if (need == 4 && c == 0xf0 && c1 < 0x90)
      return false;
    if (need == 4 && c == 0xf4 && c1 >= 0x90)
      return false;
    for (int j = 2; j < need; j++) {
      if ((((unsigned char)s[i + (size_t)j]) & 0xc0) != 0x80)
        return false;
    }
    i += (size_t)need;
  }
...
static int dist_write_logits_dump(ds4_dist_coordinator_state *state,
  ...
  fprintf(fp,
          ...
          "  \"route_count\":%u,\n"
          "  \"argmax_token\":",
          ds4_engine_routed_quant_bits(state->engine), prompt->len,
          gen->ctx_size, vocab, plan->count);
...
static int dist_logits_top_logprobs(const float *logits, int vocab,
                                    ds4_dist_logprob *scores, int k) {
  if (k <= 0)
    return 0;
  for (int i = 0; i < k; i++) {
    scores[i].id = -1;
  ...
  for (int i = 0; i < vocab; i++) {
    if (isfinite(logits[i]) && logits[i] > max_logit)
      max_logit = logits[i];
  }
...
static int dist_coordinator_rebuild_from_transcript(
    ds4_dist_coordinator_state *state, ds4_session *session,
    ds4_dist_route_plan *plan, const ds4_tokens *transcript,
    uint64_t session_id, uint64_t *request_id, float *logits,
    uint64_t *plan_generation, bool forget_route, char *err, size_t errlen) {
  DIST_COORD_DEBUG(state,
                   "ds4: distributed coordinator: replaying %d tokens after "
                   "distributed %s\n",
                   transcript->len,
  ...
  if (forget_route) {
    ...
    if (!dist_coordinator_ensure_route(state, plan, &generation, err, errlen))
      return 1;
    if (plan_generation)
  ...
  if (dist_coordinator_prefill_prompt(state, session, plan, transcript,
                                      session_id, request_id, logits, err,
                                      errlen) != 0) {
    return 1;
  }
  return 0;
...
static int dist_write_logprobs_dump(ds4_dist_coordinator_state *state,
                                    const ds4_dist_generation_options *gen,
                                    const ds4_tokens *prompt,
                                    ds4_dist_route_plan *plan,
                                    ds4_session *session, uint64_t session_id,
                                    uint64_t *request_id, float *logits) {
  FILE *fp = fopen(gen->dump_logprobs_path, "wb");
  if (!fp) {
    fprintf(stderr,
            "ds4: failed to open distributed --dump-logprobs file: %s\n",
  ...
  fprintf(fp,
          ...
          "  \"route_count\":%u,\n"
          "  \"steps\":[\n",
          prompt->len, gen->ctx_size, k, plan->count);
  ...
  for (int generated = 0; generated < max_tokens; generated++) {
    const int n = dist_logits_top_logprobs(
        logits, ds4_engine_vocab_size(state->engine), scores, k);
    const int token =
        dist_logits_argmax(logits, ds4_engine_vocab_size(state->engine));
    if (generated)
      fputs(",\n", fp);
    ...
    for (int i = 0; i < n; i++) {
      ...
      fprintf(fp, ",\"logit\":%.9g,\"logprob\":%.9g}", scores[i].logit,
              scores[i].logprob);
    }
    fputs("]}", fp);
    ...
    if (dist_coordinator_eval_span(state, session, plan, &token, 1, token_pos,
                                   session_id, (*request_id)++, false, logits,
                                   err, sizeof(err)) != 0) {
      fprintf(stderr,
              "ds4: distributed decode failed while dumping logprobs: %s\n",
              err);
      if (dist_coordinator_rebuild_from_transcript(
              state, session, plan, &transcript, session_id, request_id, logits,
              NULL, true, err, sizeof(err)) != 0) {
        fprintf(stderr,
  ...
  if (fclose(fp) != 0) {
    fprintf(stderr,
            "ds4: failed to close distributed --dump-logprobs file: %s\n",
            gen->dump_logprobs_path);
    rc = 1;
  }
...
static int dist_prefill_sender_init(
    ds4_dist_prefill_sender *sender, ds4_dist_coordinator_state *state,
    const ds4_dist_route_plan *plan, const ds4_tokens *prompt,
    uint64_t session_id, int fd, uint32_t chunk_count,
    uint32_t max_hidden_bytes, char *err, size_t errlen) {
  ...
  if (!sender->slots) {
    if (errlen)
      snprintf(err, errlen, "out of memory allocating prefill sender slots");
    return 1;
  }
  for (uint32_t i = 0; i < sender->slot_count; i++) {
    ...
    if (!sender->slots[i].hidden) {
      if (errlen)
        snprintf(
            err, errlen,
            "out of memory allocating prefill sender hidden-state buffers");
      return 1;
    }
  }
  return 0;
...
static ds4_dist_prefill_send_slot *
dist_prefill_sender_acquire_slot(ds4_dist_prefill_sender *sender, char *err,
                                 size_t errlen) {
  ...
  if (sender->stop || sender->rc != 0) {
    if (errlen)
      snprintf(err, errlen, "%s",
               sender->err[0] ? sender->err
                              : "distributed prefill sender stopped");
...
static int dist_prefill_sender_enqueue_slot(ds4_dist_prefill_sender *sender,
                                            char *err, size_t errlen) {
  pthread_mutex_lock(&sender->mu);
  if (sender->stop || sender->rc != 0) {
    if (errlen)
...
static void *dist_prefill_sender_main(void *arg) {
  ...
  for (;;) {
    ...
    int rc = dist_coordinator_send_remote_work_on_fd(
        sender->state, sender->plan, sender->fd, sender->prompt->v + slot->pos,
        slot->n_tokens, slot->pos, sender->session_id, slot->request_id,
        slot->prefix_hash, slot->result_hash, slot->reset_session,
        slot->ack_only, slot->hidden, slot->hidden_bytes, send_err,
        sizeof(send_err));
    const double send_t1 = dist_now_sec();

    pthread_mutex_lock(&sender->mu);
    uint32_t slot_hidden_wire_bytes = slot->hidden_bytes;
    (void)dist_activation_wire_bytes_from_f32_bytes(
        sender->state->activation_bits, slot->hidden_bytes,
...
static void
dist_prefill_reader_signal_progress(ds4_dist_prefill_result_reader *reader,
                                    uint32_t completed, bool done) {
  pthread_mutex_lock(&reader->progress_mu);
  if (completed > reader->progress_completed)
    reader->progress_completed = completed;
  if (done)
    reader->progress_done = true;
  pthread_cond_broadcast(&reader->progress_cv);
  pthread_mutex_unlock(&reader->progress_mu);
}

static void
dist_prefill_reader_emit_progress(ds4_dist_prefill_result_reader *reader,
                                  uint32_t *reported) {
  if (!reader || !reported || !reader->progress_session)
    return;
  ...
  while (*reported < completed) {
    ...
    if (rel > reader->total_tokens)
      rel = reader->total_tokens;
    uint32_t current = reader->progress_base + rel;
    if (current > reader->progress_total)
      current = reader->progress_total;
    ds4_session_report_progress(reader->progress_session, "prefill_chunk",
                                (int)current, (int)reader->progress_total);
  }
}

static bool
dist_prefill_reader_wait_emit_progress(ds4_dist_prefill_result_reader *reader,
                                       uint32_t *reported) {
  if (!reader || !reported)
    return true;
...
static bool
dist_prefill_reader_wait_flow_window(ds4_dist_prefill_result_reader *reader,
                                     uint32_t submitted, uint32_t window,
                                     uint32_t *reported) {
  if (!reader || window == 0)
    return true;
...
static void *dist_prefill_result_reader_main(void *arg) {
  ...
  const uint32_t logits_bytes =
      (uint32_t)((uint64_t)ds4_engine_vocab_size(reader->state->engine) *
                 sizeof(float));
  for (uint32_t i = 0; i < reader->count; i++) {
    ...
    uint64_t result_hash = 0;
    void *payload = NULL;
    int recv_rc = dist_recv_result_alloc(
        reader->fd, reader->state, request_id, &kind, &result_hash, &payload,
        &payload_bytes, reader->err, sizeof(reader->err));
    if (recv_rc != 0) {
      reader->rc = recv_rc;
      free(payload);
    ...
    const uint32_t chunk =
        remaining < reader->chunk_cap ? remaining : reader->chunk_cap;
    const uint64_t hidden_bytes64 =
        (uint64_t)chunk * reader->hc_values * sizeof(float);
    const bool final_chunk = i + 1u == reader->count;
    const bool valid_ack =
        !final_chunk && kind == DS4_DIST_RESULT_ACK && payload_bytes == 0;
    const bool valid_logits =
        kind == DS4_DIST_RESULT_LOGITS && payload_bytes == logits_bytes;
...
static bool dist_coordinator_can_pipeline_prefill(
    const ds4_dist_coordinator_state *state, const ds4_dist_route_plan *plan,
    ds4_session *session, uint32_t n_tokens, uint32_t chunk_cap) {
  if (getenv("DS4_DIST_DISABLE_PREFILL_PIPELINE"))
    return false;
  if (!state || !plan)
    return false;
  (void)session;
  if (chunk_cap == 0 || n_tokens <= chunk_cap)
    return false;
  if (plan->count == 0)
    return false;
  if (plan->entry[0].fd < 0)
    return false;
...
static int
dist_coordinator_prefill_chunk_cap(const ds4_dist_coordinator_state *state,
                                   ds4_session *session, uint32_t *chunk_cap,
                                   char *err, size_t errlen) {
  if (!chunk_cap)
    return 1;
  ...
  if (prefill_cap_i <= 0) {
    if (errlen)
      snprintf(err, errlen, "distributed coordinator has no prefill capacity");
    return 1;
  }
  ...
  if (requested == 0 && env && env[0]) {
    if (!dist_parse_positive_u32(env, "DS4_DIST_PREFILL_CHUNK", &requested, err,
                                 errlen)) {
      return 1;
    }
  }
...
static int dist_coordinator_prefill_window(
    const ds4_dist_coordinator_state *state, const ds4_dist_route_plan *plan,
    uint32_t chunk_count, uint32_t *window, char *err, size_t errlen) {
  if (!window)
    return 1;
  uint32_t requested = state ? state->prefill_window : 0u;
  const char *env = getenv("DS4_DIST_PREFILL_WINDOW");
  if (requested == 0 && env && env[0]) {
    if (!dist_parse_positive_u32(env, "DS4_DIST_PREFILL_WINDOW", &requested,
                                 err, errlen)) {
      return 1;
    }
  }
  if (requested > 64u) {
    if (errlen)
      snprintf(err, errlen, "distributed prefill window %u exceeds limit 64",
               requested);
    return 1;
  }
  if (requested == 0) {
  ...
  if (chunk_count != 0 && requested > chunk_count)
    requested = chunk_count;
  *window = requested ? requested : 1u;
  return 0;
}

static void dist_report_prefill_progress(ds4_session *session, uint32_t current,
                                         uint32_t total) {
  if (!session)
    return;
  if (current > (uint32_t)INT_MAX)
    current = (uint32_t)INT_MAX;
  if (total > (uint32_t)INT_MAX)
    total = (uint32_t)INT_MAX;
  ds4_session_report_progress(session, "prefill_chunk", (int)current,
                              (int)total);
}
...
static int dist_coordinator_prefill_prompt_pipelined(
    ds4_dist_coordinator_state *state, ds4_session *session,
    const ds4_dist_route_plan *plan, const ds4_tokens *prompt,
    uint32_t span_start, uint32_t n_tokens, bool reset_first_chunk,
    uint32_t chunk_cap, uint64_t session_id, uint64_t *request_id,
    float *logits, char *err, size_t errlen) {
  const uint32_t total = n_tokens;
  if (!prompt || span_start > (uint32_t)prompt->len || n_tokens == 0 ||
      n_tokens > (uint32_t)prompt->len - span_start) {
    if (errlen)
      snprintf(err, errlen, "invalid distributed pipelined prefill span");
    return 1;
  }
  const uint32_t span_end = span_start + n_tokens;
  const uint32_t chunk_count = (total + chunk_cap - 1u) / chunk_cap;
  const uint64_t hc_values = ds4_engine_hidden_f32_values(state->engine);
  const uint64_t max_hidden_bytes64 =
      (uint64_t)chunk_cap * hc_values * sizeof(float);
  ...
  if (dist_coordinator_prefill_window(state, plan, chunk_count, &flow_window,
                                      err, errlen) != 0) {
    return 1;
  }

  ds4_dist_prefill_sender sender;
  if (dist_prefill_sender_init(&sender, state, plan, prompt, session_id,
                               plan->entry[0].fd, chunk_count, max_hidden_bytes,
                               err, errlen) != 0) {
  ...
  reader.allow_hidden = (plan->entry[plan->count - 1u].flags &
                         DS4_DIST_ROUTE_F_OUTPUT_LOGITS) == 0;
  pthread_mutex_init(&reader.progress_mu, NULL);
  pthread_cond_init(&reader.progress_cv, NULL);
  reader.expected_hashes =
      calloc(chunk_count, sizeof(reader.expected_hashes[0]));
  if (!reader.expected_hashes) {
    ...
    if (errlen)
      snprintf(err, errlen,
               "out of memory allocating distributed prefill hashes");
    return 1;
  }
  ...
  for (uint32_t i = 0, hash_pos = span_start; i < chunk_count; i++) {
    ...
    chunk_prefix_hash = dist_token_hash_update_span(
        chunk_prefix_hash, prompt->v + hash_pos, chunk);
    reader.expected_hashes[i] = chunk_prefix_hash;
    hash_pos += chunk;
  }

  pthread_t reader_tid;
  if (pthread_create(&reader_tid, NULL, dist_prefill_result_reader_main,
                     &reader) != 0) {
    ...
    if (errlen)
      snprintf(err, errlen,
               "failed to start distributed prefill result reader");
    return 1;
  }
  pthread_t sender_tid;
  if (pthread_create(&sender_tid, NULL, dist_prefill_sender_main, &sender) !=
      0) {
    ...
    dist_prefill_sender_destroy(&sender);
    if (errlen)
      snprintf(err, errlen, "failed to start distributed prefill sender");
    return 1;
  }

  DIST_COORD_DEBUG(state,
                   "ds4: distributed coordinator: pipelined prefill %u chunks "
                   "of up to %u tokens through %u worker%s, first hop %s:%u, "
                   "send depth %u, flow window %u\n",
                   chunk_count, chunk_cap, plan->count,
                   plan->count == 1u ? "" : "s", plan->entry[0].host,
                   plan->entry[0].port, sender.slot_count, flow_window);
  ...
  while (pos < span_end) {
    if (!dist_prefill_reader_wait_flow_window(&reader, submitted_chunks,
                                              flow_window, &reported_chunks)) {
      if (errlen)
        snprintf(err, errlen, "distributed prefill result reader stopped");
      rc = 1;
      break;
    }
    ...
    if (pos == span_start && reset_first_chunk &&
        ds4_session_layer_slice_reset(session, err, errlen) != 0) {
      rc = 1;
      break;
    }
    ...
    rc = ds4_session_eval_layer_slice(
        session, prompt->v + pos, chunk, pos, state->local_start,
        state->local_end, NULL, slot->hidden, false, NULL, err, errlen);
    ...
    slot->ack_only =
        !getenv("DS4_DIST_DISABLE_PREFILL_ACK_ONLY") && pos + chunk < span_end;
  ...
  if (rc == 0)
    dist_prefill_sender_finish(&sender);
  else
    dist_prefill_sender_cancel(&sender);
  ...
  if (rc == 0 && sender.rc != 0) {
    if (errlen)
      snprintf(err, errlen, "%s",
               sender.err[0] ? sender.err
                             : "distributed prefill sender failed");
    rc = 1;
  }
  if (rc != 0) {
  ...
  if (rc == 0 && reader.rc == 0) {
    ...
    DIST_COORD_DEBUG(
        state,
        "ds4: distributed coordinator: pipelined prefill done tokens=%u "
        "chunks=%u total=%.3fs %.2f t/s local=%.3fs send=%.3fs %.2f MiB/s\n",
        total, chunk_count, total_sec,
        total_sec > 0.0 ? (double)total / total_sec : 0.0, local_eval_sec,
        sender.send_sec,
        sender.send_sec > 0.0
  ...
  if (reader.rc != 0) {
    if (errlen)
      snprintf(err, errlen, "%s",
               reader.err[0] ? reader.err
                             : "distributed pipelined prefill failed");
  ...
  if (reader.final_kind == DS4_DIST_RESULT_HIDDEN_STATE &&
    ...
    int head_rc = ds4_session_eval_output_head_from_hc(
        session, reader.final_payload, last_tokens, logits, err, errlen);
    free(reader.final_payload);
    return head_rc;
  }
  free(reader.final_payload);
  if (errlen)
    snprintf(err, errlen,
             "distributed pipelined prefill did not return a final result");
  return 1;
}
...
static int dist_coordinator_prefill_prompt(ds4_dist_coordinator_state *state,
  ...
  if (dist_coordinator_prefill_chunk_cap(state, session, &chunk_cap, err,
                                         errlen) != 0) {
    return 1;
  }
  const uint32_t prompt_len = (uint32_t)prompt->len;
  if (dist_coordinator_can_pipeline_prefill(state, plan, session, prompt_len,
                                            chunk_cap)) {
    return dist_coordinator_prefill_prompt_pipelined(
        state, session, plan, prompt, 0, prompt_len, true, chunk_cap,
        session_id, request_id, logits, err, errlen);
  }
  ...
  while (pos < prompt_len) {
    ...
    int eval_rc = dist_coordinator_eval_span(
        state, session, plan, prompt->v + pos, chunk, pos, session_id,
        (*request_id)++, pos == 0, logits, err, errlen);
    if (eval_rc != 0) {
      return eval_rc;
    }
    pos += chunk;
...
static int dist_replay_check_logits(ds4_dist_coordinator_state *state,
  ...
  if (mismatches != 0) {
    fprintf(stderr,
            "ds4: distributed replay check failed: mismatches=%u max_abs=%g "
            "token=%d before=%g after=%g\n",
            mismatches, max_abs, max_i, before[max_i], after[max_i]);
    return 1;
  }
  fprintf(stderr, "ds4: distributed replay check passed: logits exact match "
                  "across reset/replay\n");
  return 0;
}

static int
dist_run_coordinator_generation(ds4_dist_coordinator_state *state,
                                const ds4_dist_generation_options *gen) {
  char err[256];
  ds4_dist_route_plan plan;
  uint64_t plan_generation = 0;
  if (!dist_coordinator_ensure_route(state, &plan, &plan_generation, err,
                                     sizeof(err))) {
  ...
  if (ds4_session_create(&session, state->engine, gen->ctx_size) != 0) {
    fprintf(stderr,
            "ds4: distributed coordinator: failed to create local session\n");
    dist_route_plan_free(&plan);
    return 1;
  }
  ...
  if (dist_prompt_is_rendered_chat(gen->prompt)) {
    ...
    ds4_encode_chat_prompt(state->engine, gen->system, gen->prompt,
  ...
  const uint64_t session_id =
      ((uint64_t)(uint32_t)time(NULL) << 32) ^ (uint64_t)getpid();
  uint64_t request_id = 1;
  float *logits =
      malloc((size_t)ds4_engine_vocab_size(state->engine) * sizeof(float));
  if (!logits) {
    fprintf(stderr,
            "ds4: distributed coordinator: out of memory allocating logits\n");
  ...
  int prefill_rc = dist_coordinator_prefill_prompt(
      state, session, &plan, &prompt, session_id, &request_id, logits, err,
      sizeof(err));
  if (prefill_rc != 0) {
    fprintf(stderr, "ds4: distributed prompt processing failed: %s\n", err);
    if (dist_coordinator_rebuild_from_transcript(
            state, session, &plan, &prompt, session_id, &request_id, logits,
            NULL, prefill_rc != DS4_DIST_RECV_REMOTE_ERROR, err,
            sizeof(err)) != 0) {
      fprintf(stderr, "ds4: distributed prompt recovery failed: %s\n", err);
      free(logits);
      ds4_tokens_free(&prompt);
      ds4_session_free(session);
      dist_route_plan_free(&plan);
  ...
  if (state->replay_check) {
    const size_t logits_bytes =
        (size_t)ds4_engine_vocab_size(state->engine) * sizeof(logits[0]);
    float *before = malloc(logits_bytes);
    if (!before) {
      fprintf(stderr, "ds4: distributed replay check: out of memory allocating "
                      "logits copy\n");
      free(logits);
      ds4_tokens_free(&prompt);
      ds4_session_free(session);
    ...
    int replay_prefill_rc = dist_coordinator_prefill_prompt(
        state, session, &plan, &prompt, session_id, &request_id, logits, err,
        sizeof(err));
    if (replay_prefill_rc != 0) {
      fprintf(stderr, "ds4: distributed replay prompt processing failed: %s\n",
              err);
      if (dist_coordinator_rebuild_from_transcript(
              state, session, &plan, &prompt, session_id, &request_id, logits,
              NULL, replay_prefill_rc != DS4_DIST_RECV_REMOTE_ERROR, err,
              sizeof(err)) != 0) {
        fprintf(stderr, "ds4: distributed replay recovery failed: %s\n", err);
        free(before);
        free(logits);
        ds4_tokens_free(&prompt);
        ds4_session_free(session);
  ...
  if (gen->dump_logprobs_path) {
    int rc = dist_write_logprobs_dump(state, gen, &prompt, &plan, session,
                                      session_id, &request_id, logits);
    free(logits);
    ds4_tokens_free(&prompt);
    ds4_session_free(session);
    dist_route_plan_free(&plan);
  ...
  uint64_t rng = gen->seed ? gen->seed
                           : ((uint64_t)time(NULL) ^
                              ((uint64_t)getpid() << 32) ^ (uint64_t)clock());
  const int eos = ds4_token_eos(state->engine);
  ds4_tokens transcript = {0};
  ds4_tokens_copy(&transcript, &prompt);
  while (generated < max_tokens) {
    int token =
        ds4_sample_logits(logits, ds4_engine_vocab_size(state->engine),
                          gen->temperature, 0, gen->top_p, gen->min_p, &rng);
    if (token == eos)
      break;
    ...
    int decode_rc = dist_coordinator_eval_span(
        state, session, &plan, &token, 1, token_pos, session_id, request_id++,
        false, logits, err, sizeof(err));
    if (decode_rc != 0) {
      fprintf(stderr, "\nds4: distributed decode failed: %s\n", err);
      if (dist_coordinator_rebuild_from_transcript(
              state, session, &plan, &transcript, session_id, &request_id,
              logits, NULL, decode_rc != DS4_DIST_RECV_REMOTE_ERROR, err,
              sizeof(err)) != 0) {
...
static void dist_coordinator_remove_worker(ds4_dist_coordinator_state *state,
                                           int fd) {
  pthread_mutex_lock(&state->mu);
  ds4_dist_worker_entry **link = &state->workers;
  while (*link) {
    ...
    if (entry->fd == fd) {
      *link = entry->next;
      ...
      DIST_COORD_DEBUG(
          ...
          entry->layer_end, entry->has_output ? "+output" : "");
      pthread_mutex_unlock(&state->mu);
      free(entry);
      if (dist_coordinator_debug_enabled(state))
        dist_coordinator_report_plan(state);
...
static void
dist_coordinator_monitor_worker_fd(ds4_dist_coordinator_state *state, int fd,
  ...
  for (;;) {
    ...
    if (rc < 0) {
      if (errno == EINTR)
        continue;
      DIST_COORD_DEBUG(
          state, "ds4: distributed coordinator: worker %s:%s poll failed: %s\n",
          peer_host, peer_port, strerror(errno));
      break;
    }
    if (rc == 0)
      continue;
...
static void *dist_coordinator_client_main(void *arg) {
  ...
  char model_name[DS4_DIST_MAX_MODEL_NAME + 1u];
  char err[256];
  int rc = dist_recv_hello(fd, &hello, model_name, sizeof(model_name), err,
                           sizeof(err));
  if (rc <= 0) {
    if (rc < 0)
      DIST_COORD_DEBUG(
          state, "ds4: distributed coordinator: bad HELLO from %s:%s: %s\n",
  ...
  if (hello.model_id != state->model_id) {
    snprintf(err, sizeof(err), "model id mismatch: worker=%u coordinator=%u",
             hello.model_id, state->model_id);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
  ...
  if (!expected_model_name)
    expected_model_name = "unknown";
  if (strcmp(model_name, expected_model_name) != 0) {
    snprintf(err, sizeof(err),
             "model family mismatch: worker=%s coordinator=%s", model_name,
             expected_model_name);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
  ...
  if (hello.n_layers != state->n_layers) {
    snprintf(err, sizeof(err), "layer count mismatch: worker=%u coordinator=%u",
             hello.n_layers, state->n_layers);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
    dist_send_error(fd, err);
  ...
  if (hello.quant_bits != 2u && hello.quant_bits != 4u) {
    snprintf(err, sizeof(err), "unsupported worker quant profile Q%u",
             hello.quant_bits);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
  ...
  if (hello.has_output > 1u) {
    snprintf(err, sizeof(err), "invalid worker output-head flag %u",
             hello.has_output);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
  ...
  if (hello.has_hidden > 1u) {
    snprintf(err, sizeof(err), "invalid worker hidden-state flag %u",
             hello.has_hidden);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
  ...
  if (hello.layer_start >= hello.n_layers ||
      hello.layer_end >= hello.n_layers ||
      hello.layer_end < hello.layer_start) {
    snprintf(err, sizeof(err), "invalid worker layer range %u:%u for %u layers",
             hello.layer_start, hello.layer_end, hello.n_layers);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
    dist_send_error(fd, err);
  ...
  if (hello.listen_port == 0 || hello.listen_port > 65535u) {
    snprintf(err, sizeof(err), "invalid worker data listen port %u",
             hello.listen_port);
    DIST_COORD_DEBUG(state,
                     "ds4: distributed coordinator: rejecting %s:%s: %s\n",
                     peer_host, peer_port, err);
  ...
  dist_coordinator_add_worker(state, fd, peer_host, peer_port, &hello,
                              model_name);
  ...
  if (state->use_control_for_work) {
    ...
    for (;;) {
      ...
      if (rc < 0) {
        DIST_COORD_DEBUG(
            state,
            "ds4: distributed coordinator: worker %s:%s protocol error: %s\n",
            peer_host, peer_port, err);
        break;
      }
      if (type == DS4_DIST_MSG_HELLO) {
        DIST_COORD_DEBUG(
            state,
            "ds4: distributed coordinator: worker %s:%s sent duplicate HELLO\n",
            peer_host, peer_port);
        dist_discard_bytes(fd, bytes);
        break;
      }
...
static int dist_session_ensure_route(ds4_dist_session *d, char *err,
                                     size_t errlen) {
  if (!d) {
    if (errlen)
      snprintf(err, errlen, "missing distributed session");
    return 1;
  }
  uint64_t generation = dist_coordinator_generation(&d->state);
  if (d->plan_ready && d->plan_generation == generation)
    return 0;
  dist_route_plan_free(&d->plan);
  if (!dist_coordinator_ensure_route(&d->state, &d->plan, &generation, err,
                                     errlen)) {
    d->plan_ready = false;
    d->plan_generation = 0;
    return 1;
...
static int dist_write_snapshot_load_begin(
    int fd, const ds4_dist_snapshot_begin_fixed *begin, const int *tokens) {
  ...
  for (uint32_t i = 0; i < begin->token_count; i++) {
    ...
    if (dist_write_full(fd, &t, sizeof(t)) != 0)
      return -1;
  }
  return 1;
}

static int dist_read_snapshot_begin_frame(int fd,
                                          ds4_dist_snapshot_begin_fixed *begin,
                                          char *msg, size_t msg_cap, char *err,
                                          size_t errlen) {
  if (msg_cap)
    msg[0] = '\0';
  uint32_t type = 0, bytes = 0;
  ...
  if (rc <= 0) {
    if (rc == 0 && errlen)
      snprintf(err, errlen, "distributed worker closed snapshot connection");
    return 1;
  }
  if (type != DS4_DIST_MSG_SNAPSHOT_BEGIN ||
    ...
    if (errlen)
      snprintf(err, errlen,
               "distributed worker returned invalid snapshot frame");
    return 1;
  }
  rc = dist_read_full(fd, begin, sizeof(*begin));
  if (rc <= 0) {
    if (errlen)
      snprintf(err, errlen, "failed to read distributed snapshot header");
    return 1;
  }
  dist_snapshot_begin_from_wire(begin);
  uint32_t body = bytes - (uint32_t)sizeof(*begin);
  uint64_t expected_token_bytes =
      (uint64_t)begin->token_count * sizeof(uint32_t);
  if (expected_token_bytes > UINT32_MAX ||
    ...
    if (errlen)
      snprintf(err, errlen, "invalid distributed snapshot response header");
    return 1;
  }
  if (begin->token_bytes != 0) {
    ...
    if (rc <= 0) {
      if (errlen)
        snprintf(err, errlen,
                 "failed to discard distributed snapshot response tokens");
      return 1;
    }
  ...
  if (begin->message_bytes != 0) {
    ...
    uint32_t copy =
        msg_cap && n < msg_cap ? n : (msg_cap ? (uint32_t)msg_cap - 1u : 0u);
    if (copy != 0) {
  ...
  if (body != 0) {
    ...
    if (rc <= 0) {
      if (errlen)
        snprintf(
            err, errlen,
            "failed to discard trailing distributed snapshot response bytes");
      return 1;
    }
  }
  return 0;
...
static int dist_read_snapshot_done_frame(int fd, uint64_t request_id, char *err,
                                         size_t errlen) {
  uint32_t type = 0, bytes = 0;
  int rc = dist_read_frame_header(fd, &type, &bytes, err, errlen);
  if (rc <= 0) {
    if (rc == 0 && errlen)
      snprintf(err, errlen,
               "distributed worker closed before snapshot completion");
    return 1;
  }
  ...
  if (rc <= 0) {
    if (errlen)
      snprintf(err, errlen, "failed to read distributed snapshot completion");
    return 1;
  }
  ...
  if (done.message_bytes > body) {
    dist_discard_bytes(fd, body);
    if (errlen)
      snprintf(err, errlen, "invalid distributed snapshot completion message");
    return 1;
  }
  if (done.message_bytes != 0) {
    uint32_t copy = done.message_bytes < sizeof(msg)
                        ? done.message_bytes
                        : (uint32_t)sizeof(msg) - 1u;
    rc = dist_read_full(fd, msg, copy);
    if (rc <= 0) {
      if (errlen)
        snprintf(err, errlen,
  ...
  if (body != 0) {
    ...
    if (rc <= 0) {
      if (errlen)
        snprintf(
            err, errlen,
            "failed to discard trailing distributed snapshot completion bytes");
      return 1;
    }
  }
  ...
  if (got_request != request_id) {
    if (errlen)
      snprintf(err, errlen, "distributed snapshot completion request mismatch");
    return 1;
  }
  if (done.status != 0) {
    if (errlen)
...
static int dist_receive_snapshot_chunks_to_file(int fd, uint64_t request_id,
                                                FILE *fp,
  ...
  if (!buf) {
    if (errlen)
      snprintf(err, errlen, "out of memory receiving distributed KV shard");
    return 1;
  }
  uint64_t received = 0;
  int fail = 0;
  while (!fail && received < payload_bytes) {
    ...
    if (rc <= 0) {
      if (rc == 0 && errlen)
        snprintf(err, errlen,
                 "distributed worker closed while sending KV shard");
      fail = 1;
      break;
    }
    if (type != DS4_DIST_MSG_SNAPSHOT_CHUNK ||
      ...
      dist_discard_bytes(fd, bytes);
      if (errlen)
        snprintf(err, errlen, "expected distributed KV shard chunk");
      fail = 1;
      break;
    }
    ...
    uint64_t got_request =
        dist_u64_from_halves(chunk.request_hi, chunk.request_lo);
    uint32_t chunk_bytes = bytes - (uint32_t)sizeof(chunk);
    if (got_request != request_id || chunk.chunk_bytes != chunk_bytes ||
        chunk_bytes > DS4_DIST_SNAPSHOT_CHUNK_BYTES ||
        chunk_bytes > payload_bytes - received) {
      dist_discard_bytes(fd, chunk_bytes);
      if (errlen)
        snprintf(err, errlen, "invalid distributed KV shard chunk");
      fail = 1;
      break;
    }
    ...
    if (rc <= 0) {
      if (errlen)
        snprintf(err, errlen, "failed to read distributed KV shard chunk");
      fail = 1;
      break;
    }
    if (fwrite(buf, 1, chunk_bytes, fp) != chunk_bytes) {
      if (errlen)
        snprintf(err, errlen, "failed to write distributed KV shard");
      fail = 1;
      break;
    }
...
static uint64_t dist_kv_state_bytes(uint32_t ratio, uint32_t head_dim,
                                    bool *ok) {
  const uint32_t coff = ratio == 4u ? 2u : 1u;
  uint64_t bytes = 0;
  uint64_t tmp = 0;
  if (!dist_u64_mul(coff, head_dim, &tmp) || !dist_u64_mul(tmp, coff, &tmp) ||
...
static bool dist_kv_layer_tensor_bytes(ds4_engine *engine,
                                       const ds4_dist_kv_layout *layout,
                                       uint32_t layer, uint32_t n_comp,
                                       uint32_t n_index_comp, uint64_t *out) {
  if (!layout || !out)
    return false;
  uint64_t bytes = 0;
  uint64_t tmp = 0;
  if (!dist_u64_mul(layout->raw_live, layout->head_dim, &tmp) ||
      !dist_u64_mul(tmp, sizeof(float), &tmp) || !dist_u64_add(&bytes, tmp))
    return false;
  ...
  if (ratio != 0) {
    if (!dist_u64_mul(n_comp, layout->head_dim, &tmp) ||
      ...
      return false;
    bool ok = true;
    const uint64_t attn_state =
        dist_kv_state_bytes(ratio, layout->head_dim, &ok);
    if (!ok || !dist_u64_add(&bytes, attn_state) ||
    ...
    if (ratio == 4u) {
      ...
      if (!ok || !dist_u64_add(&bytes, index_state) ||
...
static bool dist_kv_layout_matches(const ds4_dist_kv_layout *a,
                                   const ds4_dist_kv_layout *b) {
  return a->ctx == b->ctx && a->prefill_cap == b->prefill_cap &&
         a->raw_cap == b->raw_cap && a->raw_window == b->raw_window &&
         a->comp_cap == b->comp_cap && a->token_count == b->token_count &&
         a->n_layers == b->n_layers && a->head_dim == b->head_dim &&
...
static bool dist_kv_raw_live_valid(const ds4_dist_kv_layout *layout) {
  if (!layout || layout->raw_window == 0 || layout->raw_cap == 0)
    return false;
  const uint32_t expected = layout->token_count < layout->raw_window
                                ? layout->token_count
                                : layout->raw_window;
  return layout->raw_live == expected && layout->raw_live <= layout->raw_cap;
}

static int dist_kv_parse_layer_payload(
    ds4_engine *engine, FILE *fp, uint64_t bytes, uint32_t expected_start,
    uint32_t expected_end, ds4_dist_kv_layout *layout, bool *layout_set,
    uint32_t *n_comp, uint32_t *n_index_comp, ds4_dist_kv_shard_file *shard,
    char *err, size_t errlen) {
  ...
  if (h[0] != DS4_SESSION_LAYER_PAYLOAD_MAGIC ||
    ...
    if (errlen)
      snprintf(err, errlen, "unsupported distributed KV layer payload");
    return 1;
  }
  ...
  if (layer_start != expected_start || layer_end != expected_end ||
      layer_start > layer_end || layer_end >= got.n_layers) {
    if (errlen)
      snprintf(err, errlen, "distributed KV shard range mismatch");
    return 1;
  }
  if (got.n_layers != (uint32_t)ds4_engine_layer_count(engine) ||
      !dist_kv_raw_live_valid(&got)) {
    if (errlen)
      snprintf(err, errlen, "distributed KV shard layout is invalid");
    return 1;
  }
  ...
  for (uint32_t i = 0; i < slice_layers; i++) {
    ...
    if (n_comp[il] > got.comp_cap) {
      if (errlen)
        snprintf(err, errlen,
                 "distributed KV shard has invalid compressed row count");
      return 1;
    }
  }
  for (uint32_t i = 0; i < slice_layers; i++) {
    const uint32_t il = layer_start + i;
    if (dist_payload_read_u32(fp, &n_index_comp[il], &remaining, err, errlen) !=
        0)
      return 1;
    if (n_index_comp[il] > got.comp_cap) {
      if (errlen)
        snprintf(err, errlen,
                 "distributed KV shard has invalid indexer row count");
      return 1;
    }
  }
  ...
  if (tensor_pos < 0) {
    if (errlen)
      snprintf(err, errlen, "failed to locate distributed KV shard tensors");
    return 1;
  }
  uint64_t expected_tensor_bytes = 0;
  for (uint32_t il = layer_start; il <= layer_end; il++) {
    uint64_t layer_bytes = 0;
    if (!dist_kv_layer_tensor_bytes(engine, &got, il, n_comp[il],
                                    n_index_comp[il], &layer_bytes) ||
        !dist_u64_add(&expected_tensor_bytes, layer_bytes)) {
      if (errlen)
        snprintf(err, errlen, "distributed KV shard tensor size overflow");
      return 1;
    }
  }
  if (remaining != expected_tensor_bytes) {
    if (errlen)
      snprintf(err, errlen, "distributed KV shard tensor byte count mismatch");
    return 1;
  }
  if (shard) {
...
static int dist_kv_write_session_header(FILE *fp,
                                        const ds4_dist_kv_layout *layout,
                                        char *err, size_t errlen) {
  ...
  for (uint32_t i = 0; i < DS4_SESSION_PAYLOAD_U32_FIELDS; i++) {
    if (dist_payload_write_u32(fp, h[i], err, errlen) != 0)
      return 1;
  }
  return 0;
}

static int dist_kv_write_layer_header(FILE *fp,
                                      const ds4_dist_kv_layout *layout,
                                      uint32_t layer_start, uint32_t layer_end,
                                      const uint32_t *n_comp,
                                      const uint32_t *n_index_comp, char *err,
                                      size_t errlen) {
...
static void dist_kv_route_shard(const ds4_dist_session *d, uint32_t shard,
                                uint32_t *layer_start, uint32_t *layer_end,
                                const ds4_dist_route_entry **entry) {
  if (entry)
    *entry = NULL;
  if (shard == 0) {
    if (layer_start)
      *layer_start = d->state.local_start;
    if (layer_end)
      *layer_end = d->state.local_end;
    return;
  }
...
static int dist_kv_route_validate(const ds4_dist_session *d, char *err,
                                  size_t errlen) {
  if (!d || d->state.n_layers == 0 || d->state.local_start != 0 ||
      d->state.local_end >= d->state.n_layers) {
    if (errlen)
      snprintf(err, errlen, "distributed KV route does not start at layer 0");
    return 1;
  }
  uint32_t prev = d->state.local_end;
  for (uint32_t i = 0; i < d->plan.count; i++) {
    const ds4_dist_route_entry *e = &d->plan.entry[i];
    if (prev == UINT32_MAX || e->layer_start != prev + 1u ||
        e->layer_end < e->layer_start || e->layer_end >= d->state.n_layers) {
      if (errlen)
        snprintf(err, errlen, "distributed KV route is not contiguous");
      return 1;
    }
    prev = e->layer_end;
  }
  if (prev + 1u != d->state.n_layers) {
    if (errlen)
      snprintf(err, errlen, "distributed KV route does not cover all layers");
    return 1;
  }
  return 0;
}

static void dist_kv_shards_close(ds4_dist_kv_shard_file *shards,
                                 uint32_t count) {
  if (!shards)
    return;
...
static int dist_save_remote_shard_to_file(ds4_dist_session *d,
                                          const ds4_dist_route_entry *entry,
                                          const ds4_tokens *tokens,
                                          uint64_t token_hash, FILE *fp,
  ...
  if (dist_write_frame_header(fd, DS4_DIST_MSG_SNAPSHOT_SAVE_REQ,
    ...
    if (errlen)
      snprintf(err, errlen, "failed to request distributed KV shard");
    goto cleanup;
  }

  ds4_dist_snapshot_begin_fixed begin;
  char msg[256];
  if (dist_read_snapshot_begin_frame(fd, &begin, msg, sizeof(msg), err,
                                     errlen) != 0)
    goto cleanup;
  if (begin.status != 0) {
    if (errlen)
  ...
  const uint64_t payload_bytes =
      dist_u64_from_halves(begin.payload_hi, begin.payload_lo);
  uint64_t got_session =
      dist_u64_from_halves(begin.session_hi, begin.session_lo);
  uint64_t got_request =
      dist_u64_from_halves(begin.request_hi, begin.request_lo);
  uint64_t got_hash =
      dist_u64_from_halves(begin.token_hash_hi, begin.token_hash_lo);
  if (begin.model_id != d->state.model_id || got_session != d->session_id ||
      got_request != request_id || got_hash != token_hash ||
    ...
    if (errlen)
      snprintf(err, errlen, "distributed KV shard metadata mismatch");
    goto cleanup;
  }
  if (dist_receive_snapshot_chunks_to_file(fd, request_id, fp, payload_bytes,
                                           err, errlen) != 0)
    goto cleanup;
...
static int dist_prepare_shard_from_session_payload(
    ds4_dist_session *d, FILE *src, uint64_t *remaining,
    const ds4_dist_kv_layout *layout, const uint32_t *n_comp,
    const uint32_t *n_index_comp, uint32_t layer_start, uint32_t layer_end,
    FILE **tmp_out, uint64_t *payload_bytes_out, char *err, size_t errlen) {
  if (tmp_out)
    *tmp_out = NULL;
  if (payload_bytes_out)
    *payload_bytes_out = 0;
  ...
  int rc = 1;
  if (dist_kv_write_layer_header(tmp, layout, layer_start, layer_end, n_comp,
                                 n_index_comp, err, errlen) != 0)
    goto cleanup;
  for (uint32_t il = layer_start; il <= layer_end; il++) {
    uint64_t layer_bytes = 0;
    if (!dist_kv_layer_tensor_bytes(d->state.engine, layout, il, n_comp[il],
                                    n_index_comp[il], &layer_bytes)) {
      if (errlen)
        snprintf(err, errlen, "distributed KV layer byte count overflow");
      goto cleanup;
    }
    if (dist_payload_copy_bytes(src, tmp, layer_bytes, remaining, err,
                                errlen) != 0)
      goto cleanup;
  }
  uint64_t bytes = 0;
  if (dist_measure_file(tmp, &bytes, "distributed KV shard", err, errlen) !=
          0 ||
      dist_rewind_file(tmp, "distributed KV shard", err, errlen) != 0)
    goto cleanup;
  *tmp_out = tmp;
  if (payload_bytes_out)
    *payload_bytes_out = bytes;
  rc = 0;
...
static int dist_load_remote_shard_from_payload(
    ds4_dist_session *d, const ds4_dist_route_entry *entry, const int *tokens,
    uint32_t token_count, uint64_t token_hash, FILE *fp, uint64_t payload_bytes,
    char *err, size_t errlen) {
  uint64_t request_id = d->snapshot_request_id++;
  int fd = dist_connect_endpoint(entry->host, (int)entry->port, err, errlen);
  if (fd < 0)
  ...
  if (dist_write_snapshot_load_begin(fd, &begin, tokens) <= 0) {
    if (errlen)
      snprintf(err, errlen,
               "failed to send distributed KV shard restore request");
    goto cleanup;
  }
...
int ds4_dist_session_save_payload(ds4_dist_session *d, ds4_session *owner,
                                  FILE *fp, char *err, size_t errlen) {
  if (!d || !owner || !fp) {
    if (errlen)
      snprintf(err, errlen, "invalid distributed payload save");
    return 1;
  }
  if (dist_session_ensure_route(d, err, errlen) != 0)
    return 1;
  if (dist_kv_route_validate(d, err, errlen) != 0)
    return 1;

  const ds4_tokens *tokens = ds4_session_tokens(owner);
  if (!tokens || tokens->len < 0 || (uint64_t)tokens->len > UINT32_MAX) {
    if (errlen)
      snprintf(err, errlen, "distributed session has no valid token timeline");
    return 1;
  }
  ...
  if (!logits) {
    if (errlen)
      snprintf(err, errlen, "out of memory saving distributed logits");
    return 1;
  }
  if (ds4_session_copy_logits(owner, logits, (int)vocab) != (int)vocab) {
    free(logits);
    if (errlen)
      snprintf(err, errlen, "failed to copy distributed logits");
    return 1;
  }
  ...
  if (!shards || !n_comp || !n_index_comp) {
    ...
    free(n_index_comp);
    if (errlen)
      snprintf(err, errlen, "out of memory saving distributed KV payload");
    return 1;
  }
  ...
  for (uint32_t shard = 0; shard < shard_count; shard++) {
    ...
    if (!shards[shard].fp)
      goto cleanup;
    if (shard == 0) {
      if (ds4_session_save_layer_payload(owner, shards[shard].fp, layer_start,
                                         layer_end, err, errlen) != 0)
        goto cleanup;
      ...
      if (dist_save_remote_shard_to_file(d, entry, tokens, token_hash,
                                         shards[shard].fp, &shards[shard].bytes,
                                         err, errlen) != 0)
        goto cleanup;
    }
    if (shards[shard].bytes == 0) {
      if (errlen)
        snprintf(err, errlen, "distributed KV shard is empty");
      goto cleanup;
    }
    if (dist_kv_parse_layer_payload(d->state.engine, shards[shard].fp,
                                    shards[shard].bytes, layer_start, layer_end,
                                    &layout, &layout_set, n_comp, n_index_comp,
                                    &shards[shard], err, errlen) != 0)
      goto cleanup;
  }
  if (!layout_set || layout.token_count != token_count ||
      layout.n_layers != d->state.n_layers || layout.vocab != vocab) {
    if (errlen)
      snprintf(err, errlen, "distributed KV shard metadata mismatch");
    goto cleanup;
  }
  ...
  if (dist_payload_write_bytes(fp, logits, (uint64_t)vocab * sizeof(logits[0]),
                               err, errlen) != 0)
    goto cleanup;
  ...
  for (uint32_t shard = 0; shard < shard_count; shard++) {
    if (dist_copy_file_range(shards[shard].fp, shards[shard].tensor_offset,
                             shards[shard].tensor_bytes, fp, err, errlen) != 0)
      goto cleanup;
  }
  rc = 0;
...
int ds4_dist_session_load_payload(ds4_dist_session *d, ds4_session *owner,
                                  FILE *fp, uint64_t payload_bytes, char *err,
                                  size_t errlen) {
  if (!d || !owner || !fp) {
    if (errlen)
      snprintf(err, errlen, "invalid distributed payload load");
    return 1;
  }
  if (dist_session_ensure_route(d, err, errlen) != 0)
    return 1;
  ...
  if (h[0] != DS4_SESSION_PAYLOAD_MAGIC ||
    ...
    if (errlen)
      snprintf(err, errlen, "unsupported DS4 KV payload version");
    return 1;
  }
  ...
  if (layout.n_layers != d->state.n_layers ||
    ...
    if (errlen)
      snprintf(err, errlen,
               "DS4 KV payload does not match current distributed runtime");
    return 1;
  }

  int *tokens = layout.token_count
                    ? malloc((size_t)layout.token_count * sizeof(tokens[0]))
                    : NULL;
  ...
  if ((layout.token_count && !tokens) || !logits || !n_comp || !n_index_comp) {
    ...
    free(n_index_comp);
    if (errlen)
      snprintf(err, errlen, "out of memory loading distributed KV payload");
    return 1;
  }
  for (uint32_t i = 0; i < layout.token_count; i++) {
    ...
    if (tok > (uint32_t)INT_MAX ||
      ...
      if (errlen)
        snprintf(err, errlen,
                 "distributed KV payload token is outside vocabulary");
      return 1;
    }
    tokens[i] = (int)tok;
  }
  ...
  const uint64_t token_hash =
      dist_token_hash_prefix(tokens_arg, layout.token_count);
  ...
  for (uint32_t il = 0; il < layout.n_layers; il++) {
    ...
    if (n_comp[il] > layout.comp_cap) {
      if (errlen)
        snprintf(err, errlen,
                 "DS4 KV payload has invalid compressed row count");
      goto cleanup;
    }
  }
  for (uint32_t il = 0; il < layout.n_layers; il++) {
    ...
    if (n_index_comp[il] > layout.comp_cap) {
      if (errlen)
        snprintf(err, errlen, "DS4 KV payload has invalid indexer row count");
      goto cleanup;
    }
  }
  ...
  for (uint32_t shard = 0; shard < shard_count; shard++) {
    ...
    dist_kv_route_shard(d, shard, &layer_start, &layer_end, &entry);
    if (dist_prepare_shard_from_session_payload(
            d, fp, &remaining, &layout, n_comp, n_index_comp, layer_start,
            layer_end, &tmp, &shard_bytes, err, errlen) != 0)
      goto cleanup;
    if (shard == 0) {
      if (ds4_session_load_layer_payload(owner, tmp, shard_bytes, tokens_arg,
                                         layout.token_count, layer_start,
                                         layer_end, err, errlen) != 0) {
        fclose(tmp);
        goto cleanup;
      }
    } else {
      if (dist_load_remote_shard_from_payload(
              d, entry, tokens_arg, layout.token_count, token_hash, tmp,
              shard_bytes, err, errlen) != 0) {
        fclose(tmp);
        goto cleanup;
      }
    }
  ...
  if (remaining != 0) {
    if (errlen)
      snprintf(err, errlen, "DS4 KV payload has trailing bytes");
    goto cleanup;
  }
  if (ds4_session_set_logits(owner, logits, (int)layout.vocab) != 0) {
    if (errlen)
      snprintf(err, errlen, "failed to restore distributed logits");
    goto cleanup;
  }
  rc = 0;
...
int ds4_dist_session_create(ds4_dist_session **out, ds4_engine *engine,
                            const ds4_dist_options *opt, ds4_session *owner,
                            int ctx_size, char *err, size_t errlen) {
  (void)owner;
  if (!out || !engine || !opt) {
    if (errlen)
      snprintf(err, errlen, "missing distributed session parameters");
    return 1;
  }
  *out = NULL;
  if (opt->role != DS4_DISTRIBUTED_COORDINATOR) {
    if (errlen)
      snprintf(err, errlen, "distributed session requires coordinator role");
    return 1;
  }
  if (dist_validate_options(opt, err, errlen) != 0)
    return 1;

  int listen_fd =
      dist_open_listener(opt->listen_host, opt->listen_port, err, errlen);
  if (listen_fd < 0)
    return 1;
  ...
  if (!d) {
    close(listen_fd);
    if (errlen)
      snprintf(err, errlen, "out of memory creating distributed session");
    return 1;
  }
  ...
  if (opt->layers.has_output)
    ...
    snprintf(local_end, sizeof(local_end), "%u", opt->layers.end);
  DIST_COORD_DEBUG(&d->state,
                   "ds4: distributed coordinator API: listening on %s:%d "
                   "model_id=%u layers=%u local=%u:%s activation_bits=%u\n",
                   opt->listen_host, opt->listen_port, d->state.model_id,
                   d->state.n_layers, opt->layers.start, local_end,
                   d->state.activation_bits);
...
int ds4_dist_session_route_ready(ds4_dist_session *d, char *err,
                                 size_t errlen) {
  if (!d) {
    if (errlen)
      snprintf(err, errlen, "missing distributed session");
  ...
  if (!dist_coordinator_build_route_plan(&d->state, &probe, NULL, err,
                                         errlen)) {
    return 0;
  }
...
int ds4_dist_session_sync(ds4_dist_session *d, ds4_session *owner,
                          const ds4_tokens *checkpoint,
                          const ds4_tokens *prompt, float *logits, char *err,
                          size_t errlen) {
  if (!d || !owner || !prompt || prompt->len <= 0 || !logits) {
    if (errlen)
      snprintf(err, errlen, "invalid distributed sync request");
    return 1;
  }
  if (dist_session_ensure_route(d, err, errlen) != 0)
    return 1;

  if (checkpoint && checkpoint->len >= 0 && checkpoint->len <= prompt->len &&
      ds4_tokens_starts_with(prompt, checkpoint)) {
    if (checkpoint->len == prompt->len)
      return 0;

    uint32_t chunk_cap = 0;
    if (dist_coordinator_prefill_chunk_cap(&d->state, owner, &chunk_cap, err,
                                           errlen) != 0) {
      return 1;
    }
    const uint32_t pos0 = (uint32_t)checkpoint->len;
    const uint32_t suffix = (uint32_t)prompt->len - pos0;
    if (dist_coordinator_can_pipeline_prefill(&d->state, &d->plan, owner,
                                              suffix, chunk_cap)) {
      int prefill_rc = dist_coordinator_prefill_prompt_pipelined(
          &d->state, owner, &d->plan, prompt, pos0, suffix, false, chunk_cap,
          d->session_id, &d->request_id, logits, err, errlen);
      if (prefill_rc != 0) {
        if (dist_coordinator_rebuild_from_transcript(
                &d->state, owner, &d->plan, prompt, d->session_id,
                &d->request_id, logits, &d->plan_generation,
                prefill_rc != DS4_DIST_RECV_REMOTE_ERROR, err, errlen) != 0) {
          d->plan_ready = false;
          d->plan_generation = 0;
          return 1;
        }
    ...
    while (pos < (uint32_t)prompt->len) {
      ...
      const uint32_t chunk = remaining < chunk_cap ? remaining : chunk_cap;
      int eval_rc = dist_coordinator_eval_span(
          &d->state, owner, &d->plan, prompt->v + pos, chunk, pos,
          d->session_id, d->request_id++, false, logits, err, errlen);
      if (eval_rc != 0) {
        if (dist_coordinator_rebuild_from_transcript(
                &d->state, owner, &d->plan, prompt, d->session_id,
                &d->request_id, logits, &d->plan_generation,
                eval_rc != DS4_DIST_RECV_REMOTE_ERROR, err, errlen) != 0) {
          d->plan_ready = false;
          d->plan_generation = 0;
          return 1;
        }
  ...
  int prefill_rc = dist_coordinator_prefill_prompt(
      &d->state, owner, &d->plan, prompt, d->session_id, &d->request_id, logits,
      err, errlen);
  if (prefill_rc != 0) {
    if (dist_coordinator_rebuild_from_transcript(
            &d->state, owner, &d->plan, prompt, d->session_id, &d->request_id,
            logits, &d->plan_generation,
            prefill_rc != DS4_DIST_RECV_REMOTE_ERROR, err, errlen) != 0) {
      d->plan_ready = false;
      d->plan_generation = 0;
      return 1;
...
int ds4_dist_session_eval(ds4_dist_session *d, ds4_session *owner,
                          const ds4_tokens *checkpoint, int token,
                          float *logits, char *err, size_t errlen) {
  if (!d || !owner || !checkpoint || checkpoint->len < 0 || !logits) {
    if (errlen)
      snprintf(err, errlen, "invalid distributed decode request");
    return 1;
  }
  if (dist_session_ensure_route(d, err, errlen) != 0)
    return 1;
  ...
  int rc = dist_coordinator_eval_span(
      &d->state, owner, &d->plan, &token, 1, (uint32_t)checkpoint->len,
      d->session_id, d->request_id++, false, logits, err, errlen);
  if (rc != 0) {
    if (dist_coordinator_rebuild_from_transcript(
            &d->state, owner, &d->plan, &transcript, d->session_id,
            &d->request_id, logits, &d->plan_generation,
            rc != DS4_DIST_RECV_REMOTE_ERROR, err, errlen) != 0) {
      d->plan_ready = false;
      d->plan_generation = 0;
      ds4_tokens_free(&transcript);
      return 1;
...
static int dist_run_coordinator(ds4_engine *engine, const ds4_dist_options *opt,
                                const ds4_dist_generation_options *gen) {
  char err[256];
  int listen_fd =
      dist_open_listener(opt->listen_host, opt->listen_port, err, sizeof(err));
  if (listen_fd < 0) {
    fprintf(stderr, "ds4: distributed coordinator: %s\n", err);
    return 1;
  }
  ...
  if (opt->layers.has_output)
    ...
    snprintf(local_end, sizeof(local_end), "%u", opt->layers.end);
  DIST_COORD_DEBUG(&state,
                   "ds4: distributed coordinator: listening on %s:%d "
                   "model_id=%u layers=%u local=%u:%s activation_bits=%u\n",
                   opt->listen_host, opt->listen_port, state.model_id,
  ...
  pthread_t accept_tid;
  if (pthread_create(&accept_tid, NULL, dist_coordinator_accept_main,
                     &accept_ctx) != 0) {
...
static int dist_worker_read_loop(ds4_dist_worker_state *state, int fd) {
  ...
  for (;;) {
    ...
    if (type == DS4_DIST_MSG_ERROR) {
      char msg[512];
      uint32_t n =
          bytes < sizeof(msg) - 1u ? bytes : (uint32_t)sizeof(msg) - 1u;
    ...
    fprintf(stderr,
            "ds4: distributed worker: rejected unsupported frame type %u\n",
            type);
    loop_rc = 1;
    break;
  }
...
static int dist_send_work_result(int fd, uint64_t request_id,
                                 uint64_t result_hash, uint32_t status,
                                 uint32_t result_kind, uint32_t payload_bits,
                                 const ds4_dist_telemetry_fixed *telemetry,
                                 uint32_t telemetry_count, const void *payload,
                                 uint32_t payload_bytes) {
  if (payload_bytes != 0 && !payload)
    return -1;
  if (telemetry_count != 0 && !telemetry)
    return -1;
  ...
  if (status == 0 && result_kind == DS4_DIST_RESULT_HIDDEN_STATE) {
    ...
    if (!dist_activation_wire_bytes(payload_bits, hidden_values,
                                    &wire_payload_bytes))
      return -1;
  ...
  if (dist_write_frame_header(fd, DS4_DIST_MSG_RESULT, (uint32_t)frame_bytes) !=
      0)
    return -1;
  if (dist_write_full(fd, &wire, sizeof(wire)) != 0)
    return -1;
  ...
  if (status == 0 && result_kind == DS4_DIST_RESULT_HIDDEN_STATE &&
    ...
    if (dist_write_activation_payload(fd, payload, hidden_values,
                                      payload_bits) != 0)
      return -1;
  } else if (payload_bytes && payload &&
             dist_write_full(fd, payload, payload_bytes) != 0) {
    return -1;
  }
  return 1;
}
...
static int dist_send_work_error(int fd, uint64_t request_id, const char *msg) {
  if (!msg)
    msg = "distributed work failed";
  size_t len = strlen(msg);
  if (len > UINT32_MAX)
    len = UINT32_MAX;
  return dist_send_work_result(fd, request_id, 0, 1, 0, 0, NULL, 0, msg,
                               (uint32_t)len);
}

static int dist_send_snapshot_begin(int fd,
                                    const ds4_dist_snapshot_begin_fixed *begin,
                                    const int *tokens, const char *msg) {
  uint64_t token_bytes64 = (uint64_t)begin->token_count * sizeof(uint32_t);
  if (token_bytes64 > UINT32_MAX ||
      begin->token_bytes != (uint32_t)token_bytes64)
    return -1;
  uint32_t msg_len = msg ? (uint32_t)strlen(msg) : 0u;
  if (msg_len != begin->message_bytes)
    return -1;
  ...
  ds4_dist_snapshot_begin_fixed wire = *begin;
  dist_snapshot_begin_to_wire(&wire);
  if (dist_write_frame_header(fd, DS4_DIST_MSG_SNAPSHOT_BEGIN,
                              (uint32_t)frame_bytes64) != 0)
    return -1;
  if (dist_write_full(fd, &wire, sizeof(wire)) != 0)
    return -1;
  ...
  if (msg_len && dist_write_full(fd, msg, msg_len) != 0)
    return -1;
  return 1;
}

static int dist_send_snapshot_error(int fd, uint64_t request_id,
                                    uint64_t session_id, uint32_t model_id,
                                    uint32_t layer_start, uint32_t layer_end,
                                    const char *msg) {
  if (!msg)
    msg = "distributed snapshot failed";
  size_t len = strlen(msg);
  if (len > UINT32_MAX)
    len = UINT32_MAX;
  ds4_dist_snapshot_begin_fixed begin;
...
static int dist_send_snapshot_done(int fd, uint64_t request_id, uint32_t status,
                                   const char *msg) {
  if (!msg)
    msg = "";
  size_t len = strlen(msg);
  ...
  if (dist_write_frame_header(fd, DS4_DIST_MSG_SNAPSHOT_DONE,
                              (uint32_t)frame_bytes64) != 0)
    return -1;
  if (dist_write_full(fd, &wire, sizeof(wire)) != 0)
    return -1;
  if (len && dist_write_full(fd, msg, len) != 0)
    return -1;
  return 1;
}
...
static int dist_send_snapshot_file_chunks(int fd, uint64_t request_id, FILE *fp,
  ...
  while (bytes != 0) {
    ...
    if (dist_write_frame_header(fd, DS4_DIST_MSG_SNAPSHOT_CHUNK, frame_bytes) !=
            0 ||
        dist_write_full(fd, &wire, sizeof(wire)) != 0 ||
...
static bool dist_route_get_entry(const void *route_blob, uint32_t route_bytes,
                                 uint32_t route_count, uint32_t target_index,
                                 ds4_dist_route_entry *out, char *err,
                                 size_t errlen) {
  if (!route_blob || !out || target_index >= route_count) {
    if (errlen)
      snprintf(err, errlen, "invalid route entry index");
    return false;
  }
  ...
  for (uint32_t i = 0; i < route_count; i++) {
    if (remaining < sizeof(ds4_dist_route_fixed)) {
      if (errlen)
        snprintf(err, errlen, "truncated route entry");
      return false;
    }
    ds4_dist_route_fixed fixed;
    ...
    if (fixed.host_len == 0 || fixed.host_len >= NI_MAXHOST ||
        fixed.host_len > remaining) {
      if (errlen)
        snprintf(err, errlen, "invalid route host length");
      return false;
    }
    if (dist_bytes_have_nul(p, fixed.host_len)) {
      if (errlen)
        snprintf(err, errlen, "route host contains NUL bytes");
      return false;
    }
    if (i == target_index) {
  ...
  if (remaining != 0) {
    if (errlen)
      snprintf(err, errlen, "route payload has trailing bytes");
    return false;
  }
  if (errlen)
    snprintf(err, errlen, "route entry not found");
  return false;
}

static bool dist_route_get_return_target(const void *route_blob,
                                         uint32_t route_bytes,
                                         uint32_t route_count,
                                         ds4_dist_route_return *out, char *err,
                                         size_t errlen) {
  if (!route_blob || !out || route_count == 0) {
    if (errlen)
      snprintf(err, errlen, "invalid route final destination");
    return false;
  }
  const uint8_t *p = route_blob;
  uint32_t remaining = route_bytes;
  ...
  if (remaining < sizeof(ds4_dist_route_return_fixed)) {
    if (errlen)
      snprintf(err, errlen, "route payload missing final destination");
    return false;
  }
  ds4_dist_route_return_fixed fixed;
  memcpy(&fixed, p, sizeof(fixed));
  ...
  if (fixed.host_len >= NI_MAXHOST || fixed.host_len > remaining) {
    if (errlen)
      snprintf(err, errlen, "invalid route final destination host length");
    return false;
  }
  if (dist_bytes_have_nul(p, fixed.host_len)) {
    if (errlen)
      snprintf(err, errlen, "route final destination host contains NUL bytes");
    return false;
  }
...
static bool dist_route_validate_blob(const void *route_blob,
                                     uint32_t route_bytes, uint32_t route_count,
                                     uint32_t n_layers, char *err,
                                     size_t errlen) {
  if (route_count == 0) {
    if (route_bytes == 0)
      return true;
    if (errlen)
      snprintf(err, errlen, "route payload has entries without a route count");
    return false;
  }
  if (!route_blob) {
    if (errlen)
      snprintf(err, errlen, "route payload is missing");
    return false;
  }
  ...
  for (uint32_t i = 0; i < route_count; i++) {
    ...
    if (fixed.port == 0 || fixed.port > 65535u) {
      if (errlen)
        snprintf(err, errlen, "invalid route port");
      return false;
    }
    if (fixed.layer_start >= n_layers || fixed.layer_end >= n_layers ||
        fixed.layer_end < fixed.layer_start) {
      if (errlen)
        snprintf(err, errlen, "invalid route layer range");
      return false;
    }
    if ((fixed.flags & ~DS4_DIST_ROUTE_F_OUTPUT_LOGITS) != 0) {
      if (errlen)
        snprintf(err, errlen, "invalid route flags");
      return false;
    }
    if ((fixed.flags & DS4_DIST_ROUTE_F_OUTPUT_LOGITS) != 0 &&
        fixed.layer_end + 1u != n_layers) {
      if (errlen)
        snprintf(err, errlen, "route logits require final layer");
      return false;
    }
    if (i != 0 && fixed.layer_start != prev_end + 1u) {
      if (errlen)
        snprintf(err, errlen, "route layer ranges are not contiguous");
      return false;
    }
  ...
  if (ret.kind != DS4_DIST_ROUTE_RETURN_UPSTREAM) {
    if (errlen)
      snprintf(err, errlen, "unsupported route final destination");
    return false;
  }
  if (ret.host_len != 0 || ret.port != 0) {
    if (errlen)
      snprintf(err, errlen, "invalid upstream route final destination");
    return false;
  }
  p += ret.host_len;
...
static int dist_send_work_frame(int fd, const ds4_dist_work_fixed *work,
                                const int *tokens, const float *input_hc,
                                const void *route_blob) {
  if (!work || !tokens || work->n_tokens == 0)
    return -1;
  ...
  if (token_bytes > UINT32_MAX || work->token_bytes != (uint32_t)token_bytes)
    return -1;
  if (work->input_hc_bytes != 0 && !input_hc)
    return -1;
  if (work->route_bytes != 0 && !route_blob)
    return -1;
  uint64_t input_hc_values = 0;
  if (work->input_hc_bytes != 0 &&
      !dist_activation_values_from_wire_bytes(
          work->input_hc_bits, work->input_hc_bytes, &input_hc_values))
    return -1;
  const uint64_t frame_bytes = sizeof(ds4_dist_work_fixed) +
                               (uint64_t)work->token_bytes +
                               work->input_hc_bytes + work->route_bytes;
  if (frame_bytes > UINT32_MAX)
    return -1;

  ds4_dist_work_fixed wire = *work;
  dist_work_to_wire(&wire);
  if (dist_write_frame_header(fd, DS4_DIST_MSG_WORK, (uint32_t)frame_bytes) !=
      0)
    return -1;
  if (dist_write_full(fd, &wire, sizeof(wire)) != 0)
    return -1;
  ...
  if (work->input_hc_bytes &&
      dist_write_activation_payload(fd, input_hc, input_hc_values,
...
static int dist_worker_upstream_send_work_result(
    ds4_dist_worker_upstream *upstream, uint64_t request_id,
    uint64_t result_hash, uint32_t status, uint32_t result_kind,
    uint32_t payload_bits, const ds4_dist_telemetry_fixed *telemetry,
    uint32_t telemetry_count, const void *payload, uint32_t payload_bytes) {
  pthread_mutex_lock(&upstream->write_mu);
  int rc = dist_send_work_result(upstream->fd, request_id, result_hash, status,
                                 result_kind, payload_bits, telemetry,
                                 telemetry_count, payload, payload_bytes);
  pthread_mutex_unlock(&upstream->write_mu);
  return rc;
}
...
static bool dist_worker_forwarder_enqueue_request(
    ds4_dist_worker_forwarder *forwarder, uint64_t request_id,
    const ds4_dist_telemetry_fixed *telemetry, double downstream_t0) {
  pthread_mutex_lock(&forwarder->queue_mu);
  while (!forwarder->closing && forwarder->pending_depth != 0 &&
  ...
  if (forwarder->pending_tail)
    forwarder->pending_tail->next = node;
  else
    forwarder->pending_head = node;
...
static bool dist_worker_forwarder_pop_request(
    ds4_dist_worker_forwarder *forwarder, uint64_t *request_id,
    ds4_dist_telemetry_fixed *telemetry, double *downstream_t0) {
  pthread_mutex_lock(&forwarder->queue_mu);
  ...
  if (!forwarder->pending_head)
    forwarder->pending_tail = NULL;
  if (forwarder->pending_count != 0)
    forwarder->pending_count--;
  ...
  if (downstream_t0)
    *downstream_t0 = node->downstream_t0;
  free(node);
  return true;
}

static bool
dist_worker_forwarder_remove_request(ds4_dist_worker_forwarder *forwarder,
                                     uint64_t request_id) {
  pthread_mutex_lock(&forwarder->queue_mu);
  ...
  while (*link) {
    ...
    if (node->request_id == request_id) {
      *link = node->next;
      if (forwarder->pending_tail == node)
        forwarder->pending_tail = prev;
      if (forwarder->pending_count != 0)
        forwarder->pending_count--;
      pthread_cond_signal(&forwarder->queue_not_full);
      pthread_mutex_unlock(&forwarder->queue_mu);
...
static void dist_worker_forwarder_note_send_done(
    ds4_dist_worker_forwarder *forwarder, uint64_t request_id,
    uint32_t forward_send_usec, double downstream_t0) {
  pthread_mutex_lock(&forwarder->queue_mu);
  for (ds4_dist_pending_request *node = forwarder->pending_head; node;
       node = node->next) {
    if (node->request_id == request_id) {
...
static void
dist_worker_forwarder_clear_requests(ds4_dist_worker_forwarder *forwarder) {
  pthread_mutex_lock(&forwarder->queue_mu);
...
static void
dist_worker_forwarder_close_queue(ds4_dist_worker_forwarder *forwarder) {
  pthread_mutex_lock(&forwarder->queue_mu);
...
static void *dist_worker_forwarder_relay_main(void *arg) {
  ...
  DIST_DEBUG("relay start downstream=%s:%u fd=%d upstream_fd=%d",
             forwarder->host, forwarder->port, fd, upstream->fd);
  ...
  for (;;) {
    ...
    if (rc <= 0) {
      ...
      if (dist_worker_forwarder_pop_request(forwarder, &pending_request, NULL,
                                            NULL)) {
        dist_worker_upstream_send_work_error(upstream, pending_request,
                                             "next worker closed connection");
      }
      DIST_DEBUG("relay read header end downstream=%s:%u rc=%d err=%s",
                 forwarder->host, forwarder->port, rc, err);
      break;
    }
    DIST_DEBUG("relay got frame downstream=%s:%u type=%u bytes=%u",
               forwarder->host, forwarder->port, type, bytes);
    uint64_t expected_request = 0;
    if (type != DS4_DIST_MSG_RESULT || bytes < sizeof(ds4_dist_result_fixed)) {
      dist_discard_bytes(fd, bytes);
      if (dist_worker_forwarder_pop_request(forwarder, &expected_request, NULL,
                                            NULL)) {
        dist_worker_upstream_send_work_error(
            upstream, expected_request,
            "next worker did not return valid RESULT");
      }
      DIST_DEBUG("relay invalid frame downstream=%s:%u", forwarder->host,
                 forwarder->port);
      break;
    }
    ...
    if (rc <= 0) {
      ...
      DIST_DEBUG("relay read result fixed failed downstream=%s:%u rc=%d",
                 forwarder->host, forwarder->port, rc);
      break;
    }
    ...
    if (result.telemetry_bytes % (uint32_t)sizeof(ds4_dist_telemetry_fixed) !=
      ...
      if (dist_worker_forwarder_pop_request(forwarder, &expected_request, NULL,
                                            NULL)) {
        dist_worker_upstream_send_work_error(
            upstream, expected_request, "next worker RESULT metadata mismatch");
      }
      DIST_DEBUG("relay result metadata mismatch downstream=%s:%u telemetry=%u "
                 "payload=%u frame=%u",
                 forwarder->host, forwarder->port, result.telemetry_bytes,
                 result.payload_bytes, body_bytes);
      break;
    }
    ds4_dist_telemetry_fixed local_telemetry;
    double downstream_t0 = 0.0;
    if (!dist_worker_forwarder_pop_request(forwarder, &expected_request,
                                           &local_telemetry, &downstream_t0)) {
      dist_discard_bytes(fd, body_bytes);
      DIST_DEBUG(
          "relay got unexpected result request=%llu with no pending request",
    ...
    local_telemetry.downstream_wait_usec =
        dist_usec_since(downstream_t0, dist_now_sec());
    ...
    if (out_frame_bytes64 > UINT32_MAX) {
      dist_discard_bytes(fd, body_bytes);
      dist_worker_upstream_send_work_error(
          upstream, expected_request, "distributed RESULT frame is too large");
      break;
    }
    DIST_DEBUG(
        "relay result request=%llu status=%u kind=%u telemetry=%u payload=%u",
        (unsigned long long)got_request, result.status, result.result_kind,
        out_telemetry_count, result.payload_bytes);
    ...
    int write_rc = dist_write_frame_header(upstream->fd, DS4_DIST_MSG_RESULT,
                                           (uint32_t)out_frame_bytes64);
    if (write_rc == 0)
      write_rc = dist_write_full(upstream->fd, &out_wire_result,
                                 sizeof(out_wire_result));
    ...
    DIST_DEBUG("relay wrote result request=%llu write_rc=%d remaining=%u",
               (unsigned long long)got_request, write_rc, remaining);
    if (write_rc != 0)
      break;
    if (remaining != 0)
      break;
...
static ds4_dist_worker_forwarder *
dist_worker_get_forwarder(ds4_dist_worker_upstream *upstream, const char *host,
                          uint32_t port, char *err, size_t errlen) {
  pthread_mutex_lock(&upstream->forward_mu);
  for (ds4_dist_worker_forwarder *it = upstream->forwarders; it;
       it = it->next) {
  ...
  if (pthread_create(&forwarder->tid, NULL, dist_worker_forwarder_relay_main,
                     forwarder) != 0) {
    ...
    pthread_mutex_unlock(&upstream->forward_mu);
    if (errlen)
      snprintf(err, errlen, "failed to start worker-to-worker relay thread");
    return NULL;
  }
  ...
  fprintf(stderr,
          "ds4: distributed worker: opened pipelined worker-to-worker "
          "connection to %s:%u (window %u)\n",
          host, port, forwarder->pending_depth);
  return forwarder;
}
...
static void dist_worker_upstream_destroy(ds4_dist_worker_upstream *upstream) {
  ...
  while (forwarders) {
    ...
    if (forwarders->thread_started)
      pthread_join(forwarders->tid, NULL);
    if (forwarders->fd >= 0)
      close(forwarders->fd);
...
static int dist_forward_work_to_next(ds4_dist_worker_upstream *upstream,
                                     const ds4_dist_route_entry *next,
                                     const ds4_dist_work_fixed *work,
                                     const int *tokens, const float *hidden_hc,
                                     uint32_t hidden_hc_bytes,
                                     const ds4_dist_telemetry_fixed *telemetry,
                                     const void *route_blob) {
  char err[256];
  ds4_dist_worker_forwarder *forwarder = dist_worker_get_forwarder(
      upstream, next->host, next->port, err, sizeof(err));
  ...
  if (!dist_activation_wire_bytes_from_f32_bytes(forwarded.input_hc_bits,
    ...
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "invalid forwarded hidden-state size");
  }
  ...
  const double send_t0 = dist_now_sec();
  if (!dist_worker_forwarder_enqueue_request(forwarder, request_id, telemetry,
                                             send_t0)) {
    pthread_mutex_unlock(&forwarder->send_mu);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "out of memory tracking forwarded request");
  }
  DIST_DEBUG("forward send request=%llu to %s:%u route_index=%u tokens=%u "
             "pos=%u bytes=%u",
             (unsigned long long)request_id, next->host, next->port,
             forwarded.route_index, forwarded.n_tokens, forwarded.pos0,
             forwarded.input_hc_bytes);
  int rc = dist_send_work_frame(forwarder->fd, &forwarded, tokens, hidden_hc,
                                route_blob);
  const double send_t1 = dist_now_sec();
  dist_worker_forwarder_note_send_done(
      forwarder, request_id, dist_usec_since(send_t0, send_t1), send_t1);
  pthread_mutex_unlock(&forwarder->send_mu);
  if (rc != 0) {
    ...
    int err_rc = dist_worker_upstream_send_work_error(
        upstream, request_id, "failed to forward distributed work");
    shutdown(upstream->fd, SHUT_RDWR);
    return err_rc;
  }
...
static ds4_dist_worker_session *
dist_worker_get_session_locked(ds4_dist_worker_state *state,
                               uint64_t session_id, char *err, size_t errlen) {
  ...
  if (ds4_session_create(&entry->session, state->engine, state->ctx_size) !=
      0) {
    free(entry);
    if (errlen)
      snprintf(err, errlen, "failed to create distributed worker session");
    return NULL;
  }
...
static ds4_dist_worker_session *
dist_worker_find_session_locked(ds4_dist_worker_state *state,
                                uint64_t session_id) {
  for (ds4_dist_worker_session *it = state->sessions; it; it = it->next) {
    if (it->session_id == session_id)
      return it;
...
static int dist_temp_file(const char *prefix, char *path, size_t path_len,
                          FILE **fp_out) {
  char tmpl[PATH_MAX];
...
static int dist_worker_handle_snapshot_save(ds4_dist_worker_state *state,
  ...
  if (bytes != sizeof(req)) {
    ...
    pthread_mutex_lock(&upstream->write_mu);
    int rc = dist_send_snapshot_error(
        upstream->fd, 0, 0, state->model_id, state->layer_start,
        state->layer_end, "invalid distributed snapshot save request");
    pthread_mutex_unlock(&upstream->write_mu);
    return rc;
  }
  ...
  const uint64_t token_hash =
      dist_u64_from_halves(req.token_hash_hi, req.token_hash_lo);
  ...
  if (req.model_id != state->model_id ||
    ...
    ds4_dist_worker_session *session =
        dist_worker_find_session_locked(state, session_id);
    if (!session) {
      snprintf(err, sizeof(err),
      ...
      if (!timeline || timeline->len < 0 ||
          (uint32_t)timeline->len != req.token_count) {
        snprintf(err, sizeof(err), "worker snapshot token count mismatch");
      } else {
        live_hash =
            dist_token_hash_prefix(timeline->v, (uint32_t)timeline->len);
        if (live_hash != token_hash) {
          snprintf(err, sizeof(err), "worker snapshot token hash mismatch");
        }
      }
      if (!err[0] && dist_temp_file("ds4-dist-save", tmp_path, sizeof(tmp_path),
                                    &tmp) != 0) {
        snprintf(err, sizeof(err),
                 "failed to create worker snapshot temp file");
      }
      if (!err[0] && ds4_session_save_layer_payload(
                         session->session, tmp, state->layer_start,
                         state->layer_end, err, sizeof(err)) != 0) {
        if (!err[0])
          snprintf(err, sizeof(err), "failed to save worker KV shard");
      }
      ...
      if (!err[0]) {
        ...
        if (pos < 0)
          snprintf(err, sizeof(err), "failed to measure worker KV shard");
        else
          payload_bytes = (uint64_t)pos;
      }
  ...
  pthread_mutex_lock(&upstream->write_mu);
  if (err[0]) {
    rc = dist_send_snapshot_error(upstream->fd, request_id, session_id,
                                  state->model_id, state->layer_start,
                                  state->layer_end, err);
  } else {
    ds4_dist_snapshot_begin_fixed begin;
    memset(&begin, 0, sizeof(begin));
    ...
    if (rc > 0)
      rc = dist_send_snapshot_file_chunks(upstream->fd, request_id, tmp,
                                          payload_bytes);
    if (rc > 0)
      rc = dist_send_snapshot_done(upstream->fd, request_id, 0, NULL);
  }
  pthread_mutex_unlock(&upstream->write_mu);
...
static int dist_worker_handle_snapshot_load(ds4_dist_worker_state *state,
  ...
  if (expected_token_bytes > UINT32_MAX ||
      begin.token_bytes != (uint32_t)expected_token_bytes ||
      begin.message_bytes != 0 || body_bytes != begin.token_bytes) {
  ...
  if (!err[0] && (begin.model_id != state->model_id ||
                  begin.layer_start != state->layer_start ||
                  begin.layer_end != state->layer_end ||
  ...
  char tmp_path[PATH_MAX];
  if (!err[0] &&
      dist_temp_file("ds4-dist-load", tmp_path, sizeof(tmp_path), &tmp) != 0) {
    snprintf(err, sizeof(err),
             "failed to create worker snapshot restore temp file");
  }
  ...
  if (!err[0]) {
    ...
    if (!buf)
      snprintf(err, sizeof(err), "out of memory restoring worker KV shard");
  }
  uint64_t received = 0;
  while (!err[0] && received < payload_bytes) {
    uint32_t type = 0, chunk_frame_bytes = 0;
    rc = dist_read_frame_header(upstream->fd, &type, &chunk_frame_bytes, err,
                                sizeof(err));
    if (rc <= 0) {
      free(buf);
      free(tokens);
  ...
  if (!err[0]) {
    ...
    if (session &&
        ds4_session_load_layer_payload(
            session->session, tmp, payload_bytes, tokens, begin.token_count,
            state->layer_start, state->layer_end, err, sizeof(err)) == 0) {
      session->token_hash = token_hash;
      session->token_hash_valid = true;
    } else {
      if (!err[0])
        snprintf(err, sizeof(err), "failed to restore worker KV shard");
      if (session)
  ...
  rc = dist_send_snapshot_done(upstream->fd, request_id, err[0] ? 1u : 0u,
                               err[0] ? err : NULL);
  pthread_mutex_unlock(&upstream->write_mu);
  if (err[0] && received < payload_bytes)
    return -1;
  return rc;
}
...
static int dist_worker_process_work_payload(ds4_dist_worker_state *state,
                                            ds4_dist_worker_upstream *upstream,
                                            const void *payload,
                                            uint32_t bytes) {
  uint64_t request_id = 0;
  ...
  if (bytes < sizeof(ds4_dist_work_fixed)) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "truncated distributed WORK frame");
  }
  ...
  const uint64_t work_prefix_hash =
  ...
  const double total_t0 = profile ? dist_now_sec() : 0.0;
  DIST_DEBUG("worker work request=%llu layers=%u:%u tokens=%u pos=%u "
             "flags=0x%x token_bytes=%u input_hc=%u/%ub route_count=%u "
             "route_index=%u route_bytes=%u",
             (unsigned long long)request_id, work.layer_start, work.layer_end,
             work.n_tokens, work.pos0, work.flags, work.token_bytes,
             work.input_hc_bytes, work.input_hc_bits, work.route_count,
             work.route_index, work.route_bytes);
  ...
  if ((uint64_t)work.token_bytes != token_bytes_expected ||
    ...
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "invalid distributed WORK payload sizes");
  }
  if (work.route_count == 0) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "WORK frame is missing distributed route");
  }
  if (work.route_index >= work.route_count) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "invalid distributed WORK route metadata");
  }
  if (work.model_id != state->model_id) {
    snprintf(err, sizeof(err), "model id mismatch: work=%u worker=%u",
             work.model_id, state->model_id);
    return dist_worker_upstream_send_work_error(upstream, request_id, err);
  }
  if (work.layer_start != state->layer_start ||
      work.layer_end != state->layer_end) {
    snprintf(err, sizeof(err),
             "worker is assigned layers %u:%u but request asked for %u:%u",
             state->layer_start, state->layer_end, work.layer_start,
             work.layer_end);
    return dist_worker_upstream_send_work_error(upstream, request_id, err);
  }
  if ((work.flags & ~DS4_DIST_WORK_F_VALID_MASK) != 0) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "invalid distributed WORK flags");
  }
  if (work.n_tokens == 0) {
    return dist_worker_upstream_send_work_error(upstream, request_id,
                                                "WORK frame has no tokens");
  }
  if (work.pos0 > (uint32_t)state->ctx_size ||
      work.n_tokens > (uint32_t)state->ctx_size - work.pos0) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "WORK token span exceeds worker context");
  }
  ...
  if (!input_hc_present && work.layer_start != 0) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "nonzero layer WORK requires input hidden-state");
  }
  if (output_logits && !state->has_output) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "worker was not assigned the output head");
  }
  const uint32_t n_layers = (uint32_t)ds4_engine_layer_count(state->engine);
  if (output_logits && work.layer_end + 1u != n_layers) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "WORK logits require final transformer layer");
  }

  int *tokens = malloc((size_t)work.n_tokens * sizeof(tokens[0]));
  if (!tokens) {
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "out of memory reading WORK tokens");
  }
  for (uint32_t i = 0; i < work.n_tokens; i++) {
    uint32_t wire_token = 0;
  ...
  if (dist_token_hash_update_span(work_prefix_hash, tokens, work.n_tokens) !=
    ...
    free(tokens);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "WORK token prefix hash metadata mismatch");
  }
  ...
  if (expected_hc_bytes64 > UINT32_MAX) {
    free(tokens);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "distributed hidden-state payload is too large");
  }
  ...
  if (input_hc_present) {
    ...
    if (!dist_activation_bits_valid(input_hc_bits) ||
        !dist_activation_wire_bytes(input_hc_bits, expected_hc_values,
                                    &expected_hc_wire_bytes)) {
      free(tokens);
      return dist_worker_upstream_send_work_error(
          upstream, request_id, "invalid distributed activation width");
    }
    ...
    if (work.input_hc_bytes > reader.remaining) {
      DIST_DEBUG("worker input hidden read failed request=%llu rc=%d bytes=%u",
                 (unsigned long long)request_id, -1, work.input_hc_bytes);
      free(tokens);
      return -1;
    }
    ...
    DIST_DEBUG("worker input hidden read ok request=%llu bytes=%u bits=%u",
               (unsigned long long)request_id, work.input_hc_bytes,
               input_hc_bits);
  } else if (work.input_hc_bytes != 0) {
    free(tokens);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "WORK frame has hidden bytes without input flag");
  }
  ...
  if (has_route) {
    if (!dist_route_validate_blob(route_blob, work.route_bytes,
                                  work.route_count, n_layers, err,
                                  sizeof(err))) {
      free(route_blob);
      free(tokens);
      return dist_worker_upstream_send_work_error(upstream, request_id, err);
    ...
    if (!dist_route_get_entry(route_blob, work.route_bytes, work.route_count,
                              work.route_index, &current_route, err,
                              sizeof(err))) {
      free(route_blob);
      free(tokens);
      return dist_worker_upstream_send_work_error(upstream, request_id, err);
    ...
    if (current_route.layer_start != work.layer_start ||
      ...
      free(tokens);
      return dist_worker_upstream_send_work_error(
          upstream, request_id, "WORK layer range does not match route entry");
    }
    const bool route_output_logits =
        (current_route.flags & DS4_DIST_ROUTE_F_OUTPUT_LOGITS) != 0;
    if (route_output_logits != output_logits) {
      free(route_blob);
      free(tokens);
      return dist_worker_upstream_send_work_error(
          upstream, request_id, "WORK logits flag does not match route entry");
    }
    if (has_next && !dist_route_get_entry(
                        route_blob, work.route_bytes, work.route_count,
  ...
  if (has_next && output_logits) {
    ...
    free(tokens);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "non-final route entry requested logits");
  }
  if (has_route && !has_next) {
    ds4_dist_route_return ret;
    if (!dist_route_get_return_target(route_blob, work.route_bytes,
                                      work.route_count, &ret, err,
                                      sizeof(err))) {
      free(route_blob);
      free(tokens);
      return dist_worker_upstream_send_work_error(upstream, request_id, err);
    ...
    if (ret.kind != DS4_DIST_ROUTE_RETURN_UPSTREAM) {
      ...
      free(tokens);
      return dist_worker_upstream_send_work_error(
          upstream, request_id, "unsupported final result destination");
    }
  }
  ...
  const uint32_t result_kind =
      final_ack_only ? DS4_DIST_RESULT_ACK
  ...
  if (result_bytes && !result) {
    ...
    free(tokens);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "out of memory allocating distributed result");
  }

  const double decode_t0 = profile ? dist_now_sec() : 0.0;
  bool input_hc_uses_wire = false;
  uint32_t input_hc_decoded_bytes = 0;
  if (input_hc_present && dist_decode_activation_payload(
                              input_hc_wire, input_hc_bits, work.input_hc_bytes,
                              &input_hc, &input_hc_decoded_bytes,
                              &input_hc_uses_wire, err, sizeof(err)) != 0) {
    free(result);
    free(route_blob);
    free(tokens);
    return dist_worker_upstream_send_work_error(upstream, request_id, err);
  ...
  if ((work.flags & DS4_DIST_WORK_F_RESET_SESSION) != 0) {
    ...
    if (!timeline || timeline->len < 0) {
      ...
      free(tokens);
      return dist_worker_upstream_send_work_error(
          upstream, request_id, "worker session has no token timeline");
    }
    session->token_hash =
        dist_token_hash_prefix(timeline->v, (uint32_t)timeline->len);
    session->token_hash_valid = true;
  ...
  if (session->token_hash != work_prefix_hash) {
    ...
    free(tokens);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "worker KV prefix hash mismatch");
  }
  const double eval_t0 = dist_now_sec();
  int eval_rc = ds4_session_eval_layer_slice(
      session->session, tokens, work.n_tokens, work.pos0, work.layer_start,
      work.layer_end, input_hc, produce_hidden ? result : NULL,
      local_output_logits, local_output_logits ? result : NULL, err,
      sizeof(err));
  ...
  pthread_mutex_unlock(&state->mu);
  DIST_DEBUG("worker eval request=%llu layers=%u:%u tokens=%u pos=%u "
             "has_next=%d output=%d rc=%d",
             (unsigned long long)request_id, work.layer_start, work.layer_end,
             work.n_tokens, work.pos0, has_next ? 1 : 0,
             local_output_logits ? 1 : 0, eval_rc);
  ...
  if (result_kind == DS4_DIST_RESULT_HIDDEN_STATE &&
      !dist_activation_wire_bytes_from_f32_bytes(input_hc_bits, result_bytes,
                                                 &result_wire_bytes)) {
    if (!input_hc_uses_wire)
      free(input_hc);
    free(result);
    ...
    free(tokens);
    return dist_worker_upstream_send_work_error(
        upstream, request_id, "invalid output hidden-state size");
  }
  ...
  if (has_next) {
    send_rc =
        dist_forward_work_to_next(upstream, &next_route, &work, tokens, result,
                                  result_bytes, &telemetry, route_blob);
  } else {
    send_rc = dist_worker_upstream_send_work_result(
        upstream, request_id, work_result_hash, 0, result_kind,
        result_kind == DS4_DIST_RESULT_HIDDEN_STATE ? input_hc_bits : 32u,
        &telemetry, 1, result, result_bytes);
  }
  const double send_t1 = profile ? dist_now_sec() : 0.0;
  if (profile) {
    fprintf(stderr,
            "ds4: dist decode profile: worker request=%llu pos=%u layers=%u:%u "
            "input_decode=%.3fms lock_wait=%.3fms eval=%.3fms send=%.3fms "
            "total=%.3fms input=%.2fMiB output=%.2fMiB rc=%d\n",
            (unsigned long long)request_id, work.pos0, work.layer_start,
            work.layer_end, (decode_t1 - decode_t0) * 1000.0,
            (lock_t1 - lock_t0) * 1000.0, (eval_t1 - eval_t0) * 1000.0,
            (send_t1 - send_t0) * 1000.0, (send_t1 - total_t0) * 1000.0,
            (double)(work.token_bytes + work.input_hc_bytes) /
                (1024.0 * 1024.0),
            (double)result_wire_bytes / (1024.0 * 1024.0), send_rc);
  }
  DIST_DEBUG("worker send complete request=%llu has_next=%d send_rc=%d",
             (unsigned long long)request_id, has_next ? 1 : 0, send_rc);
  if (!input_hc_uses_wire)
    free(input_hc);
  free(result);
  free(route_blob);
...
static int dist_worker_handle_work(ds4_dist_worker_state *state,
  ...
  if (!payload) {
    dist_discard_bytes(upstream->fd, bytes);
    return dist_worker_upstream_send_work_error(
        upstream, 0, "out of memory reading distributed WORK frame");
  }
...
static void dist_worker_job_queue_init(ds4_dist_worker_job_queue *q,
                                       ds4_dist_worker_state *state,
                                       ds4_dist_worker_upstream *upstream) {
  memset(q, 0, sizeof(*q));
  q->state = state;
...
static bool dist_worker_job_queue_enqueue(ds4_dist_worker_job_queue *q,
...
static ds4_dist_worker_job *
dist_worker_job_queue_pop(ds4_dist_worker_job_queue *q) {
  pthread_mutex_lock(&q->mu);
...
static void *dist_worker_prefetch_eval_main(void *arg) {
  ...
  for (;;) {
    ...
    if (!job)
      break;
    int rc = dist_worker_process_work_payload(q->state, q->upstream,
                                              job->payload, job->bytes);
...
static int dist_worker_read_loop_prefetch(ds4_dist_worker_state *state,
                                          int fd) {
  ds4_dist_worker_upstream upstream;
  dist_worker_upstream_init(&upstream, state, fd);
  ...
  pthread_t eval_tid;
  if (pthread_create(&eval_tid, NULL, dist_worker_prefetch_eval_main, &queue) !=
      0) {
  ...
  for (;;) {
    ...
    if (type == DS4_DIST_MSG_WORK) {
      ...
      if (!job) {
        dist_discard_bytes(fd, bytes);
        dist_worker_upstream_send_work_error(
            &upstream, 0, "out of memory queueing distributed WORK");
        loop_rc = 1;
        break;
      }
  ...
  if (loop_rc == 0)
    dist_worker_job_queue_finish(&queue);
  else
    dist_worker_job_queue_cancel(&queue);
  pthread_join(eval_tid, NULL);
  if (loop_rc == 0 && queue.rc != 0)
    loop_rc = 1;
  dist_worker_job_queue_destroy(&queue);
  dist_worker_upstream_destroy(&upstream);
  return loop_rc;
...
static int dist_run_worker(ds4_engine *engine, const ds4_dist_options *opt,
                           int ctx_size) {
  char layer_end[32];
  if (opt->layers.has_output)
    snprintf(layer_end, sizeof(layer_end), "output");
  else
    snprintf(layer_end, sizeof(layer_end), "%u", opt->layers.end);

  char err[256];
  const char *listen_host = opt->listen_host;
  int requested_port = opt->listen_port > 0 ? opt->listen_port : 0;
  int listen_fd =
      dist_open_listener(listen_host, requested_port, err, sizeof(err));
  if (listen_fd < 0) {
  ...
  if (listen_port_i <= 0) {
    fprintf(
        stderr,
        "ds4: distributed worker: could not determine data listener port\n");
    close(listen_fd);
    return 1;
  }
  ...
  state.layer_end =
      dist_resolved_layer_end(opt, (uint32_t)ds4_engine_layer_count(engine));
  ...
  pthread_t data_tid;
  if (pthread_create(&data_tid, NULL, dist_worker_data_listener_main, &state) !=
      0) {
    fprintf(
        stderr,
  ...
  fprintf(stderr,
          "ds4: distributed worker: layers %u:%s model_id=%d data_listen=%s:%u "
          "connecting to coordinator %s:%d\n",
          opt->layers.start, layer_end, ds4_engine_model_id(engine),
          listen_host ? listen_host : "*", listen_port, opt->coordinator_host,
          opt->coordinator_port);

  for (;;) {
    int fd = dist_connect_endpoint(opt->coordinator_host, opt->coordinator_port,
                                   err, sizeof(err));
    if (fd < 0) {
    ...
    dist_peer_name(fd, peer_host, sizeof(peer_host), peer_port,
                   sizeof(peer_port));
    ...
    if (dropped_sessions) {
      fprintf(stderr,
              "ds4: distributed worker: cleared %u sessions after coordinator "
              "disconnect\n",
              dropped_sessions);
    }
    fprintf(
        stderr,
        "ds4: distributed worker: coordinator disconnected%s; reconnecting\n",
        rc ? " after error" : "");
    dist_sleep_reconnect();
  }
}
...
static bool dist_parse_u32_component(const char *p, size_t len, uint32_t *out) {
  ...
  if (errno != 0 || end == buf || *end != '\0' || v > UINT32_MAX)
    return false;
  *out = (uint32_t)v;
  return true;
}

static bool dist_parse_layers(const char *s, ds4_distributed_layers *out,
                              char *err, size_t errlen) {
  if (!s || !out) {
    if (errlen)
      snprintf(err, errlen, "missing layer range");
    return false;
  }
  ...
  if (!colon || colon == s || colon[1] == '\0') {
    if (errlen)
      snprintf(err, errlen, "expected A:B or A:output");
    return false;
  }
  if (strchr(colon + 1, ':')) {
    if (errlen)
      snprintf(err, errlen, "layer range has too many ':' separators");
    return false;
  }
  ...
  if (!dist_parse_u32_component(s, (size_t)(colon - s), &parsed.start)) {
    if (errlen)
      snprintf(err, errlen, "invalid start layer in %s", s);
    return false;
  }
  ...
  if (!strcmp(end, "output")) {
    ...
    if (!dist_parse_u32_component(end, strlen(end), &parsed.end)) {
      if (errlen)
        snprintf(err, errlen, "invalid end layer in %s", s);
      return false;
    }
    if (parsed.end < parsed.start) {
      if (errlen)
        snprintf(err, errlen, "layer range end precedes start in %s", s);
      return false;
    }
  }
...
static const char *dist_cli_need_arg(int *index, int argc, char **argv,
                                     const char *arg, char *err,
                                     size_t errlen) {
  if (!index || !argv || *index + 1 >= argc) {
    if (errlen)
      snprintf(err, errlen, "%s requires an argument", arg);
    return NULL;
  }
  return argv[++*index];
}

static bool dist_cli_parse_port(const char *s, const char *arg, int *out,
                                char *err, size_t errlen) {
  if (!s || !out) {
    if (errlen)
      snprintf(err, errlen, "%s requires a TCP port", arg);
    return false;
  }
  errno = 0;
  char *end = NULL;
  ...
  if (errno != 0 || s[0] == '\0' || *end != '\0' || v <= 0 || v > 65535) {
    if (errlen)
      snprintf(err, errlen, "invalid value for %s: %s", arg, s);
    return false;
  }
  *out = (int)v;
  return true;
...
void ds4_dist_options_free(ds4_dist_options *opt) { free(opt); }
...
void ds4_dist_usage(FILE *fp) {
  fprintf(fp,
          ...
          "  --listen HOST PORT\n"
          "      Coordinator TCP listen address. Workers may later use it to "
          "force their data listener.\n"
          "  --coordinator HOST PORT\n"
          "      Coordinator TCP address for --role worker.\n"
          "  --dist-prefill-chunk N\n"
          "      Coordinator prefill pipeline chunk size. Default: session "
          "cap, normally 4096.\n"
          "      Non-default values are experimental and can change logits "
          "unless validated.\n"
          "  --dist-prefill-window N\n"
          "      Coordinator max end-to-end prefill chunks in flight. Default: "
          "workers+2, capped at 8.\n"
          "  --dist-activation-bits N\n"
          "      Coordinator hidden-state transport width: 32, 16, or 8. "
          "Default: 32.\n"
          "  --dist-replay-check\n"
          "      Coordinator diagnostic: reset and replay the prompt, then "
          "compare logits.\n"
          "  --debug\n"
          "      Print coordinator route/debug logs. Workers keep their normal "
          "logs without this.\n");
}

ds4_dist_cli_parse_result ds4_dist_parse_cli_arg(const char *arg, int *index,
                                                 int argc, char **argv,
  ...
  if (!strcmp(arg, "--role")) {
    ...
    if (!opt || !dist_parse_role(role, &opt->role)) {
      if (errlen)
        snprintf(err, errlen,
                 "invalid distributed role: %s (valid roles: none, "
                 "coordinator, worker)",
                 role);
      return DS4_DIST_CLI_ERROR;
    }
  ...
  if (!strcmp(arg, "--layers")) {
    ...
    if (!opt) {
      if (errlen)
        snprintf(err, errlen, "missing distributed options");
      return DS4_DIST_CLI_ERROR;
    }
  ...
  if (!strcmp(arg, "--listen")) {
    ...
    if (opt->listen_host || opt->listen_port) {
      if (errlen)
        snprintf(err, errlen, "specify --listen only once");
      return DS4_DIST_CLI_ERROR;
    }
    ...
    if (!port)
      return DS4_DIST_CLI_ERROR;
    if (!dist_cli_parse_port(port, arg, &opt->listen_port, err, errlen))
      return DS4_DIST_CLI_ERROR;
  ...
  if (!strcmp(arg, "--coordinator")) {
    ...
    if (opt->coordinator_host || opt->coordinator_port) {
      if (errlen)
        snprintf(err, errlen, "specify --coordinator only once");
      return DS4_DIST_CLI_ERROR;
    }
    const char *host = dist_cli_need_arg(index, argc, argv, arg, err, errlen);
    if (!host)
    ...
    if (!port)
      return DS4_DIST_CLI_ERROR;
    if (!dist_cli_parse_port(port, arg, &opt->coordinator_port, err, errlen))
      return DS4_DIST_CLI_ERROR;
  ...
  if (!strcmp(arg, "--dist-prefill-chunk")) {
    ...
    if (!value)
      return DS4_DIST_CLI_ERROR;
    if (!dist_parse_positive_u32(value, arg, &opt->prefill_chunk, err,
                                 errlen)) {
      return DS4_DIST_CLI_ERROR;
    }
    return DS4_DIST_CLI_MATCHED;
  ...
  if (!strcmp(arg, "--dist-prefill-window")) {
    ...
    if (opt->prefill_window > 64u) {
      if (errlen)
        snprintf(err, errlen, "%s must be <= 64", arg);
      return DS4_DIST_CLI_ERROR;
    }
    return DS4_DIST_CLI_MATCHED;
  }
  if (!strcmp(arg, "--dist-activation-bits")) {
    ...
    if (!dist_activation_bits_valid(bits)) {
      if (errlen)
        snprintf(err, errlen, "%s must be 32, 16, or 8", arg);
      return DS4_DIST_CLI_ERROR;
    }
...
static int dist_validate_options(const ds4_dist_options *opt, char *err,
  ...
  if (!opt->layers.set) {
    if (errlen)
      snprintf(err, errlen, "--role %s requires --layers",
               dist_role_name(opt->role));
    return 1;
  }
  if (opt->prefill_window > 64u) {
    if (errlen)
      snprintf(err, errlen, "--dist-prefill-window must be <= 64");
    return 1;
  }
  if (opt->activation_bits != 0 &&
      !dist_activation_bits_valid(opt->activation_bits)) {
    if (errlen)
      snprintf(err, errlen, "--dist-activation-bits must be 32, 16, or 8");
    return 1;
  }
  ...
  if (opt->role == DS4_DISTRIBUTED_COORDINATOR) {
    if (!opt->listen_host || opt->listen_port <= 0) {
      if (errlen)
        snprintf(err, errlen, "--role coordinator requires --listen HOST PORT");
      return 1;
    }
    if (opt->coordinator_host || opt->coordinator_port) {
      if (errlen)
        snprintf(err, errlen, "--role coordinator must not use --coordinator");
      return 1;
    }
    return 0;
  }
  ...
  if (opt->role == DS4_DISTRIBUTED_WORKER) {
    if (!opt->coordinator_host || opt->coordinator_port <= 0) {
      if (errlen)
        snprintf(err, errlen, "--role worker requires --coordinator HOST PORT");
      return 1;
    }
    ...
    if (opt->prefill_window != 0) {
      if (errlen)
        snprintf(err, errlen,
                 "--dist-prefill-window requires --role coordinator");
      return 1;
    }
    if (opt->activation_bits != 0) {
      if (errlen)
        snprintf(err, errlen,
                 "--dist-activation-bits requires --role coordinator");
      return 1;
    }
    return 0;
  }
  ...
  if (errlen)
    snprintf(err, errlen, "invalid distributed role");
  return 1;
}

int ds4_dist_prepare_engine_options(const ds4_dist_options *opt,
                                    ds4_engine_options *engine, char *err,
                                    size_t errlen) {
  if (dist_validate_options(opt, err, errlen) != 0)
    return 1;
  if (opt && opt->replay_check && opt->role != DS4_DISTRIBUTED_COORDINATOR) {
    if (errlen)
      snprintf(err, errlen, "--dist-replay-check requires --role coordinator");
    return 1;
  }
  if (engine && opt) {
    ...
    if (ds4_dist_enabled(opt)) {
      ...
      engine->load_layer_end =
          opt->layers.has_output ? UINT32_MAX : opt->layers.end;
...
static int dist_validate_layers_for_model(const ds4_dist_options *opt,
                                          uint32_t n_layers, char *err,
                                          size_t errlen) {
  if (!opt || opt->role == DS4_DISTRIBUTED_NONE || !opt->layers.set)
    return 0;
  if (n_layers == 0) {
    if (errlen)
      snprintf(err, errlen, "model reports no layers");
    return 1;
  }
  ...
  if (opt->layers.start > last) {
    if (errlen)
      snprintf(err, errlen, "layer range starts past final model layer %u",
               last);
    return 1;
  }
  if (!opt->layers.has_output && opt->layers.end > last) {
    if (errlen)
      snprintf(err, errlen, "layer range ends past final model layer %u", last);
    return 1;
  }
  if (opt->role == DS4_DISTRIBUTED_COORDINATOR && opt->layers.start != 0) {
    if (errlen)
      snprintf(err, errlen, "coordinator layer range must start at layer 0");
    return 1;
  }
  return 0;
}
...
int ds4_dist_run(ds4_engine *engine, const ds4_dist_options *opt,
  ...
  if (!engine || !opt) {
    fprintf(stderr,
            "ds4: distributed runtime requires an open engine and options\n");
    return 1;
  }
  char err[256];
  if (dist_validate_options(opt, err, sizeof(err)) != 0 ||
      dist_validate_layers_for_model(opt,
                                     (uint32_t)ds4_engine_layer_count(engine),
                                     err, sizeof(err)) != 0) {
  ...
  fprintf(stderr,
          "ds4: distributed runtime requested without a distributed role\n");
  return 1;
}
