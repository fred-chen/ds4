...
load_i8x4_i32_aligned(const int8_t *p) {
...
__device__ __forceinline__ static int32_t
dot_i8_block(const int8_t *a, const int8_t *b, uint64_t n, int use_dp4a) {
  if (use_dp4a && n == 32u)
...
__global__ static DS4_ROCM_UNUSED void
matmul_q8_0_kernel(float *out, const unsigned char *w, const float *x,
...
matmul_q8_0_preq_kernel(float *out, const unsigned char *w, const int8_t *xq,
                        const float *xscale, uint64_t in_dim, uint64_t out_dim,
                        uint64_t n_tok, uint64_t blocks, int use_dp4a) {
  uint64_t row = (uint64_t)blockIdx.x;
  uint64_t tok = (uint64_t)blockIdx.y;
  if (row >= out_dim || tok >= n_tok)
    return;
...
__global__ static void matmul_q8_0_preq_warp8_kernel(
    ...
    uint64_t in_dim, uint64_t out_dim, uint64_t blocks, int use_dp4a) {
...
__global__ static void shared_gate_up_swiglu_q8_0_pair_preq_warp8_kernel(
    float *gate, float *up, float *mid, const unsigned char *wg,
    const unsigned char *wu, const int8_t *xq, const float *xscale,
    uint64_t in_dim, uint64_t out_dim, uint64_t blocks, int use_dp4a,
    int store_gate_up, float clamp) {
...
__global__ static void matmul_q8_0_hc_expand_preq_warp8_kernel(
    ...
    uint32_t n_embd, uint32_t n_hc, uint64_t blocks, int has_add,
    int use_dp4a) {
  const uint64_t row = (uint64_t)blockIdx.x * 8u + (threadIdx.x >> 5u);
  const uint32_t lane = threadIdx.x & 31u;
  if (row >= out_dim)
...
__global__ static void
matmul_q8_0_f32_small_block_w32_kernel(float *out, const unsigned char *w,
                                       const float *x, uint32_t n_blocks,
                                       uint64_t out_dim, uint64_t row_bytes) {
...
__global__ static void
matmul_q8_0_f32_warp8_kernel(float *out, const unsigned char *w, const float *x,
...
__global__ static void matmul_q8_0_f32_sharedx_warp_rows_w32_kernel(
    float *out, const unsigned char *w, const float *x, uint32_t n_blocks,
    uint64_t out_dim, uint64_t row_bytes) {
...
__global__ static void matmul_q8_0_f32_batch_warp8_kernel(
    float *out, const unsigned char *w, const float *x, uint64_t in_dim,
    uint64_t out_dim, uint64_t n_tok, uint64_t blocks) {
  const uint64_t row = (uint64_t)blockIdx.x * 8u + (threadIdx.x >> 5u);
  const uint64_t tok = (uint64_t)blockIdx.y;
  const uint32_t lane = threadIdx.x & 31u;
  if (row >= out_dim || tok >= n_tok)
...
__global__ static void shared_gate_up_swiglu_q8_0_batch_sharedx_w32_kernel(
    float *gate, float *up, float *mid, const unsigned char *wg,
    const unsigned char *wu, const float *x, uint32_t n_blocks,
    uint32_t out_dim, uint32_t n_tok, uint64_t row_bytes, int store_gate_up) {
  extern __shared__ float shx[];
  const uint32_t tid = threadIdx.x;
  const uint32_t lane = tid & 31u;
  const uint32_t wave = tid >> 5u;
...
template <uint32_t TOK_TILE, uint32_t BLOCKS_TILE>
__global__ static void
matmul_q8_0_f32_batch_sharedx_warp_rows_w32_toktile_kernel(
    float *out, const unsigned char *w, const float *x, uint32_t n_blocks,
    uint32_t out_dim, uint32_t n_tok, uint64_t row_bytes) {
  extern __shared__ float shx[];
  const uint32_t tid = threadIdx.x;
  const uint32_t lane = tid & 31u;
  const uint32_t wave = tid >> 5u;
...
#pragma unroll
    for (uint32_t u = 0; u < TOK_TILE; u++) {
      const uint32_t t = t0 + u;
      if (t < n_tok)
        out[(uint64_t)t * out_dim + row] = acc[u];
    }
  }
}
 ...
 *
 * gfx1201 (RDNA 4) deprecates the legacy WMMA intrinsics; this kernel is
 * disabled on that target.  The batched Q8_0 matmul falls back to the shared-X
 * path instead. */
#if !defined(__gfx1201__)
__launch_bounds__(128, 2) __global__
    static void matmul_q8_0_f32_batch_wmma_4w_kernel(
        float *out, const unsigned char *w, const float *x, uint32_t n_tokens,
  ...
  if (block_m >= out_dim || block_n >= n_tokens)
    return;
...
#pragma unroll
  for (uint32_t ntile = 0; ntile < N_TILES_PER_WARP; ntile++) {
    ...
    if (tok >= n_tokens)
      continue;
    ds4_q8_float8_t acc =
        ntile == 0u ? acc0 : (ntile == 1u ? acc1 : (ntile == 2u ? acc2 : acc3));
#pragma unroll
    for (uint32_t j = 0; j < 8u; j++) {
      const uint32_t row = warp_m + 2u * j + (lane >> 4u);
      if (row < out_dim)
        out[(uint64_t)tok * out_dim + row] = acc[j];
    }
  }
}
#endif /* !defined(__gfx1201__) */
...
__global__ static void matmul_q8_0_f32_batch_wmma_onthefly_kernel(
  ...
  using frag_a = rocwmma::fragment<rocwmma::matrix_a, BM, BN, BK, half,
                                   rocwmma::row_major>;
  using frag_b = rocwmma::fragment<rocwmma::matrix_b, BM, BN, BK, half,
                                   rocwmma::row_major>;
    ...
    for (uint32_t j = tid; j < TILES_N * BK * BN; j += blockDim.x) {
      ...
      if (row < out_dim && k < in_dim) {
        const unsigned char *blk =
            w + (uint64_t)row * row_bytes + (uint64_t)(k >> 5u) * 34u;
  ...
  if (wave < TILES_N)
    rocwmma::store_matrix_sync(shC + wave * BM * BN, acc, BN,
  ...
  for (uint32_t j = tid; j < TILES_N * BM * BN; j += blockDim.x) {
    ...
    const uint32_t t = t0 + m;
    const uint32_t row = row0 + tn * BN + nn;
    if (t < n_tokens && row < out_dim)
      out[(uint64_t)t * out_dim + row] = shC[j];
  }
}
#endif
...
__global__ static void matmul_q8_0_pair_f32_warp8_kernel(
    float *out0, float *out1, const unsigned char *w0, const unsigned char *w1,
    const float *x, uint64_t in_dim, uint64_t out0_dim, uint64_t out1_dim,
    uint64_t blocks) {
  const uint64_t row = (uint64_t)blockIdx.x * 8u + (threadIdx.x >> 5u);
  const uint32_t lane = threadIdx.x & 31u;
  if (row >= out0_dim && row >= out1_dim)
...
__global__ static void matmul_q8_0_pair_f32_sharedx_warp_rows_w32_kernel(
    float *out0, float *out1, const unsigned char *w0, const unsigned char *w1,
    const float *x, uint32_t n_blocks, uint64_t out0_dim, uint64_t out1_dim,
    uint64_t row_bytes) {
  extern __shared__ float shx[];
  const uint32_t tid = threadIdx.x;
  const uint32_t lane = tid & 31u;
...
__global__ static void shared_gate_up_swiglu_q8_0_rows_w32_kernel(
    float *gate, float *up, float *mid, const unsigned char *wg,
    const unsigned char *wu, const float *x, uint32_t n_blocks,
    uint64_t out_dim, uint64_t row_bytes, int store_gate_up, float clamp) {
  const uint32_t tid = threadIdx.x;
  const uint32_t lane = tid & 31u;
  const uint32_t wave = tid >> 5u;
  const uint32_t rows_per_block = blockDim.x >> 5u;
...
__global__ static void matmul_q8_0_hc_expand_f32_warp8_kernel(
    float *out_hc, float *block_out, const float *block_add,
    const float *residual_hc, const float *split, const unsigned char *w,
    const float *x, uint64_t in_dim, uint64_t out_dim, uint32_t n_embd,
    uint32_t n_hc, uint64_t blocks, int has_add) {
  const uint64_t row = (uint64_t)blockIdx.x * 8u + (threadIdx.x >> 5u);
  const uint32_t lane = threadIdx.x & 31u;
  if (row >= out_dim)
    return;
...
__global__ static void matmul_q8_0_hc_expand_f32_sharedx_warp_rows_w32_kernel(
    float *out_hc, float *block_out, const float *block_add,
    const float *residual_hc, const float *split, const unsigned char *w,
    const float *x, uint32_t n_blocks, uint64_t out_dim, uint64_t row_bytes,
...
q8_0_scale_broadcast_oldhip_w32(const unsigned char *blk) {
  float d = 0.0f;
  if ((threadIdx.x & 31u) == 0u)
    d = q8_0_scale_scalar(blk);
#if defined(__HIP_PLATFORM_AMD__) || defined(__HIPCC__)
...
__global__ static void
matmul_q8_0_hc_partial16_w32_kernel(float *partial, const unsigned char *w,
  ...
  for (uint32_t i = tid; i < 512u; i += blockDim.x)
    shx[i] = x[((uint64_t)b0 << 5) + i];
  __syncthreads();
...
#pragma unroll
  ...
  if (lane == 0u)
    partial[(uint64_t)split * out_dim + row] = acc;
}

__global__ static void matmul_q8_0_hc_partial_w32_kernel(
    float *partial, const unsigned char *w, const float *x, uint32_t n_blocks,
    uint32_t out_dim, uint64_t row_bytes, uint32_t n_splits) {
  extern __shared__ float shx[];
  const uint32_t tid = threadIdx.x;
  const uint32_t lane = tid & 31u;
  const uint32_t wave = tid >> 5;
  ...
  for (uint32_t i = tid; i < (chunk_blocks << 5); i += blockDim.x)
    shx[i] = x[((uint64_t)b0 << 5) + i];
  __syncthreads();
...
__global__ static void
hc_expand_partial_kernel(float *out_hc, float *block_out, const float *partial,
                         const float *residual_hc, const float *split,
                         uint32_t out_dim, uint32_t n_hc, uint32_t n_splits,
                         int store_block_out) {
  ...
  for (uint32_t s = 0; s < n_splits; s++)
    acc += partial[(uint64_t)s * out_dim + row];
  if (store_block_out)
    block_out[row] = acc;
  const float *post = split + n_hc;
  const float *comb = split + 2u * n_hc;
  for (uint32_t dst = 0; dst < n_hc; dst++) {
    ...
    for (uint32_t src = 0; src < n_hc; src++) {
      v += comb[dst + (uint64_t)src * n_hc] *
           residual_hc[(uint64_t)src * out_dim + row];
    }
...
__global__ static void hc_expand_add_partial_kernel(
    float *out_hc, float *block_out, const float *partial,
    const float *block_add, const float *residual_hc, const float *split,
    uint32_t out_dim, uint32_t n_hc, uint32_t n_splits, int store_block_out) {
  const uint32_t row = blockIdx.x * blockDim.x + threadIdx.x;
  if (row >= out_dim)
    return;
  float acc = 0.0f;
...
__global__ static void hc_expand_add_partial4_kernel(
    float *out_hc, float *block_out, const float *partial,
    const float *block_add, const float *residual_hc, const float *split,
    uint32_t out_dim, uint32_t n_hc, int store_block_out) {
  const uint32_t row = blockIdx.x * blockDim.x + threadIdx.x;
  if (row >= out_dim)
    return;
  float acc = 0.0f;
...
__global__ static void
hc_expand_partial16_kernel(float *out_hc, float *block_out,
                           const float *partial, const float *residual_hc,
                           const float *split, uint32_t out_dim, uint32_t n_hc,
                           int store_block_out) {
  const uint32_t row = blockIdx.x * blockDim.x + threadIdx.x;
  if (row >= out_dim)
    return;
...
__global__ static void grouped_q8_0_a_f32_sharedx_rows_w32_2row_kernel(
    float *low, const unsigned char *w, const float *heads, uint32_t n_groups,
    uint32_t n_blocks, uint64_t rank, uint64_t row_bytes) {
  extern __shared__ float shx[];
  const uint32_t tid = threadIdx.x;
  const uint32_t lane = tid & 31u;
  const uint32_t wave = tid >> 5u;
  ...
  for (uint32_t i = tid; i < group_dim; i += blockDim.x)
    shx[i] = x[i];
  __syncthreads();
...
__global__ static void
grouped_q8_0_a_partial16_w32_kernel(float *partial, const unsigned char *w,
                                    const float *heads, uint32_t n_groups,
  ...
  const unsigned char *wr =
      w + (uint64_t)((uint64_t)g * rank + row) * row_bytes;
  float acc = 0.0f;
#pragma unroll
  for (uint32_t bb = 0; bb < 16u; bb++) {
    const uint32_t b = b0 + bb;
  ...
  acc = warp_sum_f32_oldhip_w32(acc);
  if (lane == 0u)
    partial[(uint64_t)split * total + idx] = acc;
}

__global__ static void q8_partial_sum8_kernel(float *out, const float *partial,
                                              uint32_t out_dim) {
  const uint32_t row = blockIdx.x * blockDim.x + threadIdx.x;
  if (row >= out_dim)
    return;
...
__global__ static void grouped_q8_0_a_f32_batch_wmma_onthefly_kernel(
    float *low, const unsigned char *w, const float *heads, uint32_t n_tokens,
    uint32_t n_groups, uint32_t group_dim, uint32_t rank, uint64_t row_bytes) {
  extern __shared__ unsigned char raw_sh[];
  half *shA = reinterpret_cast<half *>(raw_sh);
  half *shB = shA + BM * BK;
  float *shC = reinterpret_cast<float *>(shB + TILES_N * BK * BN);
  const uint32_t tid = threadIdx.x;
  const uint32_t wave = tid >> 5u;
  const uint32_t row_tiles_per_group =
      (rank + TILES_N * BN - 1u) / (TILES_N * BN);
  ...
  for (uint32_t k0 = 0; k0 < group_dim; k0 += BK) {
    for (uint32_t j = tid; j < BM * BK; j += blockDim.x) {
      ...
      shA[j] = (t < n_tokens && k < group_dim)
                   ? __float2half(
                         heads[((uint64_t)t * n_groups + g) * group_dim + k])
                   : __float2half(0.0f);
    }
    for (uint32_t j = tid; j < TILES_N * BK * BN; j += blockDim.x) {
      const uint32_t tn = j / (BK * BN);
