...
struct cuda_stream_resident_key {
  ...
  bool operator==(const cuda_stream_resident_key &o) const {
    return model_map == o.model_map && layer == o.layer && expert == o.expert &&
           gate_offset == o.gate_offset && up_offset == o.up_offset &&
...
struct cuda_stream_resident_key_hash {
  size_t operator()(const cuda_stream_resident_key &k) const {
    ...
    h ^= (uint64_t)(uint32_t)k.expert + 0x9e3779b97f4a7c15ull + (h << 6) +
         (h >> 2);
...
static int cuda_u64_mul3_checked(uint64_t a, uint64_t b, uint64_t c,
                                 uint64_t *out) {
  uint64_t tmp = 0;
...
static int cuda_model_range_fits(uint64_t model_size, uint64_t offset,
                                 uint64_t bytes) {
...
static int cuda_tensor_has_elems(const ds4_gpu_tensor *t, uint64_t elems,
                                 uint64_t elem_size) {
  uint64_t bytes = 0;
  return cuda_u64_mul_checked(elems, elem_size, &bytes) &&
         cuda_tensor_has_bytes(t, bytes);
}

static int cuda_tensor_has_elems2(const ds4_gpu_tensor *t, uint64_t a,
                                  uint64_t b, uint64_t elem_size) {
  uint64_t bytes = 0;
  return cuda_u64_mul3_checked(a, b, elem_size, &bytes) &&
         cuda_tensor_has_bytes(t, bytes);
}

static int cuda_tensor_has_elems3(const ds4_gpu_tensor *t, uint64_t a,
                                  uint64_t b, uint64_t c, uint64_t elem_size) {
...
dequant_q8_0_to_f16_kernel(__half *out, const unsigned char *w, uint64_t in_dim,
                           uint64_t out_dim, uint64_t blocks);
__global__ static void
dequant_q8_0_to_f32_kernel(float *out, const unsigned char *w, uint64_t in_dim,
                           uint64_t out_dim, uint64_t blocks);
__global__ static void
dequant_q8_0_to_f16_transpose_kernel(__half *out, const unsigned char *w,
...
static void *cuda_tmp_alloc(uint64_t bytes, const char *what) {
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "temp alloc failed for %s (%.2f MiB): %s\n",
...
static int cuda_attention_score_buffer_fits(uint32_t n_comp) {
  return n_comp <=
         DS4_ROCM_ATTENTION_SCORE_CAP - DS4_ROCM_ATTENTION_RAW_SCORE_CAP;
}
...
static int cuda_model_image_find(const void *model_map) {
  ...
  for (size_t i = 0; i < g_model_images.size(); i++) {
    if (g_model_images[i].host_base == model_map)
      return (int)i;
  }
  return -1;
}

static const char *cuda_model_image_ptr(const void *model_map,
                                        uint64_t offset) {
...
static void cuda_stream_batch_selected_cache_release(void) {
  ...
  memset(&g_stream_batch_selected_cache, 0,
         sizeof(g_stream_batch_selected_cache));
}

static void cuda_stream_selected_cache_release(void) {
  (void)cuda_stream_selected_reuse_wait("streaming selected cache release");
  if (g_stream_selected_cache.gate)
    (void)cudaFree(g_stream_selected_cache.gate);
  if (g_stream_selected_cache.up)
    (void)cudaFree(g_stream_selected_cache.up);
  if (g_stream_selected_cache.down)
    (void)cudaFree(g_stream_selected_cache.down);
  if (g_stream_selected_cache.slot_ids)
    (void)cudaFree(g_stream_selected_cache.slot_ids);
  if (g_stream_selected_cache.gate_ptrs)
    (void)cudaFree(g_stream_selected_cache.gate_ptrs);
  if (g_stream_selected_cache.up_ptrs)
    (void)cudaFree(g_stream_selected_cache.up_ptrs);
  if (g_stream_selected_cache.down_ptrs)
    (void)cudaFree(g_stream_selected_cache.down_ptrs);
...
static int cuda_stream_selected_ensure_stream(void) {
  if (g_model_upload_stream)
    return 1;
  cudaError_t err =
      cudaStreamCreateWithFlags(&g_model_upload_stream, cudaStreamNonBlocking);
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "streaming selected upload stream creation failed: %s\n",
            cudaGetErrorString(err));
    (void)cudaGetLastError();
    return 0;
  }
...
static int cuda_stream_selected_reuse_ensure_event(void) {
  if (g_stream_selected_reuse_event)
    return 1;
  cudaError_t err = cudaEventCreateWithFlags(&g_stream_selected_reuse_event,
                                             cudaEventDisableTiming);
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "streaming selected reuse event creation failed: %s\n",
            cudaGetErrorString(err));
    (void)cudaGetLastError();
    return 0;
  }
...
static int cuda_stream_selected_reuse_wait(const char *what) {
  if (!g_stream_selected_reuse_event_pending)
    return 1;
...
static int cuda_stream_selected_mark_inflight(void) {
  if (!g_ssd_streaming_mode)
    return 1;
  if (!cuda_stream_selected_reuse_ensure_event())
    return 0;
  cudaError_t err = cudaEventRecord(g_stream_selected_reuse_event, 0);
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "streaming selected reuse event record failed: %s\n",
            cudaGetErrorString(err));
    (void)cudaGetLastError();
    return 0;
  }
...
static int cuda_stream_selected_ensure_buffers(uint64_t gate_bytes,
                                               uint64_t down_bytes) {
  ...
  if (g_stream_selected_cache.gate_capacity < gate_bytes) {
    ...
    err =
        cudaMalloc((void **)&g_stream_selected_cache.gate, (size_t)gate_bytes);
    if (err == cudaSuccess) {
      err =
          cudaMalloc((void **)&g_stream_selected_cache.up, (size_t)gate_bytes);
    }
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected gate/up alloc failed (%.2f MiB): %s\n",
              (double)gate_bytes / 1048576.0, cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  if (g_stream_selected_cache.down_capacity < down_bytes) {
    ...
    err =
        cudaMalloc((void **)&g_stream_selected_cache.down, (size_t)down_bytes);
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected down alloc failed (%.2f MiB): %s\n",
              (double)down_bytes / 1048576.0, cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  if (!g_stream_selected_cache.slot_ids) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected slot-id alloc failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
    int32_t slots[DS4_ROCM_N_EXPERT_USED];
    for (uint32_t i = 0; i < DS4_ROCM_N_EXPERT_USED; i++)
      slots[i] = (int32_t)i;
    err = cudaMemcpy(g_stream_selected_cache.slot_ids, slots, sizeof(slots),
                     cudaMemcpyHostToDevice);
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected slot-id upload failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  if (!g_stream_selected_cache.gate_ptrs) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected pointer table alloc failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
...
static int cuda_stream_batch_selected_ensure_buffers(uint64_t n_ids,
                                                     uint32_t n_unique) {
  if (n_ids == 0 || n_unique == 0)
    return 0;
  cudaError_t err = cudaSuccess;
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "streaming batch selected-id alloc failed "
                                 "(%.2f MiB): %s\n",
              (double)selected_bytes / 1048576.0, cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  if (g_stream_batch_selected_cache.pair_missing_capacity < n_ids) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming batch selected split-flag alloc failed "
              "(%.2f MiB): %s\n",
              (double)n_ids / 1048576.0, cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  if (g_stream_batch_selected_cache.ptr_capacity < n_unique) {
    ...
    if (err == cudaSuccess) {
      err =
          cudaMalloc((void **)&g_stream_batch_selected_cache.resident_gate_ptrs,
                     (size_t)n_unique * sizeof(char *));
    }
    if (err == cudaSuccess) {
    ...
    if (err == cudaSuccess) {
      err =
          cudaMalloc((void **)&g_stream_batch_selected_cache.missing_gate_ptrs,
                     (size_t)n_unique * sizeof(char *));
    }
    if (err == cudaSuccess) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "streaming batch pointer-table alloc failed "
                                 "(unique=%u): %s\n",
              n_unique, cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
...
static int cuda_stream_batch_selected_cache_apply(
    const void *model_map, uint32_t layer, uint32_t n_total_expert,
    uint32_t n_selected, uint32_t n_tokens, uint64_t gate_offset,
    uint64_t up_offset, uint64_t down_offset, uint64_t gate_expert_bytes,
    uint64_t down_expert_bytes, const ds4_gpu_tensor **selected_exec,
    const char ***gate_ptrs, const char ***up_ptrs, const char ***down_ptrs,
    uint32_t *unique_out) {
...
static int cuda_stream_selected_is_current(const cuda_stream_resident_expert &e,
                                           uint32_t layer,
                                           const int32_t *selected_ids,
                                           uint32_t n_selected) {
  if (!selected_ids || e.layer != layer)
    return 0;
  for (uint32_t i = 0; i < n_selected; i++) {
    if (e.expert == selected_ids[i])
      return 1;
  }
  return 0;
}
...
static cuda_stream_resident_key cuda_stream_resident_make_key(
    const void *model_map, uint32_t layer, int32_t expert, uint64_t gate_offset,
    uint64_t up_offset, uint64_t down_offset, uint64_t gate_expert_bytes,
    uint64_t down_expert_bytes) {
...
static cuda_stream_resident_key
cuda_stream_resident_entry_key(const cuda_stream_resident_expert &e) {
  return cuda_stream_resident_make_key(
      e.model_map, e.layer, e.expert, e.gate_offset, e.up_offset, e.down_offset,
      e.gate_expert_bytes, e.down_expert_bytes);
}

static int cuda_stream_resident_find(const void *model_map, uint32_t layer,
...
static int cuda_stream_resident_evict_one(uint32_t layer,
                                          const int32_t *selected_ids,
                                          uint32_t n_selected) {
...
static int cuda_stream_resident_make_room(uint64_t bytes, uint32_t layer,
                                          const int32_t *selected_ids,
                                          uint32_t n_selected) {
...
static int cuda_stream_resident_alloc(const void *model_map, uint32_t layer,
                                      int32_t expert,
                                      const int32_t *selected_ids,
                                      uint32_t n_selected, uint64_t gate_offset,
                                      uint64_t up_offset, uint64_t down_offset,
                                      uint64_t gate_expert_bytes,
                                      uint64_t down_expert_bytes) {
  ...
  if (!cuda_stream_resident_make_room(bytes, layer, selected_ids, n_selected)) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "streaming expert cache cannot keep %.2f MiB "
  ...
  while (err != cudaSuccess &&
         cuda_stream_resident_evict_one(layer, selected_ids, n_selected)) {
    (void)cudaGetLastError();
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            ...
            layer, expert, (double)bytes / 1048576.0, cudaGetErrorString(err));
    (void)cudaGetLastError();
    return -1;
  }
...
static int cuda_stream_read_job_upload(cuda_stream_read_job *job,
  ...
  cudaError_t err = cudaMemcpyAsync(job->dst, job->host_buf, (size_t)job->bytes,
  ...
  for (;;) {
    ...
    if (job->ok) {
      (void)cuda_stream_read_job_upload(
          job, worker_id < DS4_ROCM_STREAM_READ_WORKERS
                   ? g_stream_read_upload_streams[worker_id]
                   : NULL);
    }
...
static int cuda_stream_read_upload_streams_ensure(void) {
  for (uint32_t i = 0; i < DS4_ROCM_STREAM_READ_WORKERS; i++) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming read upload stream creation failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
...
static int cuda_stream_read_pool_ensure(void) {
  ...
  for (uint32_t i = 0; i < DS4_ROCM_STREAM_READ_WORKERS; i++) {
    ...
    const int rc =
        pthread_create(&g_stream_read_threads[i], NULL, cuda_stream_read_worker,
...
static int cuda_stream_read_jobs_prepare(cuda_stream_read_job *jobs,
  ...
  if (!jobs || count == 0)
    return 1;
  if (count > DS4_ROCM_STREAM_READ_MAX_JOBS)
    return 0;
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    if (jobs[i].bytes > max_bytes)
      max_bytes = jobs[i].bytes;
  }
  ...
  for (uint32_t i = 0; i < DS4_ROCM_STREAM_READ_WORKERS; i++) {
    if (g_stream_read_stage_bytes[i] < max_bytes) {
      ...
      cudaError_t err =
          cudaMallocHost(&g_stream_read_stage_raw[i], (size_t)max_bytes);
      if (err != cudaSuccess) {
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX "streaming read pinned allocation failed "
                                   "(%.2f MiB): %s\n",
                (double)max_bytes / 1048576.0, cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
      }
...
static int cuda_stream_read_jobs_start(cuda_stream_read_job *jobs,
  ...
  if (!cuda_stream_read_jobs_prepare(jobs, count))
    return 0;
  if (!cuda_stream_read_pool_ensure())
    return 0;
  pthread_mutex_lock(&g_stream_read_mutex);
  if (g_stream_read_active_jobs != NULL) {
    pthread_mutex_unlock(&g_stream_read_mutex);
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "streaming read pool already has active work\n");
    return 0;
  }
...
static int cuda_stream_read_jobs_wait(cuda_stream_read_job *jobs,
  ...
  if (g_stream_read_active_jobs != jobs) {
    ...
    fprintf(stderr, DS4_GPU_LOG_PREFIX
            "streaming read wait received inactive job set\n");
    return 0;
  }
...
static void cuda_stream_read_jobs_free(cuda_stream_read_job *jobs,
                                       uint32_t count) {
  if (!jobs)
    return;
  for (uint32_t i = 0; i < count; i++) {
...
static int cuda_stream_selected_upload_read_jobs(cuda_stream_read_job *jobs,
                                                 uint32_t count);
...
static int cuda_stream_batch_selected_pending_matches(
  ...
  return g_stream_batch_selected_pending.active &&
         ...
         g_stream_batch_selected_pending.gate_expert_bytes ==
             gate_expert_bytes &&
...
static int cuda_stream_batch_selected_apply_split(
    ...
    uint64_t up_offset, uint64_t down_offset, uint64_t gate_expert_bytes,
    uint64_t down_expert_bytes, const ds4_gpu_tensor **selected_exec,
    const char ***resident_gate_ptrs, const char ***resident_up_ptrs,
    const char ***missing_gate_ptrs, const char ***missing_up_ptrs,
    const char ***down_ptrs, const uint8_t **pair_missing,
    uint32_t *resident_count, uint32_t *missing_count, uint32_t *unique_out) {
  if (!selected_exec || !resident_gate_ptrs || !resident_up_ptrs ||
      !missing_gate_ptrs || !missing_up_ptrs || !down_ptrs || !pair_missing ||
      !resident_count || !missing_count || !unique_out ||
      !cuda_stream_batch_selected_pending_matches(
          model_map, layer, n_total_expert, n_selected, n_tokens, gate_offset,
          up_offset, down_offset, gate_expert_bytes, down_expert_bytes) ||
      !g_stream_batch_selected_cache.selected_ids ||
...
static int cuda_stream_batch_selected_finish_pending_missing(void) {
  ...
  if (!cuda_stream_read_jobs_wait(g_stream_batch_selected_pending.read_jobs,
      ...
      !cuda_stream_selected_upload_read_jobs(
          g_stream_batch_selected_pending.read_jobs, read_job_count)) {
...
static int cuda_stream_selected_upload_read_jobs(cuda_stream_read_job *jobs,
  ...
  if (!need_upload)
    return 1;
  if (!cuda_stream_selected_ensure_stream())
    return 0;
  for (uint32_t i = 0; i < count; i++) {
    if (jobs[i].uploaded)
      continue;
    cudaError_t err =
        cudaMemcpyAsync(jobs[i].dst, jobs[i].host_buf, (size_t)jobs[i].bytes,
                        cudaMemcpyHostToDevice, g_model_upload_stream);
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected cached upload failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "streaming selected upload sync failed: %s\n",
            cudaGetErrorString(err));
    (void)cudaGetLastError();
    return 0;
  }
...
static int cuda_stream_flush_read_jobs(cuda_stream_read_job *jobs,
                                       uint32_t *count) {
  if (!count || *count == 0)
    return 1;
...
static int cuda_stream_layer_expert_cache_apply(
    ...
    uint64_t gate_expert_bytes, uint64_t down_expert_bytes, const char **gate_w,
    const char **up_w, const char **down_w) {
  if (!g_ssd_streaming_mode || !gate_w || !up_w || !down_w)
    return 0;
  for (uint32_t i = 0; i < 2u; i++) {
    const cuda_stream_layer_expert_cache &c = g_stream_layer_expert_cache[i];
    if (c.active && c.model_map == model_map && c.layer == layer &&
        c.n_total_expert == n_total_expert && c.gate_offset == gate_offset &&
        c.up_offset == up_offset && c.down_offset == down_offset &&
        c.gate_expert_bytes == gate_expert_bytes &&
        c.down_expert_bytes == down_expert_bytes && c.gate && c.up && c.down) {
      *gate_w = c.gate;
      *up_w = c.up;
      *down_w = c.down;
      return 1;
...
static int cuda_stream_layer_expert_cache_load(
    ...
    uint64_t down_expert_bytes) {
  if (!g_ssd_streaming_mode || !model_map || model_size == 0 ||
      n_total_expert == 0 ||
      n_total_expert > DS4_ROCM_STREAM_READ_MAX_JOBS / 3u ||
      gate_expert_bytes == 0 || down_expert_bytes == 0 || g_model_fd < 0 ||
  ...
  if (gate_offset > model_size || up_offset > model_size ||
      down_offset > model_size || gate_bytes > model_size - gate_offset ||
  ...
  if (slot.capacity < total_bytes) {
    ...
    if (cuda_stream_cache_stats_on() && !g_stream_resident_experts.empty()) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              ...
              layer, (double)total_bytes / 1073741824.0,
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  const uint64_t gate_chunks = (gate_bytes + read_chunk - 1u) / read_chunk;
  const uint64_t down_chunks = (down_bytes + read_chunk - 1u) / read_chunk;
  ...
  for (uint64_t off = 0; off < gate_bytes; off += read_chunk) {
    const uint64_t n =
        gate_bytes - off < read_chunk ? gate_bytes - off : read_chunk;
  ...
  for (uint64_t off = 0; off < down_bytes; off += read_chunk) {
    const uint64_t n =
        down_bytes - off < read_chunk ? down_bytes - off : read_chunk;
...
static void cuda_stream_selected_cache_header(
    const void *model_map, uint32_t layer, uint32_t n_total_expert,
    uint32_t n_selected, const int32_t *selected_ids,
...
static int cuda_stream_selected_compact_mask(
    const void *model_map, uint32_t layer, const int32_t *selected_ids,
    uint32_t n_total_expert, uint32_t n_selected, uint64_t gate_offset,
    uint64_t up_offset, uint64_t down_offset, uint64_t gate_expert_bytes,
    uint64_t down_expert_bytes, uint32_t mask) {
  (void)n_total_expert;
  if (mask == 0)
    return 1;
  if (!selected_ids || !cuda_stream_selected_ensure_stream())
    return 0;
  cudaError_t err = cudaSuccess;
  for (uint32_t i = 0; i < n_selected; i++) {
    if ((mask & (1u << i)) == 0)
      continue;
    int idx = cuda_stream_resident_find(model_map, layer, selected_ids[i],
                                        gate_offset, up_offset, down_offset,
                                        gate_expert_bytes, down_expert_bytes);
    if (idx < 0) {
      fprintf(stderr, DS4_GPU_LOG_PREFIX
              "streaming selected resident expert missing during compact\n");
      return 0;
    }
    ...
    err = cudaMemcpyAsync(g_stream_selected_cache.gate +
                          ...
                          entry.gate, (size_t)gate_expert_bytes,
                          cudaMemcpyDeviceToDevice, g_model_upload_stream);
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(g_stream_selected_cache.up +
                                (uint64_t)i * gate_expert_bytes,
                            entry.up, (size_t)gate_expert_bytes,
                            cudaMemcpyDeviceToDevice, g_model_upload_stream);
    }
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(g_stream_selected_cache.down +
                                (uint64_t)i * down_expert_bytes,
                            entry.down, (size_t)down_expert_bytes,
                            cudaMemcpyDeviceToDevice, g_model_upload_stream);
    }
    if (err != cudaSuccess) {
      fprintf(stderr,
...
static int cuda_stream_selected_prepare_ptrs(
    ...
    uint64_t down_offset, uint64_t gate_expert_bytes,
    uint64_t down_expert_bytes) {
  if (!selected_ids || n_selected == 0 || n_selected > DS4_ROCM_N_EXPERT_USED ||
      !g_stream_selected_cache.gate_ptrs || !g_stream_selected_cache.up_ptrs ||
  ...
  cudaError_t err =
      cudaMemcpyAsync(g_stream_selected_cache.gate_ptrs, gate_ptrs,
                      n_selected * sizeof(gate_ptrs[0]), cudaMemcpyHostToDevice,
                      g_model_upload_stream);
  if (err == cudaSuccess) {
    err = cudaMemcpyAsync(g_stream_selected_cache.up_ptrs, up_ptrs,
                          n_selected * sizeof(up_ptrs[0]),
                          cudaMemcpyHostToDevice, g_model_upload_stream);
  }
  if (err == cudaSuccess) {
    err = cudaMemcpyAsync(g_stream_selected_cache.down_ptrs, down_ptrs,
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "streaming selected pointer upload sync failed: %s\n",
            cudaGetErrorString(err));
    (void)cudaGetLastError();
    return 0;
  }
...
static int cuda_stream_batch_selected_prepare_from_host(
    ...
    uint64_t down_expert_bytes, const ds4_gpu_tensor **selected_exec,
    const char ***gate_ptrs, const char ***up_ptrs, const char ***down_ptrs,
    uint32_t *unique_out, int begin_pending) {
  if (!g_ssd_streaming_mode || !model_map || !ids || !selected_exec ||
      !gate_ptrs || !up_ptrs || !down_ptrs || !unique_out || n_tokens <= 1 ||
  ...
  if (cuda_stream_batch_selected_cache_apply(
          model_map, layer, n_total_expert, n_selected, n_tokens, gate_offset,
          up_offset, down_offset, gate_expert_bytes, down_expert_bytes,
          selected_exec, gate_ptrs, up_ptrs, down_ptrs, unique_out)) {
    return 1;
  }
  ...
  for (uint32_t i = 0; i < DS4_ROCM_MAX_N_EXPERT; i++)
    map[i] = -1;
  ...
  if (ok) {
    for (uint64_t i = 0; i < n_ids64; i++) {
      ...
      if (expert < 0 || (uint32_t)expert >= n_total_expert) {
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX
                "streaming batch selected expert id %d outside 0..%u "
                "(layer=%u)\n",
  ...
  if (ok && !cuda_stream_selected_ensure_stream())
    ok = 0;
  ...
  const int use_fd = g_model_fd >= 0 && (g_model_fd_host_base == NULL ||
  ...
  for (uint32_t u = 0; ok && u < unique_count; u++) {
    ...
    if (!cuda_u64_mul_checked(expert, gate_expert_bytes, &gate_rel) ||
        ...
        gate_rel > model_size || down_rel > model_size ||
        gate_offset > model_size || up_offset > model_size ||
        down_offset > model_size || gate_rel > model_size - gate_offset ||
      ...
      fprintf(stderr, DS4_GPU_LOG_PREFIX
              "streaming batch selected expert offset overflow\n");
      ok = 0;
      break;
    }

    int idx = cuda_stream_resident_find(model_map, layer, expert_i, gate_offset,
    ...
    if (idx < 0) {
      idx = cuda_stream_resident_alloc(
          model_map, layer, expert_i, unique_ids, unique_count, gate_offset,
          up_offset, down_offset, gate_expert_bytes, down_expert_bytes);
      if (idx < 0) {
        ok = 0;
        break;
      ...
      if (use_fd) {
        ...
        read_jobs[read_job_count++] = {entry.gate,
        ...
        read_jobs[read_job_count++] = {
            entry.up, up_offset + gate_rel, gate_expert_bytes, NULL, NULL, 0,
            0};
        read_jobs[read_job_count++] = {entry.down,
        ...
        cudaError_t err = cudaMemcpyAsync(
            entry.gate, (const char *)model_map + gate_offset + gate_rel,
        ...
        if (err == cudaSuccess) {
          err = cudaMemcpyAsync(
              entry.down, (const char *)model_map + down_offset + down_rel,
        ...
        if (err != cudaSuccess) {
          fprintf(stderr,
                  DS4_GPU_LOG_PREFIX
                  "streaming batch selected cached copy failed: %s\n",
                  cudaGetErrorString(err));
          (void)cudaGetLastError();
          ok = 0;
          break;
  ...
  if (ok && begin_pending && use_fd && read_job_count != 0) {
    ...
    memcpy(g_stream_batch_selected_pending.read_jobs, read_jobs,
  ...
  if (ok && !use_fd) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming batch selected upload sync failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      ok = 0;
    }
  ...
  if (ok) {
    cudaError_t err =
        cudaMemcpyAsync(g_stream_batch_selected_cache.selected_ids, compact_ids,
                        (size_t)n_ids64 * sizeof(compact_ids[0]),
                        cudaMemcpyHostToDevice, g_model_upload_stream);
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(g_stream_batch_selected_cache.pair_missing,
                            pair_missing, (size_t)n_ids64,
                            cudaMemcpyHostToDevice, g_model_upload_stream);
    }
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(g_stream_batch_selected_cache.gate_ptrs, gate_host,
                            unique_count * sizeof(gate_host[0]),
                            cudaMemcpyHostToDevice, g_model_upload_stream);
    }
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(g_stream_batch_selected_cache.up_ptrs, up_host,
                            unique_count * sizeof(up_host[0]),
                            cudaMemcpyHostToDevice, g_model_upload_stream);
    }
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(g_stream_batch_selected_cache.down_ptrs, down_host,
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming batch selected table upload failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      if (g_stream_batch_selected_pending.active) {
...
static int cuda_stream_batch_selected_prepare(
    const void *model_map, uint64_t model_size, uint32_t layer,
    const ds4_gpu_tensor *selected, uint32_t n_tokens, uint32_t n_total_expert,
    uint32_t n_selected, uint64_t gate_offset, uint64_t up_offset,
    uint64_t down_offset, uint64_t gate_expert_bytes,
    uint64_t down_expert_bytes, const ds4_gpu_tensor **selected_exec,
    const char ***gate_ptrs, const char ***up_ptrs, const char ***down_ptrs,
    uint32_t *unique_out) {
  if (!selected || !cuda_tensor_has_elems2(selected, n_tokens, n_selected,
                                           sizeof(int32_t))) {
    return 0;
  }
  if (cuda_stream_batch_selected_cache_apply(
  ...
  const int copy_ok =
      cuda_ok(cudaMemcpy(ids, selected->ptr, (size_t)n_ids64 * sizeof(ids[0]),
                         cudaMemcpyDeviceToHost),
              "streaming batch selected ids copy");
  const int ok = copy_ok && cuda_stream_batch_selected_prepare_from_host(
                                model_map, model_size, layer, ids, n_tokens,
                                n_total_expert, n_selected, gate_offset,
                                up_offset, down_offset, gate_expert_bytes,
                                down_expert_bytes, selected_exec, gate_ptrs,
                                up_ptrs, down_ptrs, unique_out, 0);
  free(ids);
  return ok;
}

static int cuda_stream_layer_expert_cache_prepare_batch(
    const void *model_map, uint32_t layer, const ds4_gpu_tensor *selected,
    uint32_t n_tokens, uint32_t n_total_expert, uint32_t n_selected,
    uint64_t gate_offset, uint64_t up_offset, uint64_t down_offset,
    uint64_t gate_expert_bytes, uint64_t down_expert_bytes,
    const ds4_gpu_tensor **selected_exec, const char ***gate_ptrs,
    const char ***up_ptrs, const char ***down_ptrs, uint32_t *unique_out) {
  if (!selected || !selected_exec || !gate_ptrs || !up_ptrs || !down_ptrs ||
      !unique_out ||
      !cuda_tensor_has_elems2(selected, n_tokens, n_selected,
                              sizeof(int32_t)) ||
      n_tokens <= 1 || n_total_expert == 0 ||
      n_total_expert > DS4_ROCM_MAX_N_EXPERT || n_selected == 0 ||
      n_selected > DS4_ROCM_N_EXPERT_USED || gate_expert_bytes == 0 ||
      down_expert_bytes == 0) {
    return 0;
  }
  if (cuda_stream_batch_selected_cache_apply(
  ...
  if (!cuda_stream_layer_expert_cache_apply(
          model_map, layer, n_total_expert, gate_offset, up_offset, down_offset,
          gate_expert_bytes, down_expert_bytes, &layer_gate, &layer_up,
          &layer_down)) {
    return 0;
  }
  ...
  if (ok) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming full-layer selected table upload failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      ok = 0;
    }
...
static int cuda_stream_layer_expert_cache_seed_selected(
    const void *model_map, uint32_t layer, const ds4_gpu_tensor *selected,
    uint32_t n_tokens, uint32_t n_seed_tokens, uint32_t n_total_expert,
    uint32_t n_selected, uint64_t gate_offset, uint64_t up_offset,
    uint64_t down_offset, uint64_t gate_expert_bytes,
    uint64_t down_expert_bytes) {
  if (!g_ssd_streaming_mode || !model_map || !selected || n_tokens == 0 ||
      n_seed_tokens == 0 || n_total_expert == 0 ||
      n_total_expert > DS4_ROCM_MAX_N_EXPERT || n_selected == 0 ||
      n_selected > DS4_ROCM_N_EXPERT_USED || gate_expert_bytes == 0 ||
      down_expert_bytes == 0 ||
      !cuda_tensor_has_elems2(selected, n_tokens, n_selected,
  ...
  if (n_seed_tokens > n_tokens)
    n_seed_tokens = n_tokens;
  ...
  int ok = cuda_ok(cudaMemcpy(ids, (const char *)selected->ptr + src_off,
  ...
  for (uint32_t u = 0; ok && u < unique_count; u++) {
    ...
    int idx = cuda_stream_resident_find(model_map, layer, expert_i32,
                                        gate_offset, up_offset, down_offset,
                                        gate_expert_bytes, down_expert_bytes);
    if (idx >= 0) {
    ...
    idx = cuda_stream_resident_alloc(
        model_map, layer, expert_i32, unique, unique_count, gate_offset,
        up_offset, down_offset, gate_expert_bytes, down_expert_bytes);
    if (idx < 0) {
      ok = 0;
      break;
    ...
    cuda_stream_resident_expert &entry = g_stream_resident_experts[(size_t)idx];
    cudaError_t err = cudaMemcpyAsync(
        entry.gate, layer_gate + gate_rel, (size_t)gate_expert_bytes,
        cudaMemcpyDeviceToDevice, g_model_upload_stream);
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(entry.up, layer_up + gate_rel,
                            (size_t)gate_expert_bytes, cudaMemcpyDeviceToDevice,
                            g_model_upload_stream);
    }
    if (err == cudaSuccess) {
      err = cudaMemcpyAsync(entry.down, layer_down + down_rel,
                            (size_t)down_expert_bytes, cudaMemcpyDeviceToDevice,
                            g_model_upload_stream);
    }
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming full-layer seed D2D copy failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      ok = 0;
      break;
...
static int cuda_stream_selected_copy_range(const void *model_map, char *dst,
                                           uint64_t offset, uint64_t bytes,
                                           const char *what) {
  if (!model_map || !dst || bytes == 0)
    return 0;
  if (!cuda_stream_selected_ensure_stream())
    return 0;
  ...
  const int use_fd = g_model_fd >= 0 && (g_model_fd_host_base == NULL ||
                                         model_map == g_model_fd_host_base);
  if (use_fd && !cuda_model_stage_pool_alloc(stage_bytes))
    return 0;
  ...
  while (copied < bytes) {
    ...
    if (use_fd) {
      if (chunk_idx >= 4u) {
        ...
        if (err != cudaSuccess) {
          fprintf(stderr,
                  DS4_GPU_LOG_PREFIX
                  "streaming selected staging wait failed for %s: %s\n",
      ...
      if (!cuda_model_stage_read(g_model_stage[bi], g_model_stage_bytes,
        ...
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX
                "streaming selected read failed for %s at %.2f MiB: %s\n",
    ...
    cudaError_t err =
        cudaMemcpyAsync(dst + copied, payload, (size_t)n,
                        cudaMemcpyHostToDevice, g_model_upload_stream);
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected copy failed for %s at %.2f MiB: %s\n",
              what ? what : "expert", (double)copied / 1048576.0,
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    ...
    if (use_fd) {
      ...
      if (err != cudaSuccess) {
        fprintf(
            stderr,
            DS4_GPU_LOG_PREFIX
            "streaming selected staging record failed for %s at %.2f MiB: %s\n",
            what ? what : "expert", (double)copied / 1048576.0,
            cudaGetErrorString(err));
        (void)cudaGetLastError();
        return 0;
...
static int cuda_stream_selected_load(
  ...
  if (!cuda_stream_selected_ensure_buffers(gate_bytes, down_bytes))
    return 0;
  if (!cuda_stream_selected_ensure_stream())
    return 0;
  cuda_stream_selected_cache_header(model_map, layer, n_total_expert,
                                    n_selected, selected_ids, gate_expert_bytes,
                                    down_expert_bytes);
  if (cuda_stream_cache_stats_on()) {
  ...
  for (uint32_t i = 0; i < n_selected; i++) {
    if (selected_ids[i] < 0 || (uint32_t)selected_ids[i] >= n_total_expert) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "streaming selected expert id %d outside 0..%u "
              "(layer=%u slot=%u selected=[%d,%d,%d,%d,%d,%d])\n",
              selected_ids[i], n_total_expert, layer, i,
    ...
    if (!cuda_u64_mul_checked(expert, gate_expert_bytes, &gate_rel) ||
        ...
        down_expert_bytes > model_size - down_offset - down_rel) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "streaming selected expert offset overflow\n");
      return 0;
    }
    ...
    idx = cuda_stream_resident_alloc(model_map, layer, selected_ids[i],
                                     selected_ids, n_selected, gate_offset,
                                     up_offset, down_offset, gate_expert_bytes,
                                     down_expert_bytes);
    if (idx < 0)
      return 0;
    ...
    if (use_fd) {
      if (read_job_count + 3u > DS4_ROCM_N_EXPERT_USED * 3u)
        return 0;
      read_jobs[read_job_count++] = {
          entry.gate, gate_offset + gate_rel, gate_expert_bytes, NULL, NULL, 0,
          0};
      read_jobs[read_job_count++] = {
          entry.up, up_offset + gate_rel, gate_expert_bytes, NULL, NULL, 0, 0};
      read_jobs[read_job_count++] = {
          entry.down, down_offset + down_rel, down_expert_bytes, NULL, NULL, 0,
          0};
    } else {
      cudaError_t err = cudaMemcpyAsync(
          entry.gate, (const char *)model_map + gate_offset + gate_rel,
      ...
      if (err != cudaSuccess) {
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX
                "streaming selected cached copy failed: %s\n",
                cudaGetErrorString(err));
        (void)cudaGetLastError();
        cuda_stream_resident_cache_release();
        return 0;
  ...
  if (use_fd && read_job_count != 0 && resident_mask != 0) {
    ...
    memcpy(g_stream_selected_pending.read_jobs, read_jobs,
           (size_t)read_job_count * sizeof(read_jobs[0]));
    ...
    if (!cuda_stream_selected_prepare_ptrs(
            model_map, layer, selected_ids, n_selected, gate_offset, up_offset,
  ...
  if (resident_mask != 0 &&
      !cuda_stream_selected_compact_mask(
          model_map, layer, selected_ids, n_total_expert, n_selected,
          gate_offset, up_offset, down_offset, gate_expert_bytes,
          down_expert_bytes, resident_mask)) {
    cuda_stream_read_jobs_free(read_jobs, read_job_count);
    cuda_stream_resident_cache_release();
    return 0;
  }
...

  {
    ...
    if (!cuda_stream_selected_compact_mask(
            model_map, layer, selected_ids, n_total_expert, n_selected,
            gate_offset, up_offset, down_offset, gate_expert_bytes,
            down_expert_bytes, compact_mask)) {
      cuda_stream_resident_cache_release();
      return 0;
    }
  }
...
static int cuda_stream_selected_pending_matches(const void *model_map,
                                                uint32_t layer,
                                                uint32_t n_total_expert,
                                                uint32_t n_selected,
                                                uint64_t gate_expert_bytes,
...
static int cuda_stream_selected_apply_split(
    const void *model_map, uint32_t layer, uint32_t n_total_expert,
    uint32_t n_selected, uint64_t gate_expert_bytes, uint64_t down_expert_bytes,
    const ds4_gpu_tensor **selected_exec, const char **gate_w,
    const char **up_w, const char **down_w, const char ***gate_ptrs,
    const char ***up_ptrs, const char ***down_ptrs, uint32_t *resident_mask,
    uint32_t *missing_mask) {
  if (!g_ssd_streaming_mode || !selected_exec || !gate_w || !up_w || !down_w ||
      !gate_ptrs || !up_ptrs || !down_ptrs || !resident_mask || !missing_mask ||
      !cuda_stream_selected_pending_matches(model_map, layer, n_total_expert,
                                            n_selected, gate_expert_bytes,
                                            down_expert_bytes)) {
    return 0;
  }
...
static int cuda_stream_selected_finish_pending_missing(uint32_t compact_mask) {
  ...
  if (!cuda_stream_read_jobs_wait(g_stream_selected_pending.read_jobs,
                                  read_job_count) ||
      !cuda_stream_selected_upload_read_jobs(
          g_stream_selected_pending.read_jobs, read_job_count)) {
    cuda_stream_read_jobs_free(g_stream_selected_pending.read_jobs,
                               read_job_count);
    memset(&g_stream_selected_pending, 0, sizeof(g_stream_selected_pending));
    cuda_stream_resident_cache_release();
  ...
  if (compact_mask != 0 &&
      !cuda_stream_selected_compact_mask(
          g_stream_selected_pending.model_map, g_stream_selected_pending.layer,
          ...
          g_stream_selected_pending.down_expert_bytes, compact_mask)) {
    memset(&g_stream_selected_pending, 0, sizeof(g_stream_selected_pending));
    cuda_stream_resident_cache_release();
    return 0;
  }
...
static int cuda_stream_selected_apply(
    ...
    const ds4_gpu_tensor **selected_exec, const char **gate_w,
    const char **up_w, const char **down_w) {
  if (g_ssd_streaming_mode && !g_stream_selected_cache.loaded &&
      getenv("DS4_ROCM_DISABLE_STREAMING_SPLIT_SELECTED") != NULL &&
      cuda_stream_selected_pending_matches(model_map, layer, n_total_expert,
                                           n_selected, gate_expert_bytes,
                                           down_expert_bytes)) {
    const uint32_t compact_mask = g_stream_selected_pending.resident_mask |
  ...
  if (!g_ssd_streaming_mode || !g_stream_selected_cache.loaded ||
      !selected_exec || !gate_w || !up_w || !down_w ||
      g_routed_moe_selected_override_n != n_selected ||
      g_stream_selected_cache.model_map != model_map ||
      g_stream_selected_cache.layer != layer ||
      g_stream_selected_cache.n_total_expert != n_total_expert ||
...
static const char *cuda_model_range_copy_uncached(const void *model_map,
                                                  uint64_t offset,
                                                  uint64_t bytes,
                                                  const char *what) {
  void *dev = NULL;
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "model range alloc failed for %s (%.2f MiB): %s\n",
  ...
  g_model_ranges.push_back(
      {model_map, offset, bytes, (char *)dev, NULL, NULL, 0, 0, 0});
...
static const char *cuda_model_range_ptr(const void *model_map, uint64_t offset,
  ...
  for (const cuda_model_range &r : g_model_ranges) {
    ...
    if (r.host_base == model_map && r.host_registered && r.registered_base &&
  ...
  const char *fd_ptr =
      cuda_model_range_ptr_from_fd(model_map, offset, bytes, what);
  if (fd_ptr)
    return fd_ptr;
  ...
  if (g_model_range_mapping_supported && model_map == g_model_host_base) {
    ...
    const uint64_t reg_bytes =
        (reg_delta + bytes + page_sz - 1u) & ~(page_sz - 1u);
    void *reg_dev = NULL;
    ...
    if (err == cudaSuccess) {
      ...
      if (err == cudaSuccess && reg_dev) {
        ...
        g_model_ranges.push_back({model_map, offset, bytes, dev_ptr,
                                  (void *)reg_addr, (char *)reg_dev, reg_bytes,
                                  1, 0});
        g_model_range_by_offset[offset] = g_model_ranges.size() - 1u;
        return dev_ptr;
      }
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "model range map pointer failed for %s: %s\n",
              what ? what : "weights", cudaGetErrorString(err));
  ...
  for (uint64_t done = 0; done < bytes; done += chunk) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "model range copy failed for %s at %.2f/%.2f MiB: %s\n",
              what ? what : "weights", (double)done / 1048576.0,
...
static int cuda_model_range_is_cached(const void *model_map, uint64_t offset,
                                      uint64_t bytes) {
  if (bytes == 0)
    return 1;
  if (cuda_model_image_owned(model_map))
...
static uint64_t cuda_q8_f16_cache_limit_bytes(void) { return UINT64_MAX; }
...
static void
cuda_q8_f16_cache_budget_notice(const char *reason, uint64_t request_bytes,
                                uint64_t free_bytes, uint64_t total_bytes,
                                uint64_t reserve_bytes, uint64_t limit_bytes) {
  if (g_q8_f16_budget_notice_printed)
    return;
  g_q8_f16_budget_notice_printed = 1;
  if (limit_bytes != UINT64_MAX && free_bytes == 0 && total_bytes == 0 &&
      reserve_bytes == 0) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "q8 fp16 cache %s; using q8 kernels "
            "(request=%.2f MiB cached=%.2f GiB limit=%.2f GiB)\n",
            reason, (double)request_bytes / 1048576.0,
    ...
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "q8 fp16 cache %s; using q8 kernels "
                               "(request=%.2f MiB cached=%.2f GiB free=%.2f "
                               "GiB reserve=%.2f GiB total=%.2f GiB)\n",
            reason, (double)request_bytes / 1048576.0,
            (double)g_q8_f16_bytes / 1073741824.0,
    ...
    fprintf(
        ...
        DS4_GPU_LOG_PREFIX "q8 fp16 cache %s; using q8 kernels "
                           "(request=%.2f MiB cached=%.2f GiB limit=%.2f GiB "
                           "free=%.2f GiB reserve=%.2f GiB total=%.2f GiB)\n",
        reason, (double)request_bytes / 1048576.0,
        (double)g_q8_f16_bytes / 1073741824.0,
        (double)limit_bytes / 1073741824.0, (double)free_bytes / 1073741824.0,
        (double)reserve_bytes / 1073741824.0,
        (double)total_bytes / 1073741824.0);
  }
}

static int cuda_q8_f16_cache_has_budget(uint64_t request_bytes,
                                        const char *label) {
  (void)label;
  ...
  if (g_q8_f16_bytes > limit || request_bytes > limit - g_q8_f16_bytes) {
    cuda_q8_f16_cache_budget_notice("limit reached", request_bytes, 0, 0, 0,
                                    limit);
    return 0;
  }
...
static int cuda_q8_f16_cache_allowed(const char *label, uint64_t in_dim,
                                     uint64_t out_dim) {
  if (g_quality_mode)
    return 0;
  if (g_q8_f16_disabled_after_oom)
  ...
  if (g_q8_f16_disabled_for_multi_model)
    return 0;
  if (getenv("DS4_CUDA_NO_Q8_F16_CACHE") != NULL)
    return 0;
  if (!label)
    return 0;
...
static int cuda_q8_label_is_attention_output(const char *label) {
  return label && (strstr(label, "attn_output_a") != NULL ||
                   strstr(label, "attn_output_b") != NULL ||
                   strstr(label, "attention_output_a") != NULL ||
                   strstr(label, "attention_output_b") != NULL);
}

static int cuda_q8_f16_preload_allowed(const char *label, uint64_t in_dim,
                                       uint64_t out_dim) {
...
static const __half *cuda_q8_f16_ptr(const void *model_map, uint64_t offset,
                                     uint64_t weight_bytes, uint64_t in_dim,
  ...
  if (in_dim == 0u || out_dim == 0u ||
      !cuda_u64_mul3_checked(in_dim, out_dim, sizeof(__half), &out_bytes))
    return NULL;
  if (!cuda_q8_f16_cache_has_budget(out_bytes, label))
    return NULL;

  const char *q8 =
      cuda_model_range_ptr(model_map, offset, weight_bytes, "q8_0");
  if (!q8)
    return NULL;
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "q8 fp16 cache alloc failed (%.2f MiB): %s\n",
  ...
  if (!cuda_ok(cudaGetLastError(), "q8 fp16 dequant launch")) {
    (void)cudaFree(dev);
    cuda_q8_f16_cache_disable_after_failure("dequant launch failure",
                                            out_bytes);
    return NULL;
  }
  g_q8_f16_ranges.push_back(
      {model_map, offset, weight_bytes, in_dim, out_dim, dev});
...
static const __half *
cuda_q8_f16_transpose_ptr(const void *model_map, uint64_t offset,
                          uint64_t weight_bytes, uint64_t in_dim,
                          uint64_t out_dim, const char *label) {
  auto exact = g_q8_f16_transpose_by_offset.find(offset);
  if (exact != g_q8_f16_transpose_by_offset.end()) {
    const cuda_q8_f16_transpose_range &r =
        g_q8_f16_transpose_ranges[exact->second];
    if (r.host_base == model_map && r.weight_bytes == weight_bytes &&
        r.in_dim == in_dim && r.out_dim == out_dim) {
      return r.device_ptr;
    }
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            ...
            (double)out_bytes / 1048576.0, cudaGetErrorString(err));
    cuda_q8_f16_cache_disable_after_failure("transpose allocation failure",
                                            out_bytes);
    return NULL;
  }
  ...
  if (!cuda_ok(cudaGetLastError(), "q8 fp16 transpose dequant launch")) {
    (void)cudaFree(dev);
    cuda_q8_f16_cache_disable_after_failure("transpose launch failure",
                                            out_bytes);
    return NULL;
  }
  g_q8_f16_transpose_ranges.push_back(
      {model_map, offset, weight_bytes, in_dim, out_dim, dev});
...
static uint32_t cuda_prefill_warmup_tokens(void) {
  ...
  if (chunk_env && chunk_env[0]) {
    ...
    if (end != chunk_env && *end == '\0' && v > 0 && v <= 4096u)
      n_tok = (uint32_t)v;
  }
  return n_tok;
}
...
static void cuda_q8_f16_warmup_attention_output_a_gemm(const __half *out_a_f16,
                                                       ...
                                                       uint32_t n_groups) {
  static int warmed = 0;
  if (warmed || !g_cublas_ready || !out_a_f16 || group_dim == 0 || rank == 0 ||
      n_groups == 0)
    return;
  const ds4_rocm_runtime_config *cfg = cuda_runtime_config();
  if (!cfg->attention_output_cublas_all)
    return;
  warmed = 1;
  ...
  if (low_h_count > (UINT64_MAX - low_h_off) / sizeof(__half))
    return;
  void *tmp = cuda_tmp_alloc(low_h_off + low_h_count * sizeof(__half),
                             "attention output a warmup");
  if (!tmp)
    return;
  __half *heads_h = (__half *)tmp;
  __half *low_h = (__half *)((char *)tmp + low_h_off);
  if (cudaMemset(heads_h, 0, (size_t)heads_h_bytes) != cudaSuccess)
    return;
  const float alpha = 1.0f;
  const float beta = 0.0f;
  cublasStatus_t st = cublasGemmStridedBatchedEx(
      g_cublas, CUBLAS_OP_T, CUBLAS_OP_N, (int)rank, (int)n_tok, (int)group_dim,
      &alpha, out_a_f16, CUDA_R_16F, (int)group_dim,
      (long long)rank * (long long)group_dim, heads_h, CUDA_R_16F,
      (int)group_dim, (long long)n_tok * (long long)group_dim, &beta, low_h,
      CUDA_R_16F, (int)(n_groups * rank), (long long)rank, (int)n_groups,
...
static void
cuda_q8_f16_warmup_attention_output_b_gemm(const __half *out_b_f16_t,
                                           uint64_t low_dim, uint64_t out_dim) {
  static int warmed = 0;
  if (warmed || !g_cublas_ready || !out_b_f16_t || low_dim == 0 || out_dim == 0)
    return;
  ...
  if (out_count > (UINT64_MAX - out_off) / sizeof(float))
    return;
  void *tmp = cuda_tmp_alloc(out_off + out_count * sizeof(float),
                             "attention output b warmup");
  if (!tmp)
    return;
  __half *low_h = (__half *)tmp;
  float *out = (float *)((char *)tmp + out_off);
  if (cudaMemset(low_h, 0, (size_t)low_h_bytes) != cudaSuccess)
    return;
  const float alpha = 1.0f;
  const float beta = 0.0f;
  cublasStatus_t st =
      cublasGemmEx(g_cublas, CUBLAS_OP_N, CUBLAS_OP_N, (int)out_dim, (int)n_tok,
                   (int)low_dim, &alpha, out_b_f16_t, CUDA_R_16F, (int)out_dim,
                   low_h, CUDA_R_16F, (int)low_dim, &beta, out, CUDA_R_32F,
                   (int)out_dim, CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT);
  if (st == CUBLAS_STATUS_SUCCESS)
    (void)cudaDeviceSynchronize();
}
...
static void cuda_model_load_progress_note(uint64_t cached_bytes) {
  if (!cuda_model_load_progress_enabled())
    return;
  ...
  if (!g_model_load_progress_started) {
    ...
    if (g_model_load_progress_tty) {
      fprintf(stderr, DS4_GPU_LOG_PREFIX
              "loading model tensors into device cache: 0.00 GiB");
    } else {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "loading model tensors into device cache\n");
    }
  }

  if (cached_bytes < g_model_load_progress_next &&
      now - g_model_load_progress_last <
          (g_model_load_progress_tty ? 2.0 : 10.0)) {
    return;
  }

  if (g_model_load_progress_tty) {
    fprintf(stderr,
            "\r" DS4_GPU_LOG_PREFIX
            "loading model tensors into device cache: %.2f GiB",
            (double)cached_bytes / 1073741824.0);
  } else {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "loading model tensors %.2f GiB cached\n",
            (double)cached_bytes / 1073741824.0);
  }
  fflush(stderr);
  g_model_load_progress_last = now;
  const uint64_t step =
      (g_model_load_progress_tty ? 2ull : 16ull) * 1024ull * 1024ull * 1024ull;
...
static void cuda_model_discard_source_pages(const void *model_map,
                                            uint64_t model_size,
                                            uint64_t offset, uint64_t bytes) {
#if defined(POSIX_MADV_DONTNEED)
  if (!model_map || bytes == 0 || offset > model_size)
    return;
  if (bytes > model_size - offset)
  ...
  if (p1 > p0)
    (void)posix_madvise((void *)p0, (size_t)(p1 - p0), POSIX_MADV_DONTNEED);
#else
  (void)model_map;
  (void)model_size;
  (void)offset;
...
static void cuda_model_drop_file_pages(uint64_t offset, uint64_t bytes) {
  ...
  if (g_model_fd < 0 || bytes == 0)
    return;
  (void)posix_fadvise(g_model_fd, (off_t)offset, (off_t)bytes,
                      POSIX_FADV_DONTNEED);
#else
  (void)offset;
  (void)bytes;
...
static int cuda_model_stage_pool_alloc(uint64_t bytes) {
  ...
  if (!g_model_upload_stream) {
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "model upload stream creation failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
  ...
  if (g_model_direct_align > 1u) {
    ...
    if (alloc_bytes > UINT64_MAX - pad)
      return 0;
    alloc_bytes += pad;
  }
  if (alloc_bytes > (uint64_t)SIZE_MAX)
    return 0;
  for (size_t i = 0; i < 4; i++) {
    cudaError_t err =
        cudaMallocHost(&g_model_stage_raw[i], (size_t)alloc_bytes);
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "pinned model staging allocation failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
    g_model_stage[i] =
        cuda_align_ptr(g_model_stage_raw[i], g_model_direct_align);
    err = cudaEventCreateWithFlags(&g_model_stage_event[i],
                                   cudaEventDisableTiming);
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "model staging event creation failed: %s\n",
              cudaGetErrorString(err));
      (void)cudaGetLastError();
      return 0;
    }
...
static int cuda_model_stage_read(void *stage, uint64_t stage_bytes,
  ...
  *payload = (const char *)stage;
  ...
  if (g_model_direct_fd >= 0 && g_model_direct_align > 1 &&
    ...
    if (aligned_off <= g_model_file_size && read_size <= stage_bytes &&
...
static uint64_t cuda_model_cache_limit_bytes(void) { return UINT64_MAX; }
...
static char *cuda_model_arena_alloc(uint64_t bytes, const char *what) {
  ...
  if (g_model_range_bytes > limit || aligned > limit - g_model_range_bytes)
    return NULL;
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX
            "model arena alloc failed for %s (%.2f MiB chunk): %s\n",
            what ? what : "weights", (double)chunk / 1048576.0,
            cudaGetErrorString(err));
    (void)cudaGetLastError();
...
static const char *cuda_model_range_ptr_from_fd(const void *model_map,
  ...
  while (copied < bytes) {
    ...
    if (chunk_idx >= 4u) {
      ...
      if (err != cudaSuccess) {
        fprintf(stderr,
                DS4_GPU_LOG_PREFIX "model staging wait failed for %s: %s\n",
                what ? what : "weights", cudaGetErrorString(err));
        (void)cudaGetLastError();
        return NULL;
      }
    ...
    if (!cuda_model_stage_read(g_model_stage[bi], g_model_stage_bytes,
      ...
      fprintf(
          stderr,
          DS4_GPU_LOG_PREFIX "model range read failed for %s at %.2f MiB: %s\n",
          what ? what : "weights", (double)copied / 1048576.0, strerror(errno));
      return NULL;
    }
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX
              "model range copy failed for %s at %.2f MiB: %s\n",
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "model staging record failed for %s: %s\n",
              what ? what : "weights", cudaGetErrorString(err));
      (void)cudaGetLastError();
      return NULL;
    }
  ...
  if (err != cudaSuccess) {
    fprintf(stderr,
            DS4_GPU_LOG_PREFIX "model range upload sync failed for %s: %s\n",
            what ? what : "weights", cudaGetErrorString(err));
    (void)cudaGetLastError();
    return NULL;
  }
  ...
  g_model_ranges.push_back(
      {model_map, offset, bytes, dev, NULL, NULL, 0, 0, 1});
  g_model_range_by_offset[offset] = g_model_ranges.size() - 1u;
  g_model_range_bytes += bytes;
...
static int cuda_model_copy_chunked(const void *model_map, uint64_t model_size,
  ...
  while (copied < model_size) {
    const uint64_t n =
        (model_size - copied < chunk) ? (model_size - copied) : chunk;
    const uint64_t bi = chunk_idx % 4u;
    if (chunk_idx >= 4u) {
      err = cudaEventSynchronize(g_model_stage_event[bi]);
      if (err != cudaSuccess) {
    ...
    const char *payload = NULL;
    if (!cuda_model_stage_read(g_model_stage[bi], g_model_stage_bytes, copied,
                               n, &payload)) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "model staged read failed at %.2f GiB: %s\n",
    ...
    if (err != cudaSuccess) {
      fprintf(stderr,
              DS4_GPU_LOG_PREFIX "model chunk copy failed at %.2f GiB: %s\n",
    ...
    copied += n;
    chunk_idx++;
    cuda_model_load_progress_note(copied > map_offset ? copied - map_offset
                                                      : 0);
  }
  err = cudaStreamSynchronize(g_model_upload_stream);
  if (err != cudaSuccess) {
  ...
  fprintf(stderr,
          DS4_GPU_LOG_PREFIX
          "model chunk copy complete in %.3fs (%.2f GiB tensors)\n",
...
extern "C" int ds4_gpu_init(void) {
  int dev = 0;
  if (!cuda_ok(cudaSetDevice(dev), "set device"))
    return 0;
  cudaDeviceProp prop;
  if (cudaGetDeviceProperties(&prop, dev) == cudaSuccess) {
    fprintf(stderr, DS4_GPU_LOG_PREFIX "backend initialized on %s (sm_%d%d)\n",
            prop.name, prop.major, prop.minor);
    g_rocm_cfg.gfx12_device = (prop.major >= 12);
  }
  if (!g_cublas_ready) {
    if (!cublas_ok(cublasCreate(&g_cublas), "create handle"))
      return 0;
...
extern "C" ds4_gpu_tensor *ds4_gpu_tensor_alloc_managed(uint64_t bytes) {
  ...
  if (!t)
    return NULL;
  if (!cuda_ok(cudaMallocManaged(&t->ptr, (size_t)bytes),
...
extern "C" int ds4_gpu_should_use_managed_kv_cache(uint64_t kv_cache_bytes,
  ...
  if (kv_cache_bytes >= huge_kv)
    return 1;

  const uint64_t large_context = 8ull * 1073741824ull;
  if (context_bytes < large_context)
    return 0;
...
extern "C" int ds4_gpu_tensor_fill_f32(ds4_gpu_tensor *tensor, float value,
                                       uint64_t count) {
  if (!tensor || count > tensor->bytes / sizeof(float))
    return 0;
  if (count == 0)
    return 1;
  fill_f32_kernel<<<(count + 255u) / 256u, 256>>>((float *)tensor->ptr, count,
                                                  value);
...
extern "C" int ds4_gpu_tensor_copy(ds4_gpu_tensor *dst, uint64_t dst_offset,
                                   ...
                                   uint64_t src_offset, uint64_t bytes) {
...
extern "C" int ds4_gpu_set_model_map(const void *model_map,
                                     uint64_t model_size) {
...
extern "C" int ds4_gpu_set_model_map_range(const void *model_map,
                                           uint64_t model_size,
                                           uint64_t map_offset,
                                           uint64_t map_size,
  ...
  if (!ds4_gpu_set_model_map(model_map, model_size))
    return 0;
  if (g_ssd_streaming_mode) {
    if (!cuda_model_range_ptr(model_map, map_offset, map_size, "stream_range"))
      return 0;
...
extern "C" int
ds4_gpu_set_model_map_spans(const void *model_map, uint64_t model_size,
  ...
  if (g_ssd_streaming_mode) {
    for (uint32_t i = 0; i < count; i++) {
      if (!cuda_model_range_ptr(model_map, offsets[i], sizes[i], "stream_span"))
        return 0;
   ...
   * Leave the precise per-tensor preparation to
  ...
  if (fd >= 0) {
...
#if defined(__linux__) && defined(O_DIRECT)
    {
      ...
      if (direct_fd >= 0) {
        ...
        if (g_model_direct_align < 512)
          g_model_direct_align = 512;
      }
    }
#endif
  }
...
extern "C" int ds4_gpu_cache_model_range(const void *model_map,
                                         uint64_t model_size, uint64_t offset,
                                         uint64_t bytes, const char *label) {
  if (!model_map || bytes == 0)
    return 1;
  if (offset > model_size || bytes > model_size - offset)
    return 0;
  if (!cuda_model_range_ptr(model_map, offset, bytes,
                            label ? label : "model_tensor"))
    return 0;
  return cuda_model_range_is_cached(model_map, offset, bytes);
}

extern "C" int ds4_gpu_cache_q8_f16_range(const void *model_map,
                                          uint64_t model_size, uint64_t offset,
                                          uint64_t bytes, uint64_t in_dim,
                                          uint64_t out_dim, const char *label) {
  if (!model_map || bytes == 0)
    return 1;
  if (offset > model_size || bytes > model_size - offset)
  ...
  if (optional_q8_preload_disabled)
    return 1;
  const char *cache_label = label ? label : "q8_0";
  if (!cuda_q8_f16_preload_allowed(cache_label, in_dim, out_dim))
    return 1;
  const int preload_transposed_b =
      !g_quality_mode && strstr(cache_label, "attn_output_b") != NULL;
  if (preload_transposed_b) {
    const __half *f16_t = cuda_q8_f16_transpose_ptr(
        model_map, offset, bytes, in_dim, out_dim, cache_label);
    if (f16_t) {
      if (strstr(cache_label, "attn_output_b") != NULL && in_dim == 8192u &&
          out_dim == 4096u) {
    ...
    const __half *f16 =
        cuda_q8_f16_ptr(model_map, offset, bytes, in_dim, out_dim, cache_label);
    if (f16) {
      if (strstr(cache_label, "attn_output_a") != NULL && in_dim == 4096u &&
          out_dim == 8192u) {
...
extern "C" void ds4_gpu_print_memory_report(const char *label) {
  ...
  const char *placement =
      cuda_model_image_bytes() ? "device_copy" : "mapped/range_cache";
  fprintf(stderr,
          DS4_GPU_LOG_PREFIX
          "memory %s: used=%.2f GiB free=%.2f GiB total=%.2f GiB "
          "placement=%s model_image=%.2f GiB range_cache=%.2f GiB "
          "q8_f16_cache=%.2f GiB scratch=%.2f GiB",
          label ? label : "", (double)used_b / 1073741824.0,
          (double)free_b / 1073741824.0, (double)total_b / 1073741824.0,
          placement, (double)cuda_model_image_bytes() / 1073741824.0,
